 ...
 _toString()
{
	...
	_pDecoder->cloneBufferToMsg(&const_cast<RefreshMsg&>(other), functionName);

	if (other.hasMsgKey()) // Override with the message key generated by the Watchlist
	{
		RefreshMsgDecoder* refDecoder = static_cast<RefreshMsgDecoder*>(_pDecoder);

		refDecoder->cloneMsgKey(other);
	}
	...
	{
		static_cast<RefreshMsgDecoder*>(_pDecoder)->setServiceName(other.getServiceName());
	}
