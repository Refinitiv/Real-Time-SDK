 ...
 _toString()
{
	...
	_pDecoder->cloneBufferToMsg(&const_cast<GenericMsg&>(other), functionName);

	if (other.hasMsgKey()) // Override with the message key generated by the Watchlist
	{
		GenericMsgDecoder* genDecoder = static_cast<GenericMsgDecoder*>(_pDecoder);

		genDecoder->cloneMsgKey(other);
	}
