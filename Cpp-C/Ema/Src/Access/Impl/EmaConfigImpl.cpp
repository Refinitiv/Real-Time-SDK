...
void EmaConfigImpl::operationModel( OmmConsumerConfig::OperationModel operationModel )
{
	_operationModel = operationModel;
}

OmmConsumerConfig::OperationModel EmaConfigImpl::getOperationModel() const
{
...
EmaString EmaConfigImpl::getChannelName( const EmaString& userName ) const
{
	EmaString retVal;
	if ( _pProgrammaticConfigure && _pProgrammaticConfigure->getActiveChannelName(userName, retVal ) )
	{
		return retVal;
	}

	EmaString nodeName( _userNodeName );
	nodeName.append( userName );
...
EmaString EmaConfigImpl::getLoggerName( const EmaString& userName ) const
{
  EmaString retVal;

  if ( _pProgrammaticConfigure && _pProgrammaticConfigure->getActiveLoggerName(userName, retVal ) )
    return retVal;

  EmaString nodeName( _userNodeName );
  nodeName.append( userName ).append("|Logger");
...
EmaString EmaConfigImpl::getDictionaryName( const EmaString& userName ) const
{
	EmaString retVal;
	if ( _pProgrammaticConfigure && _pProgrammaticConfigure->getActiveDictionaryName(userName, retVal ) )
	{
		return retVal;
	}
...
bool EmaConfigImpl::validateConfigElement( const char * value, ConfigElement::ConfigElementType valueType )
{
...
AdminReqMsg::AdminReqMsg( EmaConfigImpl& ommConsConfigImpl ) :
 _emaConfigImpl( ommConsConfigImpl ),
