 ...
 *|           Copyright (C) 2015-2016,2019-2020,2022-2024,2026 LSEG. All rights reserved.
 *|-----------------------------------------------------------------------------
 */
...
bool VectorDecoder::getNextData()
{
	...
	case RSSL_RET_SUCCESS :
		...
		return false;
	case RSSL_RET_INCOMPLETE_DATA :
		Decoder::setErrorData( &_load, OmmError::IncompleteDataEnum, &_decodeIter, &_rsslVectorEntry.encData );
		return true;
	case RSSL_RET_UNSUPPORTED_DATA_TYPE :
		Decoder::setErrorData( &_load, OmmError::UnsupportedDataTypeEnum, &_decodeIter, &_rsslVectorEntry.encData );
		return false;
	default :
		Decoder::setErrorData( &_load, OmmError::UnknownErrorEnum, &_decodeIter, &_rsslVectorEntry.encData );
		return false;
	}
}
