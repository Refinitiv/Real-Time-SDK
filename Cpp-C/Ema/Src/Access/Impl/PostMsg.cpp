 ...
 _postUserRightsString()
{
	...
	_pDecoder->cloneBufferToMsg(&const_cast<PostMsg&>(other), functionName);

	if (other.hasMsgKey()) // Override with the message key generated by the Watchlist
	{
		PostMsgDecoder* postDecoder = static_cast<PostMsgDecoder*>(_pDecoder);

		postDecoder->cloneMsgKey(other);
	}
