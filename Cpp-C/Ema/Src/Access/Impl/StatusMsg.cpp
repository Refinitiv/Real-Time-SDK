 ...
 _toString()
{
	...
	_pDecoder->cloneBufferToMsg(&const_cast<StatusMsg&>(other), functionName);

	if (other.hasMsgKey()) // Override with the message key generated by the Watchlist
	{
		StatusMsgDecoder* stDecoder = static_cast<StatusMsgDecoder*>(_pDecoder);

		stDecoder->cloneMsgKey(other);
	}
...
const EmaString& StatusMsg::toString( const refinitiv::ema::rdm::DataDictionary& dictionary ) const
{
	...
	StaticDecoder::setRsslData(&statusMsg, &rsslBuffer, RSSL_DT_MSG, RSSL_RWF_MAJOR_VERSION, RSSL_RWF_MINOR_VERSION, dictionary._pImpl->rsslDataDictionary());
