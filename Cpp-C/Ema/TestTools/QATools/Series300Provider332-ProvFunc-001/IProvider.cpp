...
int main( int argc, char* argv[] )
{
	for (int i = 1; i < argc; i++)
	{
		...
		if (strcmp("-delayDict", argv[i]) == 0)
		{
			delayDict = std::stoi(argv[++i]);
		}
	}
	try
	{
		...
		OmmProvider provider( OmmIProviderConfig().port(portNumber).adminControlDictionary( OmmIProviderConfig::UserControlEnum ), appClient );
