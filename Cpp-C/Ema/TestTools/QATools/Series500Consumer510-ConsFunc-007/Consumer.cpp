...
{ 
	try { 
		...
		serviceList.concreteServiceList().push_back(serviceName1);
		serviceList.concreteServiceList().push_back(serviceName2);

		OmmConsumer consumer( OmmConsumerConfig().consumerName("Consumer_10").addServiceList(serviceList), client);
		//API QA
		switch (_TEST) {
		case 0:
			cout << "***APIQA TEST 0 : ReqMsg does NOT set qos.***" << endl;
			consumer.registerClient(ReqMsg().serviceListName("SVG1").name("LSEG.L"), client);
			break;
		case 11:
			cout << "***APIQA TEST 11 : ReqMsg.qos(Timeliness.REALTIME, Rate.TICK_BY_TICK).***" << endl;
			consumer.registerClient(ReqMsg().serviceListName("SVG1").name("LSEG.L").qos(OmmQos::Timeliness::RealTimeEnum, OmmQos::Rate::TickByTickEnum), client);
			break;
		case 12:
			cout << "***APIQA TEST 12 : ReqMsg.qos(Timeliness.BEST_DELAYED_TIMELINESS, Rate.TICK_BY_TICK).***" << endl;
			consumer.registerClient(ReqMsg().serviceListName("SVG1").name("LSEG.L").qos(OmmQos::Timeliness::InexactDelayedEnum, OmmQos::Rate::TickByTickEnum), client);
			break;
		case 21:
			cout << "***APIQA TEST 21 : ReqMsg.qos(Timeliness.REALTIME, Rate.JIT_CONFLATED).***" << endl;
			consumer.registerClient(ReqMsg().serviceListName("SVG1").name("LSEG.L").qos(OmmQos::Timeliness::RealTimeEnum, OmmQos::Rate::JustInTimeConflatedEnum), client);
			break;		
		default:
			cout << "Invalid test";
		}
