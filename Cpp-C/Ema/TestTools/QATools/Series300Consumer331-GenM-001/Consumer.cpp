...
void AppClient::processDirectoryDomainGenericMsg(const GenericMsg& genericMsg, const OmmConsumerEvent& event)
{
	cout << endl << "#####Received Generic from provider. dir Handle: " << event.getHandle() << endl << "Closure:     " << event.getClosure() << endl;
	...
	while (el.forth())
	{
		...
		if (ee.getCode() == Data::BlankEnum)
			...
			case DataType::UIntEnum:
				...
				//APIQA 
				if (ee.getName() == ENAME_ACCEPTING_CONS_STATUS)
					_sendConsumerStatus = true;
				//END APIQA
				break;
...
//APIQA
void AppClient::sendConsumerStatus(const OmmConsumerEvent& ommEvent)
{
	_sendConsumerStatus = false;
	//case 1: sending genericmsg of name "ENAME_CONS_STATUS"
	_consumer->submit(GenericMsg().domainType(MMT_DIRECTORY).name(ENAME_CONS_STATUS).
		payload(Map().addKeyUInt(2, MapEntry::AddEnum, ElementList().
			addUInt(ENAME_CONS_SOURCE_MIROR_MODE, 1).complete()).complete()).complete(), _dirHandle);
}
//END APIQA
...
int main(int argc, char* argv[])
{
	try {
		...
		_dirHandle = consumer.registerClient(ReqMsg().domainType(MMT_DIRECTORY).serviceName("DIRECT_FEED").filter(29), client, closure);
