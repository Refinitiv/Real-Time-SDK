...
int main( int argc, char* argv[] )
{
	try {
		...
		UInt64 fldHandle = consumer.registerClient( ReqMsg().name("RWFFld").filter( DICTIONARY_NORMAL ).domainType( MMT_DICTIONARY ).serviceName( "DIRECT_FEED" ), client, closure );
		UInt64 enumHandle = consumer.registerClient( ReqMsg().name("RWFEnum").filter( DICTIONARY_NORMAL ).domainType(MMT_DICTIONARY).serviceName( "DIRECT_FEED" ), client, closure);
