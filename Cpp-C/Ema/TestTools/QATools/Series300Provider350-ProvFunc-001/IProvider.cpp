...
int main( int argc, char* argv[] )
{
	try
	{
		...
		//APIQA
		UInt64 fldHandle = provider.registerClient(ReqMsg().name("RWFFld").filter(filter).serviceName("DIRECT_FEED").domainType(MMT_DICTIONARY).interestAfterRefresh(false), appClient);

		UInt64 enumHandle = provider.registerClient(ReqMsg().name("RWFEnum").filter(filter).serviceName("DIRECT_FEED").domainType(MMT_DICTIONARY).interestAfterRefresh(false), appClient);
		//END APIQA
