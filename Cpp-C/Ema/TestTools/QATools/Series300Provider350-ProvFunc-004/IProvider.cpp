...
int main( int argc, char* argv[] )
{
	try
	{
		...
		//APIQA
		UInt64 fldHandle = provider.registerClient(ReqMsg().name("RWFFld").filter(filter).serviceName("INVALID").domainType(MMT_DICTIONARY).interestAfterRefresh(false), appClient);

		UInt64 enumHandle = provider.registerClient(ReqMsg().name("RWFEnum").filter(filter).serviceName("INVALID").domainType(MMT_DICTIONARY).interestAfterRefresh(false), appClient);
		UInt64 fldHandle1 = provider.registerClient(ReqMsg().name("RWFFld").filter(filter).serviceId(999).domainType(MMT_DICTIONARY).interestAfterRefresh(false), appClient);

		UInt64 enumHandle1 = provider.registerClient(ReqMsg().name("RWFEnum").filter(filter).serviceId(999).domainType(MMT_DICTIONARY).interestAfterRefresh(false), appClient);
		//END APIQA
