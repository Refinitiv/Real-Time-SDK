...
{ 
	try { 
		...
		/*OmmConsumer consumer(config.consumerName(consumerName), errorClient);
		consumer.registerClient( ReqMsg().serviceName( "ELEKTRON_DD" ).name( itemName ), client );
		sleep( 600000 ); 				// API calls onRefreshMsg(), onUpdateMsg(), or onStatusMsg()
		*/
	
		OmmConsumer consumer(config.consumerName(consumerName).operationModel(OmmConsumerConfig::UserDispatchEnum), errorClient);
		consumer.registerClient(ReqMsg().name( itemName ).serviceName("ELEKTRON_DD"), client);
