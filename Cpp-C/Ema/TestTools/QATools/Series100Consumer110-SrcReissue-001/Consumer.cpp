...
int main( int argc, char* argv[] )
{
	try {
		...
		UInt64 handle1 = consumer->registerClient(ReqMsg().serviceId(999).name("JPY="), client,  closure1);
		sleep(100);
		UInt64 dirHandle = consumer->registerClient( ReqMsg().domainType( MMT_DIRECTORY ).interestAfterRefresh(true).filter(29).serviceId(999), client, closure );  
		cout <<"REISSUE DIRECTORY"<<endl;
		consumer->reissue( ReqMsg().domainType(MMT_DIRECTORY).interestAfterRefresh(true).filter(29).serviceId(999),dirHandle);
