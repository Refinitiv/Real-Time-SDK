...
void AppClient::processDirectoryRequest(const ReqMsg& reqMsg, const OmmProviderEvent& event)
{
	event.getProvider().submit(RefreshMsg().domainType(MMT_DIRECTORY).filter(SERVICE_INFO_FILTER | SERVICE_STATE_FILTER)
		.payload(Map()
			.addKeyUInt(1, MapEntry::AddEnum, FilterList()
				...
				.add(SERVICE_STATE_ID, FilterEntry::SetEnum, ElementList()
					...
					.addUInt(ENAME_ACCEPTING_REQS, SERVICE_UP)
					.complete())
				.complete())
			.addKeyUInt(2, MapEntry::AddEnum, FilterList()
				.add(SERVICE_INFO_ID, FilterEntry::SetEnum, ElementList()
				.addAscii(ENAME_NAME, "DIRECT_FEED1")
				.addArray(ENAME_CAPABILITIES, OmmArray()
				.addUInt(MMT_MARKET_PRICE)
				.addUInt(MMT_MARKET_BY_PRICE)
				.complete())
				...
				.add(SERVICE_STATE_ID, FilterEntry::SetEnum, ElementList()
				.addUInt(ENAME_SVC_STATE, SERVICE_UP)
				.addUInt(ENAME_ACCEPTING_REQS, SERVICE_DOWN)
				.complete())
				.complete())
...
void AppClient::onClose( const ReqMsg& reqMsg, const OmmProviderEvent& event )
{
	...
	cout << reqMsg << endl;
        cout << "InitialImage " << reqMsg.getInitialImage() << endl;
        cout << "InterestAfterRefresh " << reqMsg.getInterestAfterRefresh() << endl << endl;
}
...
int main( int argc, char* argv[] )
{
	...
	if ( argc == 2 )
	{
		if ( strcmp("-sendCloseStatus", argv[1] ) == 0 )
		{
			sendCloseStatus = true;
		}
		else if ( strcmp("-notAcceptLoginReq", argv[1] ) == 0 )
		{
			acceptLoginReq = false;
		}
		else if ( strcmp("-sendLoginCloseStatus", argv[1] ) == 0 )
		{
		...
		for (Int32 i = 0; i < 60; i++)
		{
			...
			if ( sendCloseStatus )
			{
				provider.submit(StatusMsg()
					.state(OmmState::ClosedEnum, OmmState::SuspectEnum, OmmState::NoneEnum, "Close item from provider."),
					itemHandle);    
				break;
			}
			else if ( sendLoginCloseStatus  )
			{
				provider.submit(StatusMsg().domainType(MMT_LOGIN)
					.state(OmmState::ClosedEnum, OmmState::SuspectEnum, OmmState::NoneEnum, "Close login from provider."),
					loginHandle);
				break; 
			}
		}
