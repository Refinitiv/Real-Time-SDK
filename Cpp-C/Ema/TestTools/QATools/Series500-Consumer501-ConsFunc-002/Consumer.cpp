...
void createProgramaticConfig(Map& configDb)
{
	...
	if (preferredDetectionTimeInterval > 0)
		elList.addUInt("PreferredDetectionTimeInterval", preferredDetectionTimeInterval);
	if (!preferredDetectionTimeSchedule.empty())
		elList.addAscii("PreferredDetectionTimeSchedule", preferredDetectionTimeSchedule);
