...
void AppClient::decode(const Msg& msg, bool complete)
{
	switch (msg.getPayload().getDataType())
	{
		...
		case DataType::SeriesEnum:
		{
			...
			else if (msg.getName() == "RWFEnum_TEST")
			{
...
int main( int argc, char* argv[] )
{
	try {
		...
		//APIQA
		UInt64 fldHandle = consumer.registerClient( ReqMsg().name("RWFFld_TEST").filter( DICTIONARY_NORMAL ).domainType( MMT_DICTIONARY ).serviceName( "DIRECT_FEED" ), client, closure );

		UInt64 enumHandle = consumer.registerClient( ReqMsg().name("RWFEnum_TEST").filter( DICTIONARY_NORMAL ).domainType(MMT_DICTIONARY).serviceName( "DIRECT_FEED" ), client, closure);
		//END APIQA
