...
void AppClient::onRefreshMsg( const RefreshMsg& refreshMsg, const OmmConsumerEvent& ommEvent )
{
    ...
    //END APIQA
		_pOmmConsumer->submit( postMsg.postId( postId++ ).name( "IBM.N" ).solicitAck( true ).complete().payload( 
...
{		
	try {
        ...
        for (int i = 0; i < argc; i++)
        {
            ...
            else if (strcmp(argv[i], "-postServiceName") == 0)
            {
                serviceName = (i < (argc - 1) ? argv[++i] : NULL);
            }
            else if (strcmp(argv[i], "-postServiceId") == 0)
            {
                serviceId = (i < (argc - 1) ? atoi(argv[++i]) : -1);
            }
        }
        //END APIQA
