...
int main( int argc, char* argv[] )
{
	try {
		...
		cc.addAdminMsg( ReqMsg().domainType( MMT_DIRECTORY ).filter(29).interestAfterRefresh(true).serviceName("DIRECT_FEED"));        
		...
		UInt64 dirHandle = consumer->registerClient( ReqMsg().domainType( MMT_DIRECTORY ).interestAfterRefresh(true).filter(29).serviceName( "DIRECT_FEED" ), client, closure );
		UInt64 handle1 = consumer->registerClient(ReqMsg().serviceName("DIRECT_FEED").name("TRI.N"), client,  closure1);
