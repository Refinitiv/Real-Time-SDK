...
int main( int argc, char* argv[] )
{
	try
	{
		...
		{
			provider.submit( UpdateMsg().serviceName( "DIRECT_FEED" ).payload( FieldList().
			...
			if ( i == 6)
			{
				provider.submit( UpdateMsg().name( "RWFFld" ).serviceName( "DIRECT_FEED" ).domainType( MMT_DICTIONARY )
				 , 0 );
			}
		}
	}
