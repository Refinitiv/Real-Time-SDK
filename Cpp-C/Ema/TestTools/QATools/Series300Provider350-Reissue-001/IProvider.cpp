	...
	try
	{
		...
		for (Int32 i = 0; i < 10; i++)
		{
			...
			if ( i == 6 )
			{
				...
				provider.reissue( ReqMsg().name("RWFFld").filter(filter).serviceName("DIRECT_FEED").domainType(MMT_DICTIONARY).interestAfterRefresh(true), fldHandle);
				provider.reissue( ReqMsg().name("RWFEnum").filter(filter).serviceName("DIRECT_FEED").domainType(MMT_DICTIONARY).interestAfterRefresh(true), enumHandle);
			}
			if ( i == 8 )
			{
				cout <<"Reissue Dictionary handles with filter unchange " << endl;
