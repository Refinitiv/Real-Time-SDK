...
{ 
	try { 
		...
		case 0:
			...
			consumer.registerClient(ReqMsg().serviceListName("SVG1").payload(batchView), client);
			break;
		case 11:
			cout << "***APIQA TEST 11 : ReqMsg.qos(Timeliness.REALTIME, Rate.TICK_BY_TICK).***" << endl;
			consumer.registerClient(ReqMsg().serviceListName("SVG1").payload(batchView).qos(OmmQos::Timeliness::RealTimeEnum, OmmQos::Rate::TickByTickEnum), client);
			break;
		case 12:
			cout << "***APIQA TEST 12 : ReqMsg.qos(Timeliness.BEST_DELAYED_TIMELINESS, Rate.TICK_BY_TICK).***" << endl;
			consumer.registerClient(ReqMsg().serviceListName("SVG1").payload(batchView).qos(OmmQos::Timeliness::InexactDelayedEnum, OmmQos::Rate::TickByTickEnum), client);
			break;
		case 21:
			cout << "***APIQA TEST 21 : ReqMsg.qos(Timeliness.REALTIME, Rate.JIT_CONFLATED).***" << endl;
			consumer.registerClient(ReqMsg().serviceListName("SVG1").payload(batchView).qos(OmmQos::Timeliness::RealTimeEnum, OmmQos::Rate::JustInTimeConflatedEnum), client);
			break;		
		default:
			cout << "Invalid test";
		}
