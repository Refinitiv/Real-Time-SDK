...
int main( int argc, char* argv[] )
{
	try {
		...
		for (int i = 0; i < 60; i++)
		{
			...
			if ( i == 10 )
			{
				cout << endl << "SEND RESUME USING first TOKEN" << endl;
					loginMsg.clear().name(authenticationToken).applicationId(appId).nameType(USER_AUTH_TOKEN).pause(false);
				consumer.reissue(loginMsg.getMessage(), loginClient._handle);
			}
			if ( i == 53 )
			{
				cout << endl << "Renew new TOKEN" << endl;
					loginMsg.clear().name(newauthenticationToken).applicationId(appId).nameType(USER_AUTH_TOKEN);
				consumer.reissue(loginMsg.getMessage(), loginClient._handle);
			}
		}
