...
void createProgramaticConfig( Map& configDb )
{
	...
	elementMap.addKeyAscii("Server_Info_1", MapEntry::AddEnum, ElementList()
		...
		.addAscii("PerServiceNameSet", serviceName).complete());
...
void printHelp()
{
	cout << endl << "Options:\n" << " -?\tShows this usage" << endl
		...
		<< " -s Request service name (optional), defaulted to DIRECT_FEED." << endl
		<< " -sid Request service id (optional, defaulted to service name if sid is unspecified)." << endl
		<< " -noservice Request Directory without service (optional, defaulted to false)." << endl
...
{ 
	try {
		...
		for (int i = 1; i < argc; i++)
		{
			...
			{
				if (i < (argc - 1)) serviceName.set(argv[++i]);
			}
			else if (strcmp(argv[i], "-sid") == 0)
			{
				if (i < (argc - 1)) serviceId = atoi(argv[++i]);
			}
			else if (strcmp(argv[i], "-noservice") == 0)
			{
				noservice = true;
			}
		...
		if (noservice)
		{
			cout << "QA prints : Request Directory without service." << endl;
			cout << "Filter = " << dirfilter << "." << endl;
			DirectoryHdl = consumer.registerClient(refinitiv::ema::access::ReqMsg()
				...
				client);
			//Request item
			consumer.registerClient(ReqMsg().name(itemName).serviceName(serviceName), client);
		}
		else 
		{
			if (serviceId > 0)
			{
				...
				if (dirfilter != -1)
				{
					cout << "serviceId = " << serviceId << ". Filter = " << dirfilter << "." << endl;
					DirectoryHdl = consumer.registerClient(refinitiv::ema::access::ReqMsg()
						.domainType(refinitiv::ema::rdm::MMT_DIRECTORY)
						.initialImage(true)
						.serviceId(serviceId)
				...
				{
					cout << "serviceId = " << serviceId << ". No Filter." << endl;
					DirectoryHdl = consumer.registerClient(refinitiv::ema::access::ReqMsg()
						.domainType(refinitiv::ema::rdm::MMT_DIRECTORY)
						.initialImage(true)
						.serviceId(serviceId),
				...
				//Request item
				consumer.registerClient(ReqMsg().name(itemName).serviceId(serviceId), client);
			}
			else
			{
				...
				if (dirfilter != -1)
				{
					cout << "serviceName = " << serviceName << ". Filter = " << dirfilter << "." << endl;
					DirectoryHdl = consumer.registerClient(refinitiv::ema::access::ReqMsg()
						.domainType(refinitiv::ema::rdm::MMT_DIRECTORY)
						.initialImage(true)
				...
				{
					cout << "serviceName = " << serviceName << ". No Filter." << endl;
					DirectoryHdl = consumer.registerClient(refinitiv::ema::access::ReqMsg()
						.domainType(refinitiv::ema::rdm::MMT_DIRECTORY)
						.initialImage(true)
						.serviceName(serviceName),
