...
{ 
	try { 
		...
		AppClient client;
		//API QA
		OmmConsumer consumer( OmmConsumerConfig().consumerName("Consumer_10").username(userName), client);

		switch (_TEST) {
		case 0:
			cout << "***APIQA TEST 0 : App requests with service " << serviceName << " & ReqMsg does NOT set qos.***" << endl;
			consumer.registerClient(ReqMsg().serviceName(serviceName).name(itemName), client);
			break;
		case 11:
			cout << "***APIQA TEST 11 : App requests with service " << serviceName << " & ReqMsg.qos(Timeliness.REALTIME, Rate.TICK_BY_TICK).***" << endl;
			consumer.registerClient(ReqMsg().serviceName(serviceName).name(itemName).qos(OmmQos::Timeliness::RealTimeEnum, OmmQos::Rate::TickByTickEnum), client);
			break;
		case 12:
			cout << "***APIQA TEST 12 : App requests with service " << serviceName << " & ReqMsg.qos(Timeliness.BEST_DELAYED_TIMELINESS, Rate.TICK_BY_TICK).***" << endl;
			consumer.registerClient(ReqMsg().serviceName(serviceName).name(itemName).qos(OmmQos::Timeliness::InexactDelayedEnum, OmmQos::Rate::TickByTickEnum), client);
			break;
		case 21:
			cout << "***APIQA TEST 21 : App requests with service " << serviceName << " & ReqMsg.qos(Timeliness.REALTIME, Rate.JIT_CONFLATED).***" << endl;
			consumer.registerClient(ReqMsg().serviceName(serviceName).name(itemName).qos(OmmQos::Timeliness::RealTimeEnum, OmmQos::Rate::JustInTimeConflatedEnum), client);
			break;		
		default:
			cout << "Invalid test";
		}
