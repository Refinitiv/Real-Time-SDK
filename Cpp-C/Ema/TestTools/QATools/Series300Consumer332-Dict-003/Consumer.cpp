...
void AppClient::decode(const Msg& msg, bool complete)
{
	switch (msg.getPayload().getDataType())
	{
		...
		case DataType::SeriesEnum:
		{
			...
			{
				dataDictionary.decodeFieldDictionary(msg.getPayload().getSeries(), DICTIONARY_MINIMAL);
			...
			{
				dataDictionary.decodeEnumTypeDictionary(msg.getPayload().getSeries(), DICTIONARY_MINIMAL);
...
int main( int argc, char* argv[] )
{
	try {
		...
		//APIQA
		UInt64 fldHandle = consumer.registerClient( ReqMsg().name("RWFFld").filter( DICTIONARY_MINIMAL ).domainType( MMT_DICTIONARY ).serviceName( "DIRECT_FEED" ), client, closure );

		UInt64 enumHandle = consumer.registerClient( ReqMsg().name("RWFEnum").filter( DICTIONARY_MINIMAL ).domainType(MMT_DICTIONARY).serviceName( "DIRECT_FEED" ), client, closure);
		//END APIQA
