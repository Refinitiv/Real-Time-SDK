...
int main( int argc, char* argv[] )
{
	try {
		...
		cc.addAdminMsg( ReqMsg().domainType( MMT_DICTIONARY ).interestAfterRefresh(true).name("RWFFld").filter(7));
		cc.addAdminMsg( ReqMsg().domainType( MMT_DICTIONARY ).interestAfterRefresh(true).name("RWFEnum").filter(7));      
