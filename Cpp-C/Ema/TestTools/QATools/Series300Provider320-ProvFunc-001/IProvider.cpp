...
int main( int argc, char* argv[] )
{
	try
	{
		...
		OmmProvider provider( OmmIProviderConfig().adminControlDictionary( OmmIProviderConfig::ApiControlEnum ), appClient );
		...
		for (Int32 i = 0; i < 60; i++)
		{
			...
			if ( i == 2)
				{
        		provider.submit( StatusMsg().name( "RWFFld" ).serviceId(1).domainType( MMT_DICTIONARY ).
	        		state( OmmState::OpenEnum, OmmState::SuspectEnum, OmmState::NoneEnum, "Open Suspect Status on dictionary" ), 0 );
				}
			if ( i == 6)
				{
        		provider.submit( StatusMsg().name( "RWFFld" ).serviceName( "DIRECT_FEED" ).domainType( MMT_DICTIONARY ).
	        		state( OmmState::ClosedEnum, OmmState::SuspectEnum, OmmState::NoneEnum, "Closed Status on dictionary" ), 0 );
				}
		}
	}
