...
int main( int argc, char* argv[] )
{
	try {
		...
		int dfilter = 1;
                ...
                else if (strcmp(argv[1], "-directoryFilter") == 0)
                    {
		        dfilter = atoi(argv[2]);
		        cout << "FILTER SPECIFIED IS " << dfilter << endl;
                    }
                UInt64 dirHandle = consumer.registerClient( ReqMsg().domainType( MMT_DIRECTORY ).serviceName( "DIRECT_FEED" ).filter(dfilter), client, closure );
