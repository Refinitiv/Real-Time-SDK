...
void AppClient::onRefreshMsg( const RefreshMsg& refreshMsg, const OmmConsumerEvent& )
{
	...
	{
		_pOmmConsumer->submit(PostMsg().postId(1).serviceId( 955 ).name("IBM.N").solicitAck(true).complete().payload(
			RefreshMsg().payload(FieldList().addReal(25, 35, OmmReal::ExponentPos1Enum).complete()).complete()), _loginHandle);
	}
}
...
void AppLoginClient::onRefreshMsg(const RefreshMsg& refreshMsg, const OmmConsumerEvent& event)
{
	...
	_loginHandle = _handle = event.getHandle();
...
int main( int argc, char* argv[] )
{
	try {
		...
		consumer.registerClient( ReqMsg().serviceName( "NI_PUB" ).name( "TRI.N" ), appClient );
