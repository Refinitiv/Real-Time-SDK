...
//APIQA
bool ackWithServiceId = false;
bool ackWithServiceName = false;
bool ackWithInvalidSID = false;
//END APIQA
...
void AppClient::onPostMsg( const PostMsg& postMsg, const OmmProviderEvent& event )
{
	...
	if (postMsg.getSolicitAck())
	{
		...
		//API QA
		if (ackWithServiceName)
		{
			cout << "QA Testing ackMsg.serviceName(postMsg.getServiceName()). \nServiceName: " << postMsg.getServiceName()  << endl;
			ackMsg.serviceName(postMsg.getServiceName());
		}
		if (ackWithServiceId)
		{
			cout << "QA Testing ackMsg.serviceId(postMsg.getServiceId)). \nServiceId: " << postMsg.getServiceId() << endl;
			ackMsg.serviceId(postMsg.getServiceId());
		}
		if (ackWithInvalidSID)
		{
			cout << "QA Testing ackMsg.serviceId(555)). \nServiceId: 555 is invalid!"  << endl;
			ackMsg.serviceId(555);
		}
		//END API QA
...
int main(int argc, char* argv[])
{
	try
	{
		...
		for (int i = 0; i < argc; i++)
		{
			...
			else if (strcmp(argv[i], "-ackWithServiceId") == 0)
			{
				ackWithServiceId = true;
			}
			else if (strcmp(argv[i], "-ackWithServiceName") == 0)
			{
				ackWithServiceName = true;
			}
			else if (strcmp(argv[i], "-ackWithInvalidSID") == 0)
			{
				ackWithInvalidSID = true;
			}
		}
		//END APIQA
