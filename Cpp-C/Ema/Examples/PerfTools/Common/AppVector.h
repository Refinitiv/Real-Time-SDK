...
public :
	...
	//@{
		...
		*/
	AppVector( rtsdk::ema::access::UInt64 capacity = 0 );
	//@}
	...
	//@{
		...
		*/
		rtsdk::ema::access::UInt64 size() const;

		/** index operator
			\remark allows read only access to the i-th element of the list
		*/
		const T& operator[]( rtsdk::ema::access::UInt64 index ) const;

		/** index operator
			\remark allows read & write access to the i-th element of the list
		*/
		T& operator[]( rtsdk::ema::access::UInt64 index );
		...
		*/
		rtsdk::ema::access::Int64 getPositionOf( const T& value ) const;
	...
	//@{
		...
		*/
		bool removePosition( rtsdk::ema::access::UInt64 pos );
...
private :
	...
	rtsdk::ema::access::UInt64		m_capacity;

	rtsdk::ema::access::UInt64		m_size;
...
// default constructor
template< class T >
AppVector< T >::AppVector( rtsdk::ema::access::UInt64 capacity ) :
	m_size( 0 ),
	m_capacity( capacity ),
	m_list( 0 )
{
	...
	m_list( 0 )
{
	...
	for ( rtsdk::ema::access::UInt64 pos = 0; pos < m_size; ++pos )
	{
...
template <class T >
rtsdk::ema::access::UInt64 AppVector< T >::size() const
{
	return m_size;
}

template < class T >
T& AppVector< T >::operator[]( rtsdk::ema::access::UInt64 position ) 
{
...
template < class T >
const T& AppVector< T >::operator[]( rtsdk::ema::access::UInt64 position ) const
{
	assert ( position < m_size);
	return m_list[position];
}
...
template < class T >
rtsdk::ema::access::Int64 AppVector< T >::getPositionOf( const T& value ) const
{
	rtsdk::ema::access::Int64 position = -1;

	// check which element matches the passed one
	for ( rtsdk::ema::access::UInt64 idx = 0; idx < m_size; ++idx )
	{
		if ( operator[]( idx ) == value )
		{
			position = (rtsdk::ema::access::Int64)(idx);
			break;
		}
	}
...
template < class T >
bool AppVector< T >::removePosition( rtsdk::ema::access::UInt64 position )
{
	// return if index out of range
	if ( position >= m_size ) return false;

	// move down elements above passed idx by one
	for ( rtsdk::ema::access::UInt64 i = position + 1; i < m_size; ++i )
	{
		m_list[ i - 1 ] = m_list[ i ];
	}
