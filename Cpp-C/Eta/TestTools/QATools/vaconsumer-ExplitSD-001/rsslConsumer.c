...
RsslReactorCallbackRet authTokenEventCallback(RsslReactor *pReactor, RsslReactorChannel *pReactorChannel, RsslReactorAuthTokenEvent *pAuthTokenEvent)
{
	...
	{
		printf("\n%s New auth token received %s\n", timeBuf, pAuthTokenEvent->pReactorAuthTokenInfo->accessToken.data);
	...
	for (i = 0; i < channelCommandCount; ++i)
	{
		...
		if (i == 0)
		{
			...
			if (userName.length)
			{
				...
				pOAuthCredential->userName = userName;
			}
		...
		else
		{
			...
			if (userName2.length)
			{
				...
				pOAuthCredential->userName = userName2;
			}
		...
		pCommand->pLoginRequest = pLoginRequest;  // API QA
