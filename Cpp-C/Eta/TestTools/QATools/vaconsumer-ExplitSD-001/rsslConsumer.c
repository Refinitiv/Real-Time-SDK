...
int blockingExplicitSD = -1; /* Undefiend */
//END APIQA
...
void printUsageAndExit(char *appName)
{
	...
	printf("Usage: %s or\n%s  [-tcp|-encrypted|-encryptedSocket|-encryptedWebSocket|-encryptedHttp [<hostname>:<port> <service name>]] [<domain>:<item name>,...] ] [-uname <LoginUsername>] [-passwd <LoginPassword>] [ -clientId <Client ID> ] [-sessionMgnt] [-view] [-post] [-offpost] [-snapshot] [-runtime <seconds>] [-cache] [-cacheInterval <seconds>] [-statisticInterval <seconds>] [-tunnel] [-tsDomain <number> ] [-tsAuth] [-tsServiceName] [-x] [-runtime]\n"
			...
			"\n -uname specifies the username used when logging into the provider. The machine ID for Real-Time - Optimized (mandatory).\n"
			"\n -passwd specifies the password used when logging into the provider. The password for Real-Time - Optimized (mandatory).\n"
			/*API QA*/
			...
			/*END API QA*/
			"\n -clientId specifies the Client ID for Real-Time - Optimized (mandatory). You can generate and manage client Ids using an AppKeyGenerator\n"
...
//APIQA
			...
			else if (strcmp("-blockingExplicitSD", argv[i]) == 0)
			{
				i += 2; if (i > argc) printUsageAndExit(argv[0]);
				blockingExplicitSD = atoi(argv[i - 1]);
			}
			//END APIQA
...
RsslReactorCallbackRet authTokenEventCallback(RsslReactor *pReactor, RsslReactorChannel *pReactorChannel, RsslReactorAuthTokenEvent *pAuthTokenEvent)
{
	...
	else if (pCommand && pCommand->canSendLoginReissue && pAuthTokenEvent->pReactorAuthTokenInfo)
	{
		...
		pCommand->pLoginRequest->userName = pAuthTokenEvent->pReactorAuthTokenInfo->accessToken;
		pCommand->pLoginRequest->flags |= (RDM_LG_RQF_HAS_USERNAME_TYPE | RDM_LG_RQF_NO_REFRESH);
		pCommand->pLoginRequest->userNameType = RDM_LOGIN_USER_AUTHN_TOKEN;

		submitMsgOpts.pRDMMsg = (RsslRDMMsg*)pCommand->pLoginRequest;
	...
	else if (pCommand && pAuthTokenEvent->pReactorAuthTokenInfo)
	{
		printf("\nNew auth token received %s\n", pAuthTokenEvent->pReactorAuthTokenInfo->accessToken.data);

		if (pAuthTokenEvent->pReactorAuthTokenInfo->accessToken.length > ACCESSTOKEN_BUFSIZE - 1)
		{
			printf("The access token is too long for the buffer size!!!");
			exit(-1);
		}

		memset(pCommand->accessTokenBuf.data, 0, ACCESSTOKEN_BUFSIZE);

		pCommand->accessTokenBuf.length = pAuthTokenEvent->pReactorAuthTokenInfo->accessToken.length;

		memcpy(pCommand->accessTokenBuf.data, pAuthTokenEvent->pReactorAuthTokenInfo->accessToken.data, pCommand->accessTokenBuf.length);
	}
	/* END */
	...
	switch(pConnEvent->channelEventType)
	{
		case RSSL_RC_CET_CHANNEL_UP:
		{
			...
			/* API QA */
			{
				...
				pCommand->pLoginRequest->userName = pCommand->accessTokenBuf;
				pCommand->pLoginRequest->flags |= (RDM_LG_RQF_HAS_USERNAME_TYPE);
				pCommand->pLoginRequest->userNameType = RDM_LOGIN_USER_AUTHN_TOKEN;
				...
				if ((ret = rsslReactorSubmitMsg(pReactor, pReactorChannel, &submitMsgOpts, &rsslErrorInfo)) != RSSL_RET_SUCCESS)
				{
					printf("Login request failed:  %d(%s)\n", ret, rsslErrorInfo.rsslError.text);
				}
				else
				{
...
int main(int argc, char **argv)
{
	...
	/* API QA */
	RsslRDMLoginRequest loginRequest[2];
	/* END */
	...
	if (authnToken.length)
	{
		...
		for (i = 0; i < 2; i++)
		{
			loginRequest[i].flags |= RDM_LG_RQF_HAS_USERNAME_TYPE;
			loginRequest[i].userName = authnToken;
			loginRequest[i].userNameType = RDM_LOGIN_USER_AUTHN_TOKEN;

			if (authnExtended.length)
			{
				loginRequest[i].flags |= RDM_LG_RQF_HAS_AUTHN_EXTENDED;
				loginRequest[i].authenticationExtended = authnExtended;
			}
		}
	}
	...
	if (appId.length)
	{
		loginRequest[i].flags |= RDM_LG_RQF_HAS_APPLICATION_ID;
		loginRequest[i].applicationId = appId;
	}

	if (RTTSupport == RSSL_TRUE)
	{
		loginRequest[i].flags |= RDM_LG_RQF_RTT_SUPPORT;
	}
	...
	for(i = 0; i < 2; ++i) /* API QA support only two ReactorChannel */
	{
		ChannelCommand *pCommand = &chanCommands[i];
		...
		//API QA
			/* Initialize the default login request(Use 1 as the Login Stream ID). */
		if (rsslInitDefaultRDMLoginRequest(&loginRequest[i], 1) != RSSL_RET_SUCCESS)
		{
			printf("rsslInitDefaultRDMLoginRequest() failed\n");
			cleanUpAndExit(-1);
		}
		if (i == 0)
		{
			if (userName.length)
			{
				loginRequest[i].userName = userName;
			...
			/* API QA */
			if(blockingExplicitSD != -1)
			{
				RsslReactorServiceDiscoveryOptions	serviceDiscoveryOpts;
				rsslClearReactorServiceDiscoveryOptions(&serviceDiscoveryOpts);

				serviceDiscoveryOpts.restBlocking = blockingExplicitSD > 0 ? RSSL_TRUE : RSSL_FALSE;

				serviceDiscoveryOpts.userName = userName;

				serviceDiscoveryOpts.password = password;
			...
			pCommand->pLoginRequest = &loginRequest[i];
		...
		else
		{
			if (userName2.length)
			{
				loginRequest[i].userName = userName2;
				oAuthCredential.userName = userName2;
			}
			...
			if (blockingExplicitSD != -1)
			{
				...
				serviceDiscoveryOpts.userName = userName2;

				serviceDiscoveryOpts.password = password2;

				serviceDiscoveryOpts.clientId = userName2;
...
RsslReactorCallbackRet serviceEndpointEventCallback(RsslReactor* pReactor, RsslReactorServiceEndpointEvent* pEndPointEvent)
{
	...
	{
		printf("\nError requesting Service Discovery Endpoint Information: %s\n", pEndPointEvent->pErrorInfo->rsslError.text);
	}
	else
	{
		printf("\nNumber of service discovery endpoint = %d\n", pEndPointEvent->serviceEndpointInfoCount);
	}
