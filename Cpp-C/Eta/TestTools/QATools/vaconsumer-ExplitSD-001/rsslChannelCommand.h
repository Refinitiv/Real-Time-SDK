...
/* API QA */
#define ACCESSTOKEN_BUFSIZE 8192
/* END API QA */
...
// END API QA
	/* API QA */
	char accessTokenMem[ACCESSTOKEN_BUFSIZE];
	RsslBuffer accessTokenBuf;
...
RTR_C_INLINE void initChannelCommand(ChannelCommand *pCommand)
{
	...
	// API QA
	rsslClearBuffer(&pCommand->userName);
	rsslClearBuffer(&pCommand->password);
	// END API QA
	/* API QA */
	rsslClearBuffer(&pCommand->accessTokenBuf);
	pCommand->accessTokenBuf.data = pCommand->accessTokenMem;
	pCommand->accessTokenBuf.length = ACCESSTOKEN_BUFSIZE;
	/* END */
}
