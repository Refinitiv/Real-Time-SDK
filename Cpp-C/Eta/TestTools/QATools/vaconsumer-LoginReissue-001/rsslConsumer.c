...
static RsslReactorCallbackRet defaultMsgCallback(RsslReactor *pReactor, RsslReactorChannel *pChannel, RsslMsgEvent* pMsgEvent)
{
	...
	switch ( pMsg->msgBase.domainType )
	{
		case RSSL_DMT_MARKET_PRICE:
			...
			// APIQA: Send a PAUSE ALL after getting some updates
			eventCounter++;
			if (eventCounter == 3)
			{
				...
				{
					printf("APIQA: rsslReactorSubmitMsg failed when attempting to send PAUSE ALL:  %d(%s)\n", ret, rsslErrorInfo.rsslError.text);
				}
				printf("APIQA: sending PAUSE ALL\n");
			}
			// END APIQA: Send a PAUSE ALL after getting some updates
			break;
