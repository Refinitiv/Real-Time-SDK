...
// API QA
static RsslBool xmlTracePing = RSSL_FALSE;
static RsslBool xmlTrace1 = RSSL_FALSE;
static RsslBool xmlTraceNoWrite = RSSL_FALSE;
static RsslBool xmlTraceNoRead = RSSL_FALSE;
// END API QA
...
// API QA
	printf(" -xping provides an XML trace of PING-messages.\n");
	printf(" -x1 provides an alternative XML trace of PING-messages.\n");
	printf(" -xnowrite uses together with -x or -x1: exclude trace write operations.\n");
	printf(" -xnoread uses together with -x or -x1: exclude trace read operations.\n");
// END API QA
#ifdef _WIN32
...
int main(int argc, char **argv)
{
	...
	for(iargs = 1; iargs < argc; ++iargs)
	{
		...
		// API QA
		else if (0 == strcmp("-xping", argv[iargs]))
		{
			xmlTracePing = RSSL_TRUE;
			snprintf(traceOutputFile, 128, "RsslProvider\0");
		}
		else if (0 == strcmp("-x1", argv[iargs]))
		{
			xmlTrace1 = RSSL_TRUE;
			snprintf(traceOutputFile, 128, "RsslProvider\0");
		}
		else if (0 == strcmp("-xnowrite", argv[iargs]))
		{
			xmlTraceNoWrite = RSSL_TRUE;
		}
		else if (0 == strcmp("-xnoread", argv[iargs]))
		{
			xmlTraceNoRead = RSSL_TRUE;
		}
		// END API QA
...
static void createNewClientSession(RsslServer *srvr)
{
	...
	else 
	{
		if (xmlTrace)
		{
			...
			printf("Set trace flags: %4.4Xh\n", (int)(traceOptions.traceFlags & 0xFFFF));
			// END API QA
			rsslIoctl(sckt, (RsslIoctlCodes)RSSL_TRACE, (void *)&traceOptions, &error);
		}
// API QA
 		else if (xmlTracePing)
 		{
			RsslTraceOptions traceOptions;
			rsslClearTraceOptions(&traceOptions);
			traceOptions.traceMsgFileName = traceOutputFile;
			traceOptions.traceMsgMaxFileSize = 10000000;
			traceOptions.traceFlags |= RSSL_TRACE_TO_FILE_ENABLE | RSSL_TRACE_TO_MULTIPLE_FILES | RSSL_TRACE_TO_STDOUT | RSSL_TRACE_PING_ONLY;
			rsslIoctl(sckt, (RsslIoctlCodes)RSSL_TRACE, (void*)&traceOptions, &error);
			printf("Set trace flags: %4.4Xh\n", (int)(traceOptions.traceFlags & 0xFFFF));
		}
 		else if (xmlTrace1)
 		{
			...
			traceOptions.traceMsgFileName = traceOutputFile;
			traceOptions.traceMsgMaxFileSize = 10000000;
			traceOptions.traceFlags |= RSSL_TRACE_TO_FILE_ENABLE | RSSL_TRACE_TO_MULTIPLE_FILES | RSSL_TRACE_TO_STDOUT | RSSL_TRACE_WRITE | RSSL_TRACE_READ | RSSL_TRACE_DUMP | RSSL_TRACE_PING_ONLY;
			if (xmlTraceNoWrite)
				traceOptions.traceFlags &= ~(RSSL_TRACE_WRITE);
			if (xmlTraceNoRead)
				traceOptions.traceFlags &= ~(RSSL_TRACE_READ);
...
static void handlePings()
{
	...
	for (i = 0; i < NUM_CLIENT_SESSIONS; i++)
	{
		if ((clientSessions[i].clientChannel != NULL) && (clientSessions[i].clientChannel->socketId != -1))
		{
			...
			{
				// API QA
				// Ping doesn't send when an internal queue has some messages.
				// rsslFlush sends all the messages from an internal queue.
				if ((retval = rsslFlush(clientSessions[i].clientChannel, &error)) < RSSL_RET_SUCCESS)
				{
					printf("rsslFlush() failed with return code %d - <%s>\n", retval, error.text);
					removeClientSessionForChannel(clientSessions[i].clientChannel);
