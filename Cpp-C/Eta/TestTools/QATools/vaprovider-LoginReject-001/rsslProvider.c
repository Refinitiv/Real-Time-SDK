...
RsslInt32 nItemsToSend = -1; // API QA
...
void exitWithUsage()
{
	...
	// API QA
	printf(" -ni number of items to send before sending login closed suspect and cutting the channel\n");
	// END API QA
...
int main(int argc, char **argv)
{
	...
	for (iargs = 1; iargs < argc; ++iargs)
	{
		...
		// API QA
		else if (strcmp("-ni", argv[iargs]) == 0)
		{
			++iargs; if (iargs == argc) exitWithUsage();
			nItemsToSend = atoi(argv[iargs]);
		}
		// END API QA
		...
		if (time(NULL) >= nextSendTime)
		{
			...
			for (i = 0; i < MAX_CLIENT_SESSIONS; i++)
			{
				if (clientSessions[i].clientChannel != NULL)
				{
					...
					{
// API QA
						sendLoginClosedSuspect(pReactor, clientSessions[i].clientChannel);
#if defined WIN32
						Sleep(200);
#else
						struct timespec sleeptime;
						sleeptime.tv_sec = 200 / 1000;
						sleeptime.tv_nsec = (200 % 1000) * 1000000;
