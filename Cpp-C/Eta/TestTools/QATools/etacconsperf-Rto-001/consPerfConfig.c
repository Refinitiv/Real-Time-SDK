 ...
 * Copyright (C) 2020-2021 LSEG. All rights reserved.
*/
...
// APIQA:
	snprintf(consPerfConfig.clientId, sizeof(consPerfConfig.clientId), "");
	snprintf(consPerfConfig.clientSecret, sizeof(consPerfConfig.clientSecret), "");
	snprintf(consPerfConfig.password, sizeof(consPerfConfig.password), "");
	consPerfConfig.enableSessionMgnt = RSSL_FALSE;
	consPerfConfig.hostNameProvided = RSSL_FALSE;
	snprintf(consPerfConfig.location, sizeof(consPerfConfig.location), "us-east-1");

	snprintf(consPerfConfig.proxyHost, sizeof(consPerfConfig.proxyHost), "");
	snprintf(consPerfConfig.proxyPort, sizeof(consPerfConfig.proxyPort), "");
	snprintf(consPerfConfig.proxyLogin, sizeof(consPerfConfig.proxyLogin), "");
	snprintf(consPerfConfig.proxyPassword, sizeof(consPerfConfig.proxyPassword), "");
	snprintf(consPerfConfig.proxyDomain, sizeof(consPerfConfig.proxyDomain), "");
// END APIQA

	consPerfConfig.ticksPerSec = 1000;
	// APIQA:
	consPerfConfig.itemRequestCount = 1;
	// END APIQA
...
// APIQA:
		...
		{
			++iargs; if (iargs == argc) exitMissingArgument(argv, iargs - 1);
			sscanf(argv[iargs++], "%s", consPerfConfig.clientId);
		}
		else if (strcmp("-password", argv[iargs]) == 0)
		{
			++iargs; if (iargs == argc) exitMissingArgument(argv, iargs - 1);
			sscanf(argv[iargs++], "%s", consPerfConfig.password);
		}
		else if (strcmp("-clientSecret", argv[iargs]) == 0)
		{
			++iargs; if (iargs == argc) exitMissingArgument(argv, iargs - 1);
			sscanf(argv[iargs++], "%s", consPerfConfig.clientSecret);
		}
		else if (strcmp("-sessionMgnt", argv[iargs]) == 0)
		{
			++iargs;
			consPerfConfig.enableSessionMgnt = RSSL_TRUE;
		}
		else if (strcmp("-l", argv[iargs]) == 0)
		{
			++iargs; if (iargs == argc) exitMissingArgument(argv, iargs - 1);
			sscanf(argv[iargs++], "%s", consPerfConfig.location);
		}
		else if (strcmp("-ph", argv[iargs]) == 0)
		{
			++iargs; if (iargs == argc) exitMissingArgument(argv, iargs - 1);
			sscanf(argv[iargs++], "%s", consPerfConfig.proxyHost);
		}
		else if (strcmp("-pp", argv[iargs]) == 0)
		{
			++iargs; if (iargs == argc) exitMissingArgument(argv, iargs - 1);
			sscanf(argv[iargs++], "%s", consPerfConfig.proxyPort);
		}
		else if (strcmp("-plogin", argv[iargs]) == 0)
		{
			++iargs; if (iargs == argc) exitMissingArgument(argv, iargs - 1);
			sscanf(argv[iargs++], "%s", consPerfConfig.proxyLogin);
		}
		else if (strcmp("-ppasswd", argv[iargs]) == 0)
		{
			++iargs; if (iargs == argc) exitMissingArgument(argv, iargs - 1);
			sscanf(argv[iargs++], "%s", consPerfConfig.proxyPassword);
		}
		else if (strcmp("-pdomain", argv[iargs]) == 0)
		{
			++iargs; if (iargs == argc) exitMissingArgument(argv, iargs - 1);
			sscanf(argv[iargs++], "%s", consPerfConfig.proxyDomain);
		}
// END APIQA
		else if(strcmp("-h", argv[iargs]) == 0)
		{
			...
			consPerfConfig.hostNameProvided = RSSL_TRUE;
		}
...
// APIQA:
		strlen(consPerfConfig.clientId) ? "<removed from displaying>" : "n/a",
		strlen(consPerfConfig.password) ? "<removed from displaying>" : "n/a",
		strlen(consPerfConfig.clientSecret) ? "<removed from displaying>" : "n/a",
// END APIQA
...
// APIQA:
		consPerfConfig.proxyHost,
		consPerfConfig.proxyPort
// END APIQA
	  );
...
// APIQA:
			...
			"  -clientId <name>                      Client Id to use in login request\n"
			"  -password <name>                      Password to use in login request\n"
			"  -clientSecret <name>                  Client Secret to use in login request\n"
// END APIQA
			...
			"\n"
// APIQA:
			"  -ph <proxy hostname> Specify proxy hostname.\n"
			"  -pp <proxy port> Specify proxy port.\n"
			"  -plogin <proxy login> Specify proxy login.\n"
			"  -ppasswd <proxy password> Specify proxy password.\n"
			"  -pdomain <proxy domain> Specify proxy domain.\n"
// END APIQA
	);
#ifdef _WIN32
