...
// APIQA:
	if (consPerfConfig.hostNameProvided == RSSL_TRUE)
	{
...
// APIQA:
	cInfo.rsslConnectOptions.proxyOpts.proxyHostName = consPerfConfig.proxyHost;
	cInfo.rsslConnectOptions.proxyOpts.proxyPort = consPerfConfig.proxyPort;
	cInfo.rsslConnectOptions.proxyOpts.proxyUserName = consPerfConfig.proxyLogin;
	cInfo.rsslConnectOptions.proxyOpts.proxyPasswd = consPerfConfig.proxyPassword;
	cInfo.rsslConnectOptions.proxyOpts.proxyDomain = consPerfConfig.proxyDomain;

	cInfo.enableSessionManagement = consPerfConfig.enableSessionMgnt;
	cInfo.location.data = consPerfConfig.location;
	cInfo.location.length = (RsslUInt32)strlen(consPerfConfig.location);

	if (strlen(consPerfConfig.clientId) != 0)
	{
		rsslClearReactorOAuthCredential(&oauthCredential);
		oauthCredential.clientId.data = consPerfConfig.clientId;
		oauthCredential.clientId.length = (RsslUInt32)strlen(consPerfConfig.clientId);
		if (strlen(consPerfConfig.clientSecret) != 0)
		{
			oauthCredential.clientSecret.data = consPerfConfig.clientSecret;
			oauthCredential.clientSecret.length = (RsslUInt32)strlen(consPerfConfig.clientSecret);
		}
		if (strlen(consPerfConfig.password) != 0)
		{
			oauthCredential.password.data = consPerfConfig.password;
			oauthCredential.password.length = (RsslUInt32)strlen(consPerfConfig.password);
		}
#if 0
		oauthCredential.takeExclusiveSignOnControl = consPerfConfig.takeExclusiveSignOnControl;
		if (strlen(consPerfConfig.tokenScope) != 0)
		{
			oauthCredential.tokenScope.data = consPerfConfig.tokenScope;
			oauthCredential.tokenScope.length = strlen(consPerfConfig.tokenScope);
		}
#endif
		pConsumerThread->consumerRole.pOAuthCredential = &oauthCredential; /* This is used only when the session management is enabled */
	}
// END APIQA
