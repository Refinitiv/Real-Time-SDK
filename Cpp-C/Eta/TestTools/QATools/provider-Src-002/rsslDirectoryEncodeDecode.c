...
RsslRet encodeSourceDirectoryResponse(RsslChannel* chnl, RsslSourceDirectoryResponseInfo* srcDirRespInfo, RsslMsgKey* requestKey, RsslBuffer* msgBuf, RsslUInt16 refreshFlags)
{
	...
	RsslBuffer originalServiceName;
	originalServiceName.data = (char*)malloc(256*sizeof(char));

	originalServiceName.length = (RsslUInt32)strlen(srcDirRespInfo->ServiceGeneralInfo.ServiceName);
	memcpy(originalServiceName.data,srcDirRespInfo->ServiceGeneralInfo.ServiceName,originalServiceName.length);
	originalServiceName.data[originalServiceName.length] = '\0';
        //END APIQA
	...
	if ((ret = rsslEncodeMsgInit(&encodeIter, (RsslMsg*)&msg, 0)) < RSSL_RET_SUCCESS)
	{
		...
		for (i = 0; i < 100; i++)
		{
			...
			if ((!(requestKey->flags & RSSL_MKF_HAS_SERVICE_ID) && !(requestKey->flags & RSSL_MKF_HAS_NAME)) ||
				...
				((requestKey->flags & RSSL_MKF_HAS_NAME) && rsslBufferIsEqual(&requestKey->name, &originalServiceName) && !(requestKey->flags & RSSL_MKF_HAS_SERVICE_ID)) ||
				(((requestKey->flags & RSSL_MKF_HAS_SERVICE_ID) && (requestKey->serviceId == srcDirRespInfo->ServiceId) &&
				(requestKey->flags & RSSL_MKF_HAS_NAME) && rsslBufferIsEqual(&requestKey->name, &originalServiceName))))
			{
				/* encode map entry */
				mEntry.action = RSSL_MPEA_ADD_ENTRY;
				serviceId = srcDirRespInfo->ServiceId + i;
				...
				sprintf(srcDirRespInfo->ServiceGeneralInfo.ServiceName, "%s%i", originalServiceName.data, i);
