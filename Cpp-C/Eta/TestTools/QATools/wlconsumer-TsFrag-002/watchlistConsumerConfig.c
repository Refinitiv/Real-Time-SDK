...
void printUsageAndExit(int argc, char **argv)
{
	printf("Usage: %s"
			// APIQA:
			" or %s [-c <Connection Type> ] [-if <Interface Name>] [ -u <Login UserName> ] [-s <ServiceName>] [ -mp <MarketPrice ItemName> ] [ -mbo <MarketByOrder ItemName> ] [ -mbp <MarketByPrice ItemName> ] [ -yc <YieldCurve ItemName> ] [ -sl <SymbolList ItemName> ] [ -view ] [-x] [ -msgSize <Message Size> ] [ -runTime <TimeToRun> ]\n"
			// END APIQA:
			...
			// APIQA:
			" -msgSize Specify message size to use when sending a tunnel stream message.\n"
			// END APIQA:
...
// APIQA: 
		else if (0 == strcmp(argv[i], "-msgSize"))
		{
			if (++i == argc) printUsageAndExit(argc, argv);
			watchlistConsumerConfig.msgSize = atoi(argv[i]);
		}
		// END APIQA:
