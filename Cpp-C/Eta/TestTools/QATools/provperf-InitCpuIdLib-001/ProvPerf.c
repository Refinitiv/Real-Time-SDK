...
int main(int argc, char **argv)
{
	...
	if (provPerfConfig.useReactor == RSSL_FALSE) // use ETA Channel
	{
		...
		initOpts.rsslLocking = (providerThreadConfig.threadCount > 1 ? RSSL_LOCK_GLOBAL : RSSL_LOCK_NONE);
		if (provPerfConfig.shouldInitializeCPUIDlib == 0)
		{
			initOpts.shouldInitializeCPUIDlib = 0;
		}
