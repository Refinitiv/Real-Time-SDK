...
int main(int argc, char **argv)
{
	...
	******************************************************************************************************************/
	...
	rsslClearQos(&upaChannelManagementInfo.upaProviderQos);
	upaChannelManagementInfo.upaProviderQos.dynamic = RSSL_FALSE; /*!< @brief If RSSL_TRUE, Rssl Qos is dynamic (used to describe the changeability of the quality of service, typically over the life of a data stream) */
	upaChannelManagementInfo.upaProviderQos.rate = RSSL_QOS_RATE_TICK_BY_TICK; /*!< Rate is Tick By Tick, indicates every change to information is conveyed */
	upaChannelManagementInfo.upaProviderQos.timeliness = RSSL_QOS_TIME_REALTIME; /*!< Timeliness is Realtime, indicates information is updated as soon as new information becomes available */

	/* Initialize item handler - initialize/reset market price item information */
	upaChannelManagementInfo.marketPriceItemRequestInfo.ItemData = (void*)&marketPriceItem;
	...
	******************************************************************************************************************/
	...
	while (1)
	{
		...
		if (selRet == 0)
		{
			...
			if ((retval = sendMarketPriceItemResponse(&upaChannelManagementInfo, &dataDictionary)) > RSSL_RET_SUCCESS)
			{
				...
				while (retval_rsslRead > RSSL_RET_SUCCESS) /* read until no more to read */
				{
					...
					if ((msgBuf = rsslRead(upaChannelManagementInfo.upaChannel, &retval_rsslRead, &error)) != 0)
					{
						...
						switch ( msg.msgBase.domainType )
						{
							...
							case 200:
							{
								if ((retval = processMarketPriceItemRequest(&upaChannelManagementInfo, &msg, &decodeIter, &dataDictionary)) > RSSL_RET_SUCCESS)
								{
									/* There is still data left to flush, leave our write notification enabled so we get called again.
									 * If everything wasn't flushed, it usually indicates that the TCP output buffer cannot accept more yet
									 */
...
RsslRet processMarketPriceItemRequest(UpaChannelManagementInfo *upaChannelManagementInfo, RsslMsg* msg, RsslDecodeIterator* decodeIter, RsslDataDictionary* dataDictionary)
{
	...
	switch(msg->msgBase.msgClass)
	{
		case RSSL_MC_REQUEST:
		{
			...
			if (requestKey->serviceId != upaChannelManagementInfo->sourceDirectoryRequestInfo.ServiceId)
			{
				if (sendMarketPriceItemRequestRejectStatusMsg(upaChannelManagementInfo, msg->msgBase.streamId, INVALID_SERVICE_ID) != RSSL_RET_SUCCESS)
					return RSSL_RET_FAILURE;
				break;
			}
				...
				((RsslRequestMsg *)msg)->flags & RSSL_RQMF_HAS_QOS)
			{
				...
				if (!rsslQosIsInRange(&((RsslRequestMsg *)msg)->qos, &((RsslRequestMsg *)msg)->worstQos, &upaChannelManagementInfo->upaProviderQos))
				{
					if (sendMarketPriceItemRequestRejectStatusMsg(upaChannelManagementInfo, msg->msgBase.streamId, QOS_NOT_SUPPORTED) != RSSL_RET_SUCCESS)
						return RSSL_RET_FAILURE;
					break;
				}
			}
			else if (((RsslRequestMsg *)msg)->flags & RSSL_RQMF_HAS_QOS)
			{
				...
				if (!rsslQosIsEqual(&((RsslRequestMsg *)msg)->qos, &upaChannelManagementInfo->upaProviderQos))
				{
					if (sendMarketPriceItemRequestRejectStatusMsg(upaChannelManagementInfo, msg->msgBase.streamId, QOS_NOT_SUPPORTED) != RSSL_RET_SUCCESS)
						return RSSL_RET_FAILURE;
					break;
			...
			{
				if (sendMarketPriceItemRequestRejectStatusMsg(upaChannelManagementInfo, msg->msgBase.streamId, KEY_ENC_ATTRIB_NOT_SUPPORTED) != RSSL_RET_SUCCESS)
					return RSSL_RET_FAILURE;
				break;
			}
			...
			if (upaChannelManagementInfo->marketPriceItemRequestInfo.IsInUse
				/* Compares two MsgKey structures to determine if they are the same */
				&& RSSL_RET_SUCCESS == rsslCompareMsgKeys(&upaChannelManagementInfo->marketPriceItemRequestInfo.MsgKey, requestKey))
			{
				/* The request has the same request key as one currently open for this channel for the Market Price domain. */

				if ( upaChannelManagementInfo->marketPriceItemRequestInfo.StreamId != msg->msgBase.streamId )
				{
					/* The request has a different stream ID, meaning it would open the same item on another stream.
						* This is not allowed */
					if (sendMarketPriceItemRequestRejectStatusMsg(upaChannelManagementInfo, msg->msgBase.streamId, ITEM_ALREADY_OPENED) != RSSL_RET_SUCCESS)
						return RSSL_RET_FAILURE;
					break;
				}
				else
			...
			else if (upaChannelManagementInfo->marketPriceItemRequestInfo.StreamId == msg->msgBase.streamId)
			{
				/* This stream ID is already in use for a different item. */
				if (sendMarketPriceItemRequestRejectStatusMsg(upaChannelManagementInfo, msg->msgBase.streamId, STREAM_ALREADY_IN_USE) != RSSL_RET_SUCCESS)
					return RSSL_RET_FAILURE;
				break;
			}

			/* No matching items. This is a new request. */
			upaChannelManagementInfo->marketPriceItemRequestInfo.IsInUse = RSSL_TRUE;
			if (rsslCopyMsgKey(&upaChannelManagementInfo->marketPriceItemRequestInfo.MsgKey, requestKey) == RSSL_RET_FAILURE)
			{
				if (sendMarketPriceItemRequestRejectStatusMsg(upaChannelManagementInfo, msg->msgBase.streamId, ITEM_NOT_SUPPORTED) != RSSL_RET_SUCCESS)
					return RSSL_RET_FAILURE;
				break;
			}

			/* get item information */
			upaChannelManagementInfo->marketPriceItemRequestInfo.ItemName[upaChannelManagementInfo->marketPriceItemRequestInfo.MsgKey.name.length] = '\0';

			upaChannelManagementInfo->marketPriceItemRequestInfo.StreamId = msg->msgBase.streamId;

			/* Provide a refresh if one was requested. */
			upaChannelManagementInfo->marketPriceItemRequestInfo.IsRefreshComplete = (msg->requestMsg.flags & RSSL_RQMF_NO_REFRESH) ? RSSL_TRUE : RSSL_FALSE;

			printf("\nReceived Market Price Item Request for %.*s (streamId=%d)\n", (int)strlen(upaChannelManagementInfo->marketPriceItemRequestInfo.ItemName), upaChannelManagementInfo->marketPriceItemRequestInfo.ItemName, msg->msgBase.streamId);
			...
			{
				upaChannelManagementInfo->marketPriceItemRequestInfo.IsRefreshComplete = RSSL_FALSE;
				if (retval = sendMarketPriceItemResponse(upaChannelManagementInfo, dataDictionary) != RSSL_RET_SUCCESS)
					return retval;
			}
			...
			upaChannelManagementInfo->marketPriceItemRequestInfo.IsRefreshComplete = RSSL_TRUE;
		}
		break;
		...
		case RSSL_MC_CLOSE:
		{
			...
			/* close item stream */
			closeMarketPriceItemStream(msg->msgBase.streamId, upaChannelManagementInfo);
		}
		break;
...
RsslRet sendMarketPriceItemResponse(UpaChannelManagementInfo *upaChannelManagementInfo, RsslDataDictionary* dataDictionary)
{
	...
	mpItem = (UpaMarketPriceItem*)upaChannelManagementInfo->marketPriceItemRequestInfo.ItemData;

	/* set-up message */
	/* set message depending on whether refresh or update */

	if (!upaChannelManagementInfo->marketPriceItemRequestInfo.IsRefreshComplete) /* this is a refresh message */
	{
		...
		/* ServiceId */
		msgBase->msgKey.serviceId = (RsslUInt16)upaChannelManagementInfo->sourceDirectoryRequestInfo.ServiceId;
		...
		printf("UPA Interactive Provider application is providing a MarketPrice Item refresh message.\n\n");
	}
	else /* this is an update message */
	{
		...
		printf("UPA Interactive Provider application is providing a MarketPrice Item update message.\n\n");
	}
	...
	/* StreamId */
        msgBase->streamId = upaChannelManagementInfo->marketPriceItemRequestInfo.StreamId; 
	/* encode message - populate message and encode it */
	...
	if (!upaChannelManagementInfo->marketPriceItemRequestInfo.IsRefreshComplete)
	{
		/* RDNDISPLAY */
	...
	if (upaChannelManagementInfo->marketPriceItemRequestInfo.IsRefreshComplete == RSSL_FALSE)
	{
		upaChannelManagementInfo->marketPriceItemRequestInfo.IsRefreshComplete = RSSL_TRUE;
	}
...
RsslRet sendMarketPriceItemCloseStatusMsg(UpaChannelManagementInfo *upaChannelManagementInfo)
{
	...
	/* set-up message */
	msg.msgBase.msgClass = RSSL_MC_STATUS; /*!< (3) Status Message */
	msg.msgBase.streamId = upaChannelManagementInfo->marketPriceItemRequestInfo.StreamId;
	...
	msg.state.dataState = RSSL_DATA_SUSPECT;
	msg.state.code = RSSL_SC_NONE;
	sprintf(stateText, "Item stream closed for item: %s", upaChannelManagementInfo->marketPriceItemRequestInfo.ItemName);
	msg.state.text.data = stateText;
	msg.state.text.length = (RsslUInt32)strlen(stateText);
...
RsslRet sendMarketPriceItemRequestRejectStatusMsg(UpaChannelManagementInfo *upaChannelManagementInfo, RsslInt32 streamId, UpaItemRejectReason reason)
{
	...
	/* Obtains a non-packable buffer of the requested size from the UPA Transport guaranteed buffer pool to write into for the Market Price item request Reject Status Msg.
	 * When the RsslBuffer is returned, the length member indicates the number of bytes available in the buffer (this should match the amount
	 * the application requested). When populating, it is required that the application set length to the number of bytes actually used.
	 * This ensures that only the required bytes are written to the network.
	 */
...
{
	/* find the original request information associated with streamId */
	if (upaChannelManagementInfo->marketPriceItemRequestInfo.StreamId == streamId)
	{
		printf("Closing market price item stream id %d with item name: %.*s\n", upaChannelManagementInfo->marketPriceItemRequestInfo.StreamId, (int)strlen(upaChannelManagementInfo->marketPriceItemRequestInfo.ItemName), upaChannelManagementInfo->marketPriceItemRequestInfo.ItemName);
