...

{
	...
	if((domainType == RSSL_DMT_SYMBOL_LIST) && (msg->msgBase.msgKey.name.length != 0))
	{
		...
		etaItemListName.data = (char *)"_ETA_ITEM_LIST";
		etaItemListName.length = (RsslUInt32)strlen("_ETA_ITEM_LIST");
...
RsslRet sendGenericMessageLogin(RsslReactor* pReactor, RsslReactorChannel* pReactorChannel, RsslInt32 serviceId)
{
	...
	if (msgBuf != NULL)
	{
		...
		genericMsg.msgBase.streamId = 1;
		genericMsg.msgBase.containerType = RSSL_DT_ELEMENT_LIST;
		genericMsg.flags |= RSSL_GNMF_HAS_MSG_KEY;
		snprintf(nameChar, 20, "%s", "genericMsgInGeneral");
		snprintf(nameChar2, 20, "%s", "valueFromProvider");
		...
		elemEntry.name.data = nameChar2;
		elemEntry.name.length = (RsslUInt32)strlen(nameChar2);
		RsslInt tempInt = 2;
		...
		if (ret < RSSL_RET_SUCCESS)
		{
			printf("Error %s (%d) encountered with rsslEncodeElementListComple. Error Text: %s\n",
				rsslRetCodeToString(ret), ret, rsslRetCodeInfo(ret));
		}
		...
		if (sendMessage(pReactor, pReactorChannel, msgBuf) != RSSL_RET_SUCCESS)
		{
			fprintf(stderr, "\n\n-------------APIQ: Error submitting generic message on LOGIN domain streamId=%d\n", 1);
			return RSSL_RET_FAILURE;
		}
	}
	else
	{
		fprintf(stderr, "sendGenericMessageLogin: rsslReactorGetBuffer(): Failed <%s>\n", rsslErrorInfo.rsslError.text);
		return RSSL_RET_FAILURE;
	}
