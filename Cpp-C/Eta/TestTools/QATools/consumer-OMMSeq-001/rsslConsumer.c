...
//APIQA
static int _initialDirFilter = RDM_DIRECTORY_SERVICE_INFO_FILTER | \
...
static int _reissueDirFilter = -1;
static int _initialDictFilter = RDM_DICTIONARY_NORMAL;
static int _reissueDictFilter = -1;
static RsslBool _loadDictFromFile = RSSL_FALSE;

static RsslBool _reissueQos = RSSL_FALSE;
static RsslBool _reissueQosAndWorstQos = RSSL_FALSE;
static int _qosRate = 0;
static int _qosTimeliness = 0;
static int _worstQosRate = 0;
static int _worstQosTimeliness = 0;
static int _changeServiceId = 1;

static RsslBool _sendGenMsgOnLogin = RSSL_FALSE;
static RsslBool _sendGenMsgOnDirectory = RSSL_FALSE;
static RsslBool _sendDirReqWithoutLogin = RSSL_FALSE;
static RsslBool _sendDictReqWithoutLogin = RSSL_FALSE;
static RsslBool _sendItemReqWithoutLogin = RSSL_FALSE;
//END APIQA
...
void loginSuccessCallBack(RsslChannel* chnl)
{
	...
	if (isLoginReissue == RSSL_FALSE)
	{
		...
		{
			//APIQA
			if (_sendGenMsgOnLogin)
			{
				sendGenericMsgOnLogin(chnl);
				return;
			}
			//sendSourceDirectoryRequest(chnl);
			sendSourceDirectoryRequest(chnl,_sendGenMsgOnDirectory);
			//END APIQA
			if (offPostEnabled)
			{
				enableOffstreamPost();
