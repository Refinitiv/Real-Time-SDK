...
//APIQA
static RsslBool _isReissueQos = RSSL_FALSE;

static RsslBool _isReissueQosAndWorstQos = RSSL_FALSE;

static RsslQos _qos;

static RsslQos _worstQos;

static RsslBool _isReissueChangeServiceName = RSSL_FALSE;

static int _changeServiceId = 1;

static RsslBool _privateStreamReq = RSSL_FALSE;

static RsslBool _sendReqSameKeyWithDiffStreamId = RSSL_FALSE;

static int _numberOfUpdateMessages = 0;

static int _specificServiceId = -1;

/* Rate value definition
	- RSSL_QOS_RATE_TICK_BY_TICK = 0;
	- RSSL_QOS_RATE_JIT_CONFLATED = -1;
	- if 0 < rate <= 65535, RSSL_QOS_RATE_TIME_CONFLATED using the rate for rateInfo
*/

/* Timeliness value definition
    - RSSL_QOS_TIME_REALTIME = 0
	- RSSL_QOS_TIME_DELAYED_UNKNOWN = -1
	- if 0 < timeliness <= 65535, RSSL_QOS_TIME_DELAYED using the timelines for timeInfo
*/

void setRate(int rate,RsslQos* qos)
{
	if (rate == 0)
	{
...
void setTimeliness(int timeliness, RsslQos* qos)
{
	if (timeliness == 0)
	{
	...
	else if ((timeliness > 0) && (timeliness <= 65535))
	{
		qos->timeliness = RSSL_QOS_TIME_DELAYED;
		qos->timeInfo = timeliness;
	}
	else
	{
		qos->timeliness = RSSL_QOS_TIME_DELAYED_UNKNOWN;
...
{
	_isReissueQos = RSSL_TRUE;

	rsslClearQos(&_qos);

	setRate(rate, &_qos);
	setTimeliness(timeliness, &_qos);
}

void setReissueQosAndWorstQos(int rate, int timeliness, int worstRate, int worstTimeliness)
{
	_isReissueQosAndWorstQos = RSSL_TRUE;

	rsslClearQos(&_qos);
	rsslClearQos(&_worstQos);

	setRate(rate, &_qos);
	setTimeliness(timeliness, &_qos);

	setRate(worstRate, &_worstQos);
	setTimeliness(worstTimeliness, &_worstQos);
}

void setReissueChangeServiceName(int newServiceId)
{
	_isReissueChangeServiceName = RSSL_TRUE;
	_changeServiceId = newServiceId;
}

void setPriviateStream()
{
	_privateStreamReq = RSSL_TRUE;
}

void setReqSameKeyWithDiffStreamId()
{
	_sendReqSameKeyWithDiffStreamId = RSSL_TRUE;
}

void setSpecificServiceId(int id)
{
	_specificServiceId = id;
}
//END APIQA
/*
...
static RsslRet encodeItemRequest(RsslChannel* chnl, RsslBuffer* msgBuf, RsslInt32 streamId, RsslBool isPrivateStream)
{
	...
	if (msg.qos.rate == 0)
	{
		msg.qos.rate = RSSL_QOS_RATE_TICK_BY_TICK;
	}

	if (msg.qos.timeliness == 0)
	{
		msg.qos.timeliness = RSSL_QOS_TIME_REALTIME;
	}
	//END APIQA
	...
	if (_specificServiceId == -1)
	{
		msg.msgBase.msgKey.serviceId = (RsslUInt16)getServiceId();
	}
	else
	{
		msg.msgBase.msgKey.serviceId = _specificServiceId;
	}
	//END APIQA
...
//APIQA
static RsslRet encodeItemRequestReissue(RsslChannel* chnl, RsslBuffer* msgBuf, RsslInt32 streamId, RsslBool isPrivateStream,RsslQos* qos, RsslQos* worstQos)
{
	RsslRet ret = 0;
	RsslRequestMsg msg = RSSL_INIT_REQUEST_MSG;
	RsslEncodeIterator encodeIter;
	...
	/* copy the QoS information */
	rsslCopyQos(&(msg.qos), qos);

	if (worstQos)
	{
		msg.flags |= RSSL_RQMF_HAS_WORST_QOS;
		rsslCopyQos(&(msg.worstQos), worstQos);
	}
...
static RsslRet encodeItemRequestReissueService(RsslChannel* chnl, RsslBuffer* msgBuf, RsslInt32 streamId, RsslBool isPrivateStream, int serviceId)
{
	RsslRet ret = 0;
	RsslRequestMsg msg = RSSL_INIT_REQUEST_MSG;
	RsslEncodeIterator encodeIter;
	...
	msg.msgBase.msgKey.serviceId = (RsslUInt16)serviceId;
		...
		else 
		{
			...
			for (i = 0; i < itemCount; i++)
			{
				...
				if (msgBuf != NULL)
				{
					...
					if (encodeItemRequest(chnl, msgBuf, (i + MARKETPRICE_STREAM_ID_START), _privateStreamReq) != RSSL_RET_SUCCESS)
					//END APIQA
					{
						rsslReleaseBuffer(msgBuf, &error); 
					...
					if (sendMessage(chnl, msgBuf) != RSSL_RET_SUCCESS)
						return RSSL_RET_FAILURE;
					//APIQA
					if (_sendReqSameKeyWithDiffStreamId) // Send another request with same key but different stream ID
					{
						msgBuf = rsslGetBuffer(chnl, MAX_MSG_SIZE, RSSL_FALSE, &error);
						...
						{
							/* encode item request */
							if (encodeItemRequest(chnl, msgBuf, (i + MARKETPRICE_STREAM_ID_START + 1), _privateStreamReq) != RSSL_RET_SUCCESS)
							{
								rsslReleaseBuffer(msgBuf, &error);
								printf("\nMarket Price encodeItemRequest() failed\n");
								return RSSL_RET_FAILURE;
...
RsslRet sendMarketPriceItemReissue(RsslChannel* chnl, RsslQos* qos, RsslQos* worstQos)
{
	...
	char qosText[256];
	RsslBuffer qosBufferText = { 255, (char*)qosText };
	char worstQosText[256];
	RsslBuffer worstQosBufferText = { 255, (char*)worstQosText };
	
	rsslQosToString(&qosBufferText, qos);

	if (worstQos)
	{
		rsslQosToString(&worstQosBufferText, worstQos);
		printf("\nSend market price item reissue with Qos = %s\n WorstQos = %s\n", qosBufferText.data, worstQosBufferText.data);
	}
	else
	{
		printf("\nSend market price item reissue with Qos = %s\n", qosBufferText.data);
	...
	{
		/* encode item request */
		if (encodeItemRequestReissue(chnl, msgBuf, (i + MARKETPRICE_STREAM_ID_START), _privateStreamReq,qos, worstQos) != RSSL_RET_SUCCESS)
		{
			rsslReleaseBuffer(msgBuf, &error);
			printf("\nMarket Price encodeItemRequest() failed\n");
			return RSSL_RET_FAILURE;
...
RsslRet sendMarketPriceItemReissueService(RsslChannel* chnl, int serviceId)
{
	...
	{
		/* encode item request */
		if (encodeItemRequestReissueService(chnl, msgBuf, (i + MARKETPRICE_STREAM_ID_START), _privateStreamReq, serviceId) != RSSL_RET_SUCCESS)
		{
			rsslReleaseBuffer(msgBuf, &error);
			printf("\nMarket Price encodeItemRequest() failed\n");
			return RSSL_RET_FAILURE;
...
RsslRet processMarketPriceResponse(RsslChannel *chnl, RsslMsg* msg, RsslDecodeIterator* dIter)
{
	...
	switch(msg->msgBase.msgClass)
	{
		...
		case RSSL_MC_UPDATE:
			...
			//APIQA
			if (++_numberOfUpdateMessages == 4)
			{
				if (_isReissueQos)
				{
					printf("Reissue with change QoS");
					sendMarketPriceItemReissue(chnl, &_qos, 0);
				}
				else if (_isReissueQosAndWorstQos)
				{
					printf("Reissue with change QoS and WorstQoS");
					sendMarketPriceItemReissue(chnl, &_qos, &_worstQos);
				}
				else if (_isReissueChangeServiceName)
				{
					printf("Reissue with change service ID to %d", _changeServiceId);
					sendMarketPriceItemReissueService(chnl, _changeServiceId);
				}
			}
			//END APIQA
			break;
