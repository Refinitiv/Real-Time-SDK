...
char clientJWK1Block[2048];
char audience1Block[255];
//End API QA
char scopeBlock[128];
//API QA
...
RsslBuffer clientJWK1 = RSSL_INIT_BUFFER;
RsslBuffer audience1 = RSSL_INIT_BUFFER;
...
void parseCommandLine(int argc, char **argv)
{
	...
	/* Check usage and retrieve operating parameters */
	{
		...
		while(i < argc)
		{
	        ...
	        else if (0 == strcmp(argv[i], "-jwkFile1"))
			{
				...
				readSize = (int)fread(clientJWK1Block, sizeof(char), 2048, pFile);
				if (readSize == 0)
				{
					printf("Cannot read jwk file.\n");
					printUsageAndExit(argv[0]);
				}
				clientJWK1.data = clientJWK1Block;
				clientJWK1.length = readSize;

				fclose(pFile);
				i++;
			}
			else if (strcmp("-audience1", argv[i]) == 0)
			{
				i += 2;
				audience1.length = snprintf(audience1Block, sizeof(audience1Block), "%s", argv[i - 1]);
				audience1.data = audience1Block;
			}									
		...
		// API QA
	{
		...
		for (i = 0; i < channelCommandCount; ++i)
		{
			if (i == 0)
			{
				...
				reactorOAuthCredentialRenewal.clientJWK = clientJWK1;
			}
			else
			{
				reactorOAuthCredentialRenewal.clientSecret = clientSecret2;
				reactorOAuthCredentialRenewal.clientJWK = clientJWK2;
			}
		}
	}
		// End API QA
...
int main(int argc, char **argv)
{
	...
	for(i = 0; i < channelCommandCount; ++i)
	{
		...
		if (i == 0)
		{
			...
			/* If a JWK was specified */
			if (clientJWK1.length)
			{
				oAuthCredential.clientJWK = clientJWK1;
				/* Specified the RsslReactorOAuthCredentialEventCallback to get sensitive information as needed to authorize with the token service. */
				oAuthCredential.pOAuthCredentialEventCallback = oAuthCredentialEventCallback; 
			}
			/* If an audience was specified */ 
			if (audience1.length)
			{
				oAuthCredential.audience = audience1;
			} 
		}
		else
		{
			...
			{
				oAuthCredential.clientJWK = clientJWK2;
				/* Specified the RsslReactorOAuthCredentialEventCallback to get sensitive information as needed to authorize with the token service. */
				oAuthCredential.pOAuthCredentialEventCallback = oAuthCredentialEventCallback; 
			}
			/* If an audience was specified */ 
			if (audience1.length)
			{
				oAuthCredential.audience = audience2;
			} 
		}
		// END API QA
