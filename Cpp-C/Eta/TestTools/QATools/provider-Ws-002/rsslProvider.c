...
int main(int argc, char **argv)
{
	...
	for(iargs = 1; iargs < argc; ++iargs)
	{
		...
		else if (strcmp("-serverBlockingMode", argv[iargs]) == 0)
		{
			serverBlockingMode = RSSL_TRUE;
		}
		else if (strcmp("-channelsBlockingMode", argv[iargs]) == 0)
		{
...
static void readFromChannel(RsslChannel* chnl)
{
	...
	if(chnl->socketId != -1 && chnl->state == RSSL_CH_STATE_ACTIVE)
	{
		...
		if ((serverBlockingMode == RSSL_TRUE || channelsBlockingMode == RSSL_TRUE)
			&& chnl->protocolType == RSSL_JSON_PROTOCOL_TYPE
			&& ((RsslJsonSession*)chnl->userSpecPtr)->jsonSessionInitialized != RSSL_TRUE)
		{
			if (rsslJsonSessionInitialize((RsslJsonSession*)chnl->userSpecPtr, &error) != RSSL_RET_SUCCESS)
			{
				printf("Unable to initialize the Json Converter.  Error text: %s\n", error.text);
