...
RsslRet encodeSourceDirectoryResponse(RsslChannel* chnl, RsslSourceDirectoryResponseInfo* srcDirRespInfo, RsslMsgKey* requestKey, RsslBuffer* msgBuf, RsslUInt16 refreshFlags)
{
		  ...
		  (requestKey->flags & RSSL_MKF_HAS_NAME) && rsslBufferIsEqual(&requestKey->name, &tempBuffer))))
	{
		...
		if (requestKey->filter & RDM_DIRECTORY_SERVICE_GROUP_FILTER)
		{
			...
			byteArray[0] = 0;
			byteArray[1] = 1;
			memcpy(&srcDirRespInfo->ServiceGroupInfo.Group, &byteArray, 2);
			
			byteArray[0] = 0;
			byteArray[1] = 2;
			memcpy(&srcDirRespInfo->ServiceGroupInfo.MergedToGroup, &byteArray, 2);
			...
			rsslState.text.data = "All is well";
			rsslState.text.length = 11;
			
			srcDirRespInfo->ServiceGroupInfo.Status = rsslState;
		...
		{
			RsslBuffer data = { 3, "ABC" };
	        memcpy(&srcDirRespInfo->ServiceDataInfo.Data, &data, sizeof(RsslBuffer));
			srcDirRespInfo->ServiceDataInfo.Type = 1;
