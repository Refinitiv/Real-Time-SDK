...
static RsslRet encodePostWithMsg(RsslReactorChannel* chnl, RsslBuffer* msgBuf, RsslInt32 streamId, RsslBuffer *itemName)
{
	...
	{ /* Encode Field List for putting it as the attib container */
		...
		RsslBuffer keyBuffer = RSSL_INIT_BUFFER;
		RsslBuffer inputValue = RSSL_INIT_BUFFER;
		keyBuffer.length = 1024;
		keyBuffer.data = (char*)malloc(1024);

		rsslClearEncodeIterator(&eIter);
		rsslSetEncodeIteratorRWFVersion(&eIter, chnl->majorVersion, chnl->minorVersion);
		if ((ret = rsslSetEncodeIteratorBuffer(&eIter, &keyBuffer)) != RSSL_RET_SUCCESS)
		{
		...
		if ((ret = rsslEncodeFieldListInit(&eIter, &fList, 0, 0)) < RSSL_RET_SUCCESS)
		{
		...
		dictionaryEntry = dictionary->entriesArray[32745];
		if (dictionaryEntry)
		{
			fEntry.fieldId = 32745;
			fEntry.dataType = dictionaryEntry->rwfType;
			uintValue = 1;
			if ((ret = rsslEncodeFieldEntry(&eIter, &fEntry, (void*)&uintValue)) < RSSL_RET_SUCCESS)
			{
		...
		/* CD_TOKEN */
		rsslClearFieldEntry(&fEntry);
		dictionaryEntry = dictionary->entriesArray[32746];
		if (dictionaryEntry)
		{
			fEntry.fieldId = 32746;
			fEntry.dataType = dictionaryEntry->rwfType;
			inputValue.data = "QVFJQzV3TTJMWTRTZmN5WElqRNE9USTFNems1QUFKVE1RQUNNREk9Iw==";
			inputValue.length = (RsslUInt32)strlen(inputValue.data);
			if ((ret = rsslEncodeFieldEntry(&eIter, &fEntry, (void*)&inputValue)) < RSSL_RET_SUCCESS)
			{
				printf("rsslEncodeFieldEntry() failed with return code: %d\n", ret);
				return ret;
			}
		...
		/* CD_APPTYPE */
		rsslClearFieldEntry(&fEntry);
		dictionaryEntry = dictionary->entriesArray[32747];
		if (dictionaryEntry)
		{
		...
		/* CD_APPNAME */
		rsslClearFieldEntry(&fEntry);
		dictionaryEntry = dictionary->entriesArray[32748];
		if (dictionaryEntry)
		{
		...
		/* CD_APPVER */
		rsslClearFieldEntry(&fEntry);
		dictionaryEntry = dictionary->entriesArray[32749];
		if (dictionaryEntry)
		{
		...
		/* CD_RSRVD1 */
		rsslClearFieldEntry(&fEntry);
		dictionaryEntry = dictionary->entriesArray[32750];
		if (dictionaryEntry)
		{
		...
		keyBuffer.length = rsslGetEncodedBufferLength(&eIter);

		postMsg.msgBase.domainType = RSSL_DMT_CONTRIBUTION;
		postMsg.msgBase.msgKey.attribContainerType = RSSL_DT_FIELD_LIST;
