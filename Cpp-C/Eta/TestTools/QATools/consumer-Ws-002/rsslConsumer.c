...
static RsslBool socketBlockingMode = RSSL_FALSE;
...
int main(int argc, char **argv)
{
	...
	else if (argc > 1) /* all operating parameters entered by user */
	{
		...
		while(i < argc)
		{
			...
			else if (strcmp("-socketBlockingMode", argv[i]) == 0)
			{
				i++;
				socketBlockingMode = RSSL_TRUE;
			}
			else
			{
				...
				printf(" -socketBlockingMode if specified, turn on socket blocking mode\n");
#ifdef _WIN32
	...
	while(1)
	{
		...
		while(shouldRecoverConnection)
		{
			...
			/* If the connection protocol is JSON, intitalize the JSON converter for ACTIVE state (for blocking mode)*/
			if (socketBlockingMode == RSSL_TRUE
				&& rsslConsumerChannel != NULL
				&& rsslConsumerChannel->state == RSSL_CH_STATE_ACTIVE
				&& rsslConsumerChannel->protocolType == RSSL_JSON_PROTOCOL_TYPE
				&& ((RsslJsonSession*)rsslConsumerChannel->userSpecPtr)->jsonSessionInitialized != RSSL_TRUE)
			{
...
static RsslChannel* connectToRsslServer(RsslConnectionTypes connType, RsslError* error)
{
	...
	copts.blocking = socketBlockingMode;
