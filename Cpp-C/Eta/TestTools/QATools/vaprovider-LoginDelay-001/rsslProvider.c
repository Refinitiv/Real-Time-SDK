...
// API QA
static RsslUInt nSendingLoginRefreshDelayedCycles = 0;
// END API QA
...
void exitWithUsage()
{
	...
	// API QA
	printf(" -delaySendLoginRefresh <count>\tspecifies that Provider should delay sending of Login Refresh on the number of main-loop cycles\n");
	// END API QA
#ifdef _WIN32
...
int main(int argc, char **argv)
{
	...
	for (iargs = 1; iargs < argc; ++iargs)
	{
		...
		// API QA
		else if (0 == strcmp("-delaySendLoginRefresh", argv[iargs]))
		{
			++iargs; if (iargs == argc) exitWithUsage();
			nSendingLoginRefreshDelayedCycles = atoi(argv[iargs]);
		}
		// END API QA
		else
		{
			...
			if (FD_ISSET(rsslSrvr->socketId, &useRead))
			{
				...
				for (i = 0; i < MAX_CLIENT_SESSIONS; i++)
				{
					if (!clientSessions[i].isInUse)
					{
						...
						// API QA
						pClientSessionInfo->nSendingLoginRefreshDelayedCycles = nSendingLoginRefreshDelayedCycles;
						// END API QA
						break;
					}
				}
		...
		if (time(NULL) >= nextSendTime)
		{
			...
			for (i = 0; i < MAX_CLIENT_SESSIONS; i++)
			{
				if (clientSessions[i].clientChannel != NULL)
				{
					...
					// API QA
					if (clientSessions[i].nSendingLoginRefreshDelayedCycles > 0)
					{
						if (--clientSessions[i].nSendingLoginRefreshDelayedCycles == 0)
						{
							// Sends a delayed login refresh.
							printf("Sends delayed Login Refresh.\n");
							sendLoginRefreshDelayed(pReactor, clientSessions[i].clientChannel);
						}
					}
					// END API QA
				}
...
// API QA
/*
* Should Provider delay sending Login Refresh on Consumer Login request in loginMsgCallback
*/
RsslBool delaySendLoginRefresh()
{
	if (nSendingLoginRefreshDelayedCycles > 0)
		return RSSL_TRUE;
	return RSSL_FALSE;  // default - Provider should send Login Refresh at once (normal way)
}
// END API QA
