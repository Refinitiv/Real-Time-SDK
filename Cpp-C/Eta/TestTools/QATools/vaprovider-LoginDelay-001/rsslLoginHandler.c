...
RsslReactorCallbackRet loginMsgCallback(RsslReactor *pReactor, RsslReactorChannel *pReactorChannel, RsslRDMLoginMsgEvent* pLoginMsgEvent)
{
	...
	switch(pLoginMsg->rdmMsgBase.rdmMsgType)
	{
		case RDM_LG_MT_REQUEST:
		{
			...
			/* send login response */
			if (!delaySendLoginRefresh())  // API QA
			{
			...
			// API QA
				...
				printf("Sending Login Refresh is delayed.\n");
			}
			// END API QA
...
// API QA
/*
* Sends a delayed login refresh to a channel.
*/
RsslRet sendLoginRefreshDelayed(RsslReactor* pReactor, RsslReactorChannel* pReactorChannel)
{
	RsslUInt i;
	LoginRequestInfo* reqInfo = NULL;
	...
	if (reqInfo == NULL)
	{
		printf("sendLoginRefreshDelayed() Error: LoginRequestInfo was not found.\n");
		return ret;
	}

	ret = sendLoginRefresh(pReactor, pReactorChannel, &reqInfo->loginRequest);
