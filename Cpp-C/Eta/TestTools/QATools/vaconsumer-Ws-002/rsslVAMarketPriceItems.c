...
RsslRet updateMarketPriceItemFieldsFromPost(RsslMarketPriceItem* mpItem, RsslDecodeIterator* dIter, RsslErrorInfo *error)
{
	...
	while ((ret = rsslDecodeFieldEntry(dIter, &fEntry)) != RSSL_RET_END_OF_CONTAINER)
	{
		...
		case PRCTCK_1_FID:
			...
			{
				sprintf(error->rsslError.text, "Error=%d Decoding PRCTCK_1", ret);
				error->rsslError.rsslErrorId = ret;
				return ret;
			}
			break;
		...
		case LST_PRCTCK_FID:
			if ((ret = rsslDecodeEnum(dIter, &mpItem->LST_PRCTCK)) != RSSL_RET_SUCCESS)
			{
				sprintf(error->rsslError.text, "Error=%d Decoding LST_PRCTCK", ret);
				error->rsslError.rsslErrorId = ret;
				return ret;
			}
			break;
		...
		case DIVIDENDTP_FID:
			if ((ret = rsslDecodeEnum(dIter, &mpItem->DIVIDENDTP)) != RSSL_RET_SUCCESS)
			{
				sprintf(error->rsslError.text, "Error=%d Decoding DIVIDENDTP", ret);
				error->rsslError.rsslErrorId = ret;
				return ret;
			}
			break;
		...
		case TRD_UNITS_FID:
			if ((ret = rsslDecodeEnum(dIter, &mpItem->TRD_UNITS)) != RSSL_RET_SUCCESS)
			{
				sprintf(error->rsslError.text, "Error=%d Decoding TRD_UNITS", ret);
				error->rsslError.rsslErrorId = ret;
				return ret;
			}
			break;
		...
		case MPV_FID:
			if ((ret = rsslDecodeEnum(dIter, &mpItem->MPV)) != RSSL_RET_SUCCESS)
			{
				sprintf(error->rsslError.text, "Error=%d Decoding MPV", ret);
				error->rsslError.rsslErrorId = ret;
				return ret;
			}
			break;
		...
		case LOTSZUNITS_FID:
			if ((ret = rsslDecodeEnum(dIter, &mpItem->LOTSZUNITS)) != RSSL_RET_SUCCESS)
			{
				sprintf(error->rsslError.text, "Error=%d Decoding LOTSZUNITS", ret);
				error->rsslError.rsslErrorId = ret;
				return ret;
			}
			break;
...
RsslRet encodeMPFieldList(RsslItemInfo* itemInfo, RsslEncodeIterator* encodeIter, RsslBool isPrivateStream, RsslDataDictionary* dictionary)
{
	...
	if (!itemInfo->IsRefreshComplete)
	{
		...
		if (dictionaryEntry)
		{
			fEntry.fieldId = PRCTCK_1_FID;
			fEntry.dataType = dictionaryEntry->rwfType;
			if ((ret = rsslEncodeFieldEntry(encodeIter, &fEntry, (void*)&mpItem->PRCTCK_1)) < RSSL_RET_SUCCESS)
			{
				printf("rsslEncodeFieldEntry() failed with return code: %d\n", ret);
				return ret;
			}
		...
		/* LST_PRCTCK */
		rsslClearFieldEntry(&fEntry);
		dictionaryEntry = dictionary->entriesArray[LST_PRCTCK_FID];
		if (dictionaryEntry)
		{
			fEntry.fieldId = LST_PRCTCK_FID;
			fEntry.dataType = dictionaryEntry->rwfType;
			if ((ret = rsslEncodeFieldEntry(encodeIter, &fEntry, (void*)&mpItem->LST_PRCTCK)) < RSSL_RET_SUCCESS)
			{
				printf("rsslEncodeFieldEntry() failed with return code: %d\n", ret);
				return ret;
			}
		...
		/* DIVIDENDTP */
		rsslClearFieldEntry(&fEntry);
		dictionaryEntry = dictionary->entriesArray[DIVIDENDTP_FID];
		if (dictionaryEntry)
		{
			fEntry.fieldId = DIVIDENDTP_FID;
			fEntry.dataType = dictionaryEntry->rwfType;
			if ((ret = rsslEncodeFieldEntry(encodeIter, &fEntry, (void*)&mpItem->DIVIDENDTP)) < RSSL_RET_SUCCESS)
			{
				printf("rsslEncodeFieldEntry() failed with return code: %d\n", ret);
				return ret;
			}
		...
		/* TRD_UNITS */
		rsslClearFieldEntry(&fEntry);
		dictionaryEntry = dictionary->entriesArray[TRD_UNITS_FID];
		if (dictionaryEntry)
		{
			fEntry.fieldId = TRD_UNITS_FID;
			fEntry.dataType = dictionaryEntry->rwfType;
			if ((ret = rsslEncodeFieldEntry(encodeIter, &fEntry, (void*)&mpItem->TRD_UNITS)) < RSSL_RET_SUCCESS)
			{
				printf("rsslEncodeFieldEntry() failed with return code: %d\n", ret);
				return ret;
			}
		...
		/* MPV */
		rsslClearFieldEntry(&fEntry);
		dictionaryEntry = dictionary->entriesArray[MPV_FID];
		if (dictionaryEntry)
		{
			fEntry.fieldId = MPV_FID;
			fEntry.dataType = dictionaryEntry->rwfType;
			if ((ret = rsslEncodeFieldEntry(encodeIter, &fEntry, (void*)&mpItem->MPV)) < RSSL_RET_SUCCESS)
			{
				printf("rsslEncodeFieldEntry() failed with return code: %d\n", ret);
				return ret;
			}
		...
		/* LOTSZUNITS */
		rsslClearFieldEntry(&fEntry);
		dictionaryEntry = dictionary->entriesArray[LOTSZUNITS_FID];
		if (dictionaryEntry)
		{
			fEntry.fieldId = LOTSZUNITS_FID;
			fEntry.dataType = dictionaryEntry->rwfType;
			if ((ret = rsslEncodeFieldEntry(encodeIter, &fEntry, (void*)&mpItem->LOTSZUNITS)) < RSSL_RET_SUCCESS)
			{
				printf("rsslEncodeFieldEntry() failed with return code: %d\n", ret);
				return ret;
			}
