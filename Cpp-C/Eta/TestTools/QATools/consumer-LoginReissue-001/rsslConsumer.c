 ...
 *|           Copyright (C) 2017-2018,2020,2024 LSEG. All rights reserved.
 *|-----------------------------------------------------------------------------
 */
...
*/
// APIQA: Adding counters here to track when to send reissues
int srcDirCounter = 0;
int updCounter = 0;
int sentPause = 0;
// END APIQA: 
int main(int argc, char **argv)
{
	...
	while(1)
	{
		...
		// APIQA: Adding extra parameter on sendLoginRequest
		if (sendLoginRequest(rsslConsumerChannel, "rsslConsumer", RSSL_CONSUMER, &loginSuccessCallBack, 0) != RSSL_RET_SUCCESS)
		{
			cleanUpAndExit();
		}
		...
		switch (msg.msgBase.domainType)
	{
		...
		case RSSL_DMT_SOURCE:
			...
			if (srcDirCounter == 2)
			{
				if (sendLoginRequest(rsslConsumerChannel, "rsslConsumer", RSSL_CONSUMER, &loginSuccessCallBack, 0) != RSSL_RET_SUCCESS)
				{
					printf("APIQA: Sending RESUME failed\n");
				}
				else
				{
					printf("APIQA: Sending RESUME\n");	
				}
			}
			// END APIQA
		...
		case RSSL_DMT_MARKET_PRICE:
			...
			if (updCounter == 3)
			{
				if (sendLoginRequest(rsslConsumerChannel, "rsslConsumer", RSSL_CONSUMER, &loginSuccessCallBack, 1) != RSSL_RET_SUCCESS)
				{
					printf("APIQA: Sending PAUSE failed\n");
				}
				else
				{
					printf("APIQA: Sending PAUSE\n");	
					sentPause = 1;
				}
			}
			// END APIQA
...
void loginSuccessCallBack(RsslChannel* chnl)
{
	...
	if (isLoginReissue == RSSL_FALSE)
	{
		...
		{
			// APIQA: sending source directory request on a reissue only if there is no pause 
			if (sentPause == 0) 
			{
