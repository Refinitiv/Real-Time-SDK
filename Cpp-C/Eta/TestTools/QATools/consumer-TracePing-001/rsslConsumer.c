...
int main(int argc, char **argv)
{
	...
	else if (argc > 1) /* all operating parameters entered by user */
	{
		...
		while(i < argc)
		{
			...
			// API QA
			else if (strcmp("-xping", argv[i]) == 0)
			{
				i++;
				xmlTracePing = RSSL_TRUE;
				snprintf(traceOutputFile, 128, "RsslConsumer\0");
			}
			else if (strcmp("-x1", argv[i]) == 0)
			{
				i++;
				xmlTrace1 = RSSL_TRUE;
				snprintf(traceOutputFile, 128, "RsslConsumer\0");
			}
			else if (strcmp("-xnowrite", argv[i]) == 0)
			{
				i++;
				xmlTraceNoWrite = RSSL_TRUE;
			}
			else if (strcmp("-xnoread", argv[i]) == 0)
			{
				i++;
...
static RsslChannel* connectToRsslServer(RsslConnectionTypes connType, RsslError* error)
{
	...
	if ( (chnl = rsslConnect(&copts,error)) != 0)
	{
		...
		else if (xmlTrace1)
		{
			...
			rsslClearTraceOptions(&traceOptions);
			traceOptions.traceMsgFileName = traceOutputFile;
			traceOptions.traceFlags |= RSSL_TRACE_TO_FILE_ENABLE | RSSL_TRACE_TO_MULTIPLE_FILES | RSSL_TRACE_TO_STDOUT | RSSL_TRACE_READ | RSSL_TRACE_WRITE | RSSL_TRACE_DUMP | RSSL_TRACE_PING_ONLY;
			if (xmlTraceNoWrite)
				traceOptions.traceFlags &= ~(RSSL_TRACE_WRITE);
			if (xmlTraceNoRead)
				traceOptions.traceFlags &= ~(RSSL_TRACE_READ);
