...
// API QA
static RsslUInt cpuBindMainThreadVersion = 1;  // 1 - leverage rsslBindThread(), 2 - leverage rsslBindThreadEx()
RsslBuffer cpuBindMainThread = RSSL_INIT_BUFFER;
static char mainThreadBinding[1024];
RsslBuffer cpuBindWorkerThread = RSSL_INIT_BUFFER;
static char workerThreadBinding[1024];

RsslBool isSetCpuStrToConvert = RSSL_FALSE;
RsslBuffer cpuStrToConvert = RSSL_INIT_BUFFER;
static char cpuStr_text[1024];
// END API QA
...
void printUsageAndExit(char *appName)
{
	...
	printf("Usage: %s or\n%s  [-tcp|-encrypted|-encryptedSocket|-encryptedWebSocket|-encryptedHttp [<hostname>:<port> <service name>]] [<domain>:<item name>,...] ] [-uname <LoginUsername>] [-passwd <LoginPassword>] [ -clientId <Client ID> ] [-sessionMgnt] [-view] [-post] [-offpost] [-snapshot] [-runtime <seconds>] [-cache] [-cacheInterval <seconds>] [-statisticInterval <seconds>] [-tunnel] [-tsDomain <number> ] [-tsAuth] [-tsServiceName] [-x] [-runtime] [-rtt]\n"
			...
			// API QA
			"\n"
			"\n -mainThreadBindVer <integer value> specifies the version of API call to bind main thread: 1 - use regular (default), 2 - use extended.\n"
			"\n -mainThreadCPU <string> specifies the CPU to bind for main VAConsumer thread.\n"
			"\n -workerThreadCPU <string> specifies the CPU to bind for Reactor worker thread.\n"
			"\n -cpuStrToConvert <string> specifies CPU (list of CPUs) in PCT format or logical Ids to convert to logical Ids.\n"
			// END API QA
			"\n"
		, appName, appName);
...
void parseCommandLine(int argc, char **argv)
{
	...
	/* Check usage and retrieve operating parameters */
	{
		...
		// API QA
		snprintf(mainThreadBinding, sizeof(mainThreadBinding), "%s", "");
		snprintf(workerThreadBinding, sizeof(workerThreadBinding), "%s", "");
		snprintf(cpuStr_text, sizeof(cpuStr_text), "%s", "");
		// END API QA
		...
		while(i < argc)
		{
			...
			// API QA
			else if (strcmp("-mainThreadBindVer", argv[i]) == 0)
			{
				i += 2; if (i > argc) printUsageAndExit(argv[0]);
				cpuBindMainThreadVersion = atoi(argv[i - 1]);
				if (cpuBindMainThreadVersion != 1 && cpuBindMainThreadVersion != 2)
					cpuBindMainThreadVersion = 1;
			}
			else if (strcmp("-mainThreadCPU", argv[i]) == 0)
			{
				i += 2; if (i > argc) printUsageAndExit(argv[0]);
				cpuBindMainThread.length = snprintf(mainThreadBinding, sizeof(mainThreadBinding), "%s", argv[i - 1]);
				cpuBindMainThread.data = mainThreadBinding;
			}
			else if (strcmp("-workerThreadCPU", argv[i]) == 0)
			{
				i += 2; if (i > argc) printUsageAndExit(argv[0]);
				cpuBindWorkerThread.length = snprintf(workerThreadBinding, sizeof(workerThreadBinding), "%s", argv[i - 1]);
				cpuBindWorkerThread.data = workerThreadBinding;
			}
			else if (strcmp("-cpuStrToConvert", argv[i]) == 0)
			{
				i += 2; if (i > argc) printUsageAndExit(argv[0]);
				cpuStrToConvert.length = snprintf(cpuStr_text, sizeof(cpuStr_text), "%s", argv[i - 1]);
				cpuStrToConvert.data = cpuStr_text;
				isSetCpuStrToConvert = RSSL_TRUE;
			}
			// END API QA
			else
			{
...
int main(int argc, char **argv)
{
	...
	// API QA
	if (cpuBindMainThread.data != NULL && cpuBindMainThread.length > 0)
	{
		printf("\n");
		printf("Main thread binds to: %s\n", cpuBindMainThread.data);
		printf("Bind variant: %llu, by using ETA API method %s.\n", cpuBindMainThreadVersion,
			(cpuBindMainThreadVersion == 1 ? "rsslBindThread()" : "rsslBindTreadEx()"));
	}
	if (cpuBindWorkerThread.data != NULL && cpuBindWorkerThread.length > 0)
	{
		printf("\n");
		printf("Reactor Worker thread binds to: %s\n\n", cpuBindWorkerThread.data);
	}

	if (isSetCpuStrToConvert == RSSL_TRUE)
	{
		RsslBuffer cpuStrLogicalIds = RSSL_INIT_BUFFER;
		char cpuStrLogicalIds_text[1024];

		printf("\nCall rsslGetLogicalCpuIdsbyPCT(). cpuStrToConvert: {%s}\n", cpuStr_text);

		cpuStrLogicalIds.data = cpuStrLogicalIds_text;
		cpuStrLogicalIds.length = sizeof(cpuStrLogicalIds_text);

		if (rsslGetLogicalCpuIdsbyPCT(cpuStr_text, &cpuStrLogicalIds, &rsslErrorInfo) != RSSL_RET_SUCCESS)
		{
			printf("rsslGetLogicalCpuIdsbyPCT() Failed. error.text: {%s}\n\n", rsslErrorInfo.rsslError.text);
		}
		else
		{
			printf("rsslGetLogicalCpuIdsbyPCT() Ok. Result LogicalIds: {%s}\n\n", cpuStrLogicalIds.data);
		}
	}
	// END API QA
	...
	if (cpuBindWorkerThread.data != NULL && cpuBindWorkerThread.length > 0)
	{
		reactorOpts.cpuBindWorkerThread = cpuBindWorkerThread;
	}
	// END API QA
	...
	if (cpuBindMainThread.data != NULL && cpuBindMainThread.length > 0)
	{
		if (cpuBindMainThreadVersion == 1)
		{
			if (rsslBindThread(cpuBindMainThread.data, &rsslErrorInfo) != RSSL_RET_SUCCESS)
			{
				printf("rsslBindThread failed! cpuString: {%s} error: %s\n", cpuBindMainThread.data, rsslErrorInfo.rsslError.text);
				exit(-1);
			}
			printf("rsslBindThread for main thread successful.\n\n");
		}
		else
		{
			RsslBuffer outputBuf;
			char textResult[256] = "";
			outputBuf.data = textResult;
			outputBuf.length = sizeof(textResult);

			if (rsslBindThreadEx(cpuBindMainThread.data, &outputBuf, &rsslErrorInfo) != RSSL_RET_SUCCESS)
			{
				printf("rsslBindThreadEx failed! cpuString: {%s} error: %s\n", cpuBindMainThread.data, rsslErrorInfo.rsslError.text);
				printf("Result-text: %s\n", outputBuf.data);
				exit(-1);
			}
			printf("rsslBindThreadEx for main thread successful.\n");
			printf("Result-text: %s\n\n", outputBuf.data);
		}
	}
	// END API QA
