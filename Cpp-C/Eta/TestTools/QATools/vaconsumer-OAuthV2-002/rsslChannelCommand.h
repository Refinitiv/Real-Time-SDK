...
RTR_C_INLINE void initChannelCommand(ChannelCommand *pCommand)
{
	...
	rsslClearBuffer(&pCommand->clientId);
	rsslClearBuffer(&pCommand->clientSecret);
	rsslClearBuffer(&pCommand->clientJWK);
	rsslClearBuffer(&pCommand->audience);
	pCommand->i = 0;
	// END API QA
}
