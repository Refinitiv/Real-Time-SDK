...
int main(int argc, char **argv)
{
	...
	else if (argc > 1) /* all operating parameters entered by user */
	{
		...
		while(i < argc)
		{
			...
			//END API QA
			// API QA
			else if (strcmp("-dumpIpcIn", argv[i]) == 0)
			{
				i++;
				dumpDebugFlags += RSSL_DEBUG_IPC_DUMP_IN;
			}
			else if (strcmp("-dumpIpcOut", argv[i]) == 0)
			{
				i++;
				dumpDebugFlags += RSSL_DEBUG_IPC_DUMP_OUT;
			}
			else if (strcmp("-dumpIpcComp", argv[i]) == 0)
			{
				i++;
				dumpDebugFlags += RSSL_DEBUG_IPC_DUMP_COMP;
			}
			else if (strcmp("-dumpIpcInit", argv[i]) == 0)
			{
				i++;
				dumpDebugFlags += RSSL_DEBUG_IPC_DUMP_INIT;
			}
			else if (strcmp("-dumpRsslIn", argv[i]) == 0)
			{
				i++;
				dumpDebugFlags += RSSL_DEBUG_RSSL_DUMP_IN;
			}
			else if (strcmp("-dumpRsslOut", argv[i]) == 0)
			{
				i++;
...
#endif
		// API QA
		if (activateDump(rsslConsumerChannel) != RSSL_RET_SUCCESS)
		{
...
static RsslChannel* connectToRsslServer(RsslConnectionTypes connType, RsslError* error)
{
	...
	if (testCompressionZlib)
		copts.compressionType = RSSL_COMP_ZLIB;
	// End API QA
