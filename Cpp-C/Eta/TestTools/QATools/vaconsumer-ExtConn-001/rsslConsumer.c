...
// API QA
static RsslInt32 timeToRun = 5000;
// END API QA
...
static RsslThreadId monitorThreadId;

char proxyHost2[256] = "webproxy.pln.colo.services";
char proxyPort2[256] = "80";
static RsslBool connectionListItems3 = RSSL_FALSE;


RSSL_THREAD_DECLARE(monitorThread, pArg)
{
	int i;
	...
	do
	{
		...
		for (i = 0; i < channelCommandCount; i++)
		{
			...
			if (pCommand && pCommand->reactorChannel)
			{
				pRsslChannel = pCommand->reactorChannel->pRsslChannel;
				if (pRsslChannel)
				{
					chanImpl = (rsslChannelImpl*)pRsslChannel;
					rsslSocketChannel = (RsslSocketChannel*)chanImpl->transportInfo;

					if (pRsslChannel->state == RSSL_CH_STATE_ACTIVE)
					{
						indErr = 0;
						...
						else if (rsslSocketChannel->state != RSSL_CH_STATE_ACTIVE)
						{
							indErr = 2;
						}
						else if (rsslSocketChannel->guarBufPool == NULL)
						{
							indErr = 3;
						}
						...
						{
							failTest = 4;
							printf("Warning 1 found in Client side!!! +++++++++++++++++++++++++++++++++++++\n");
							printf("pRsslChannel->state != RSSL_CH_STATE_ACTIVE, indErr(before Lock)=%d\n", indErr);
						...
						{
							failTest = 1;
							printf("Error 1 found in Client side!!! +++++++++++++++++++++++++++++++++++++\n");
							printf("rsslSocketChannel == NULL, indErr(before Lock)=%d\n", indErr);
							IPC_MUTEX_UNLOCK(rsslSocketChannel);
							assert(0); // , "rsslSocketChannel == NULL");
							break;
						}
						else if (rsslSocketChannel->state != RSSL_CH_STATE_ACTIVE)
						{
							failTest = 2;
							printf("Error 2 found in Client side!!! +++++++++++++++++++++++++++++++++++++\n");
							printf("rsslSocketChannel->state=%d, guarBufPool?=%s, indErr(before Lock)=%d\n",
								rsslSocketChannel->state,
								(rsslSocketChannel->guarBufPool == NULL ? "(null)" : "ok"),
								indErr);
							IPC_MUTEX_UNLOCK(rsslSocketChannel);
							assert(0); // , "rsslSocketChannel->state != RSSL_CH_STATE_ACTIVE");
							break;
						}
						else if (rsslSocketChannel->guarBufPool == NULL)
						{
							failTest = 3;
							printf("Error 3 found in Client side!!! +++++++++++++++++++++++++++++++++++++\n");
							printf("rsslSocketChannel->state=%d, guarBufPool?=%s, indErr(before Lock)=%d\n",
								rsslSocketChannel->state,
								(rsslSocketChannel->guarBufPool == NULL ? "(null)" : "ok"),
								indErr);
							IPC_MUTEX_UNLOCK(rsslSocketChannel);
							assert(0); // , "rsslSocketChannel->guarBufPool == NULL");
							break;
						}
						else if (indErr > 0)
						{
							failTest = 4;
							printf("Warning 2 found in Client side!!! Synchronization error!!! +++++++++++++++++++++++++++++++++++++\n");
							printf("pRsslChannel->state=%d, rsslSocketChannel->state=%d, guarBufPool?=%s, indErr(before Lock)=%d\n",
								pRsslChannel->state, rsslSocketChannel->state,
								(rsslSocketChannel->guarBufPool == NULL ? "(null)" : "ok"),
								indErr);
							IPC_MUTEX_UNLOCK(rsslSocketChannel);
							assert(0);
	...
	} while (!runTimeExpired && !failTest);

	printf("Monitor thread stopped. failTest = %d.   ++++++++++++++++++++++++++\n", failTest);
	return 0;
}
// END API QA
...
void printUsageAndExit(char *appName)
{
	...
	printf("Usage: %s or\n%s  [-tcp|-encrypted|-encryptedSocket|-encryptedWebSocket|-encryptedHttp [<hostname>:<port> <service name>]] [<domain>:<item name>,...] ] [-uname <LoginUsername>] [-passwd <LoginPassword>] [ -clientId <Client ID> ] [-sessionMgnt] [-view] [-post] [-offpost] [-snapshot] [-runtime <seconds>] [-cache] [-cacheInterval <seconds>] [-statisticInterval <seconds>] [-tunnel] [-tsDomain <number> ] [-tsAuth] [-tsServiceName] [-x] [-runtime] [-rtt]\n"
			...
			"\n"
			// API QA
			"\n QATool additional options to support connection list with 2 or 3 options:"
			"\n (1)direct (no proxy), (2)via proxy (-ph, -pp) and (3) via additonal proxy2 (-ph2, -pp2)."
			"\n -connectlist3 enable reactorConnectionList with 3 items (instead 2 items)"
			"\n Options for establishing connection(s) and sending requests through a proxy server2:\n"
			"   [ -ph2 <proxy host> ] [ -pp2 <proxy port> ]\n"
			"\n"
			// END API QA
		, appName, appName);
...
void parseCommandLine(int argc, char **argv)
{
		...
		while(i < argc)
		{
				...
				(strcmp("-encryptedSocket", argv[i]) == 0))
			{
				...
				if (strstr(argv[i], "-webSocket") != 0)
				{
					...
					// API QA
					pCommand->arr_cInfo[0].rsslConnectOptions.connectionType = RSSL_CONN_TYPE_WEBSOCKET;
					pCommand->arr_cInfo[0].rsslConnectOptions.wsOpts.protocols = protocolList;
					pCommand->arr_cInfo[1].rsslConnectOptions.connectionType = RSSL_CONN_TYPE_WEBSOCKET;
					pCommand->arr_cInfo[1].rsslConnectOptions.wsOpts.protocols = protocolList;
					pCommand->arr_cInfo[2].rsslConnectOptions.connectionType = RSSL_CONN_TYPE_WEBSOCKET;
					pCommand->arr_cInfo[2].rsslConnectOptions.wsOpts.protocols = protocolList;
					// END API QA
				}
				...
				if (strstr(argv[i], "-encrypted") != 0)
				{
					...
					// API QA
					pCommand->arr_cInfo[0].rsslConnectOptions.connectionType = RSSL_CONN_TYPE_ENCRYPTED;
					pCommand->arr_cInfo[1].rsslConnectOptions.connectionType = RSSL_CONN_TYPE_ENCRYPTED;
					pCommand->arr_cInfo[2].rsslConnectOptions.connectionType = RSSL_CONN_TYPE_ENCRYPTED;
					// END API QA
				}
				...
				if (strcmp("-encryptedHttp", argv[i]) == 0)
				{
					...
					// API QA
					pCommand->arr_cInfo[0].rsslConnectOptions.encryptionOpts.encryptedProtocol = RSSL_CONN_TYPE_HTTP;
					pCommand->arr_cInfo[1].rsslConnectOptions.encryptionOpts.encryptedProtocol = RSSL_CONN_TYPE_HTTP;
					pCommand->arr_cInfo[2].rsslConnectOptions.encryptionOpts.encryptedProtocol = RSSL_CONN_TYPE_HTTP;
					// END API QA
				}
				else if (strcmp("-encryptedWebSocket", argv[i]) == 0)
				{
					...
					pCommand->cInfo.rsslConnectOptions.wsOpts.protocols = protocolList;
					// API QA
					pCommand->arr_cInfo[0].rsslConnectOptions.encryptionOpts.encryptedProtocol = RSSL_CONN_TYPE_WEBSOCKET;
					pCommand->arr_cInfo[0].rsslConnectOptions.wsOpts.protocols = protocolList;
					pCommand->arr_cInfo[1].rsslConnectOptions.encryptionOpts.encryptedProtocol = RSSL_CONN_TYPE_WEBSOCKET;
					pCommand->arr_cInfo[1].rsslConnectOptions.wsOpts.protocols = protocolList;
					pCommand->arr_cInfo[2].rsslConnectOptions.encryptionOpts.encryptedProtocol = RSSL_CONN_TYPE_WEBSOCKET;
					pCommand->arr_cInfo[2].rsslConnectOptions.wsOpts.protocols = protocolList;
					// END API QA
				}
				else if (strcmp("-encryptedSocket", argv[i]) == 0)
				{
					...
					// API QA
					pCommand->arr_cInfo[0].rsslConnectOptions.encryptionOpts.encryptedProtocol = RSSL_CONN_TYPE_SOCKET;
					pCommand->arr_cInfo[1].rsslConnectOptions.encryptionOpts.encryptedProtocol = RSSL_CONN_TYPE_SOCKET;
					pCommand->arr_cInfo[2].rsslConnectOptions.encryptionOpts.encryptedProtocol = RSSL_CONN_TYPE_SOCKET;
					// END API QA
				}
				...
				if (enableSessionMgnt)
				{
					...
					// API QA
					pCommand->arr_cInfo[0].enableSessionManagement = enableSessionMgnt;
					pCommand->arr_cInfo[0].pAuthTokenEventCallback = authTokenEventCallback;
					pCommand->arr_cInfo[1].enableSessionManagement = enableSessionMgnt;
					pCommand->arr_cInfo[1].pAuthTokenEventCallback = authTokenEventCallback;
					pCommand->arr_cInfo[2].enableSessionManagement = enableSessionMgnt;
					pCommand->arr_cInfo[2].pAuthTokenEventCallback = authTokenEventCallback;
					// END API QA
				}
			...
			// API QA
			else if (0 == strcmp("-connectlist3", argv[i]))
			{
				i++;
				connectionListItems3 = RSSL_TRUE;
			}
			else if (strcmp("-ph2", argv[i]) == 0)
			{
				i += 2; if (i > argc) printUsageAndExit(argv[0]);
				snprintf(proxyHost2, sizeof(proxyHost2), "%s", argv[i - 1]);
			}
			else if (strcmp("-pp2", argv[i]) == 0)
			{
				i += 2; if (i > argc) printUsageAndExit(argv[0]);
				snprintf(proxyPort2, sizeof(proxyPort2), "%s", argv[i - 1]);
			}
			// END API QA
			else
			{
	...
	for (i = 0; i < channelCommandCount; i++)
	{
		...
		// API QA
		if (proxyHost[0] != '\0' && proxyPort[0] != '\0')
		{
			// Fill up only the 1-st and 2-nd elements, not the 0-th
			// The 0-th element is defined a no-proxy connection
			// The 1-st element is the proxy defined in command line (-ph, -pp)
			// The 2-nd element is the proxy2 defined in command line (-ph2, -pp2) or hardcoded
			pCommand->arr_cInfo[1].rsslConnectOptions.proxyOpts.proxyHostName = proxyHost;
			pCommand->arr_cInfo[1].rsslConnectOptions.proxyOpts.proxyPort = proxyPort;
			pCommand->arr_cInfo[1].rsslConnectOptions.proxyOpts.proxyUserName = proxyUserName;
			pCommand->arr_cInfo[1].rsslConnectOptions.proxyOpts.proxyPasswd = proxyPasswd;
			pCommand->arr_cInfo[1].rsslConnectOptions.proxyOpts.proxyDomain = proxyDomain;
			pCommand->arr_cInfo[1].rsslConnectOptions.encryptionOpts.openSSLCAStore = sslCAStore;

			pCommand->arr_cInfo[2].rsslConnectOptions.proxyOpts.proxyHostName = proxyHost2;
			pCommand->arr_cInfo[2].rsslConnectOptions.proxyOpts.proxyPort = proxyPort2;
			pCommand->arr_cInfo[2].rsslConnectOptions.proxyOpts.proxyUserName = proxyUserName;
			pCommand->arr_cInfo[2].rsslConnectOptions.proxyOpts.proxyPasswd = proxyPasswd;
			pCommand->arr_cInfo[2].rsslConnectOptions.proxyOpts.proxyDomain = proxyDomain;
			pCommand->arr_cInfo[2].rsslConnectOptions.encryptionOpts.openSSLCAStore = sslCAStore;

			if (tlsProtocol != RSSL_ENC_NONE)
			{
				pCommand->arr_cInfo[1].rsslConnectOptions.encryptionOpts.encryptionProtocolFlags = tlsProtocol;
				pCommand->arr_cInfo[2].rsslConnectOptions.encryptionOpts.encryptionProtocolFlags = tlsProtocol;
			}
		}
		// END API QA
	}
...
static void parseConnectionListArg(ChannelCommand* pCommand, char* argStr, char* appName)
{
	...
	while (pToken)
	{
		...
		if (strstr(pToken, ":"))
		{
			...
			if (indHost == 0)
			{
				pCommand->arr_cInfo[0].rsslConnectOptions.connectionInfo.unified.address = pCommand->hostName[indHost];
				pCommand->arr_cInfo[1].rsslConnectOptions.connectionInfo.unified.address = pCommand->hostName[indHost];
				pCommand->arr_cInfo[2].rsslConnectOptions.connectionInfo.unified.address = pCommand->hostName[indHost];
			}
			// END API QA
			...
			if (indHost == 0)
			{
				pCommand->arr_cInfo[0].rsslConnectOptions.connectionInfo.unified.serviceName = pCommand->port[indHost];
				pCommand->arr_cInfo[1].rsslConnectOptions.connectionInfo.unified.serviceName = pCommand->port[indHost];
				pCommand->arr_cInfo[2].rsslConnectOptions.connectionInfo.unified.serviceName = pCommand->port[indHost];
			}
			// END API QA
...
int main(int argc, char **argv)
{
	...
	for(i = 0; i < channelCommandCount; ++i)
	{
		...
		// API QA
		pInfo->initializationTimeout = 10;
		// END API QA
		...
		for (k = 0; k < 3; ++k)
		{
			RsslReactorConnectInfo* pInfoItem = &pCommand->arr_cInfo[k];

			pInfoItem->rsslConnectOptions.guaranteedOutputBuffers = 500;
			pInfoItem->rsslConnectOptions.majorVersion = RSSL_RWF_MAJOR_VERSION;
			pInfoItem->rsslConnectOptions.minorVersion = RSSL_RWF_MINOR_VERSION;
			pInfoItem->rsslConnectOptions.userSpecPtr = &chanCommands[i];
			pInfoItem->initializationTimeout = 10;
		}

		pOpts->reactorConnectionList = &pCommand->arr_cInfo[0];
		pOpts->connectionCount = (connectionListItems3 == RSSL_TRUE ? 3 : 2);
		// END API QA

		/* Initialize connection list */
		/* When hostsCount is 0 service discovery should be called */
		// API QA Commented out if (pCommand->hostsCount > 0)
		if (pOpts->connectionCount == 0 && pCommand->hostsCount > 0)  // API QA - Don't use with this QATool
		{
	...
	// API QA
	// Run monitor thread
	RSSL_THREAD_START(&monitorThreadId, monitorThread, (void*)NULL);
	// END API QA
...
static void handleRuntime()
{
	...
	if (currentTime >= rsslConsumerRuntime || failTest > 0)  // API QA
	{
...
void cleanUpAndExit(int code)
{
	...
	// API QA
	RSSL_THREAD_JOIN(monitorThreadId);
	// END API QA
