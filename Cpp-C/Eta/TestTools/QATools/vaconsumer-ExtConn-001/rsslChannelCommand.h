...
typedef struct
{
	...
	// API QA
	RsslReactorConnectInfo arr_cInfo[3];
	// END API QA
...
RTR_C_INLINE void initChannelCommand(ChannelCommand *pCommand)
{
	...
	// API QA
	rsslClearReactorConnectInfo(&pCommand->arr_cInfo[0]);
	rsslClearReactorConnectInfo(&pCommand->arr_cInfo[1]);
	rsslClearReactorConnectInfo(&pCommand->arr_cInfo[2]);
	// END API QA
