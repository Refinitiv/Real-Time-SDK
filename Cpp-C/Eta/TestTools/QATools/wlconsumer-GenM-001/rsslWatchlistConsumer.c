 ...
 *|           Copyright (C) 2017-2020,2024-2025 LSEG. All rights reserved.
 *|-----------------------------------------------------------------------------
 */
...
int main(int argc, char **argv)
{
	...
	// ??? Strange setup. At next row we call rsslInitDefaultRDMDirectoryRequest, it cleans serviceId to 0.
	// If really needed serviceId = 1; - we should move this row after rsslInitDefaultRDMDirectoryRequest
	// END API QA
...
RsslRet sendConsumerStatus(RsslReactor* pReactor, RsslReactorChannel* pReactorChannel)
{
	...
	if (canSendDirConsumerStatus)
	{
		canSendDirConsumerStatus = RSSL_FALSE;
		RsslRDMDirectoryConsumerStatus directoryConsStatus;
		RsslRDMConsumerStatusService serviceStatusList[1];
		...
		serviceStatusList[0].serviceId = 1;
		...
		rsslClearRDMDirectoryConsumerStatus(&directoryConsStatus);
		directoryConsStatus.rdmMsgBase.streamId = 2;
		directoryConsStatus.rdmMsgBase.domainType = RSSL_DMT_SOURCE;
		directoryConsStatus.rdmMsgBase.rdmMsgType = RDM_DR_MT_CONSUMER_STATUS;
		directoryConsStatus.consumerServiceStatusCount = serviceStatusCount;
		directoryConsStatus.consumerServiceStatusList = serviceStatusList;

		rsslClearReactorSubmitMsgOptions(&submitMsgOpts);
		submitMsgOpts.pRDMMsg = (RsslRDMMsg*)&directoryConsStatus;
		if ((ret = rsslReactorSubmitMsg(pReactor, pReactorChannel, &submitMsgOpts,
			&rsslErrorInfo)) != RSSL_RET_SUCCESS)
		{
			printf("rsslReactorSubmitMsg() failed for sending RsslRDMConsumerStatus on dir stream: %d(%s)\n",
				ret, rsslErrorInfo.rsslError.text);
			return ret;
		}
	}
...
//APIQA  sending RsslRdmLoginConsumerConnectionStatus msg
RsslRet sendLoginConsumerConnStatus(RsslReactor* pReactor, RsslReactorChannel* pReactorChannel)
{
	...
	{
		//canSendLoginConsumerConnStatus = RSSL_TRUE;
		...
		loginConsConnStatus.flags = RDM_LG_CCSF_HAS_WARM_STANDBY_INFO;
		rsslClearRDMLoginWarmStandbyInfo(&loginConsConnStatus.warmStandbyInfo);
		loginConsConnStatus.warmStandbyInfo.action = RSSL_MPEA_ADD_ENTRY;
		loginConsConnStatus.warmStandbyInfo.warmStandbyMode = RDM_LOGIN_SERVER_TYPE_STANDBY;

		rsslClearReactorSubmitMsgOptions(&submitMsgOpts);
		submitMsgOpts.pRDMMsg = (RsslRDMMsg*)&loginConsConnStatus;
		if ((ret = rsslReactorSubmitMsg(pReactor, pReactorChannel, &submitMsgOpts,
			&rsslErrorInfo)) != RSSL_RET_SUCCESS)
		{
			printf("rsslReactorSubmitMsg() failed for sending RsslRDMLoginConsumerConnectionStatus on login stream: %d(%s)\n",
				ret, rsslErrorInfo.rsslError.text);
			return ret;
		}
	}
