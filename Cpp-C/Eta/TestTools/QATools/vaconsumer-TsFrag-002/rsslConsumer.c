...
{
	// APIQA: Added msgSize	
	printf("Usage: %s or\n%s  [-tcp [<hostname>:<port> <service name>]] [<domain>:<item name>,...] ] [-uname <LoginUsername>] [-view] [-post] [-offpost] [-snapshot] [-runtime <seconds>] [-cache] [-cacheInterval <seconds>] [ -msgSize <Message Size> ] [-tunnel] [-tsDomain <number> ] [-tsAuth] [-tsServiceName] [-x] [-runtime]\n"
	// END APIQA
			...
			// APIQA: Added msgSize	
			"\n -msgSize size of message to use to send a tunnel stream message\n"
			// END APIQA
...
// APIQA
			else if (strcmp("-msgSize", argv[i]) == 0)
			{
				i += 2; if (i > argc) printUsageAndExit(argv[0]);
				iMsgSizeConsumer = atoi(argv[i-1]);
			}
			// END APIQA
	...
	// APIQA:  setting message size here 
	setMsgSize(iMsgSizeConsumer);
	// END APIQA: 
