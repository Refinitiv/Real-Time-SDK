...
* @param cpuString Specifies the Cpu core in string format (Cpu core id or P:X C:Y T:Z format). P(Package) C(Core) T(Thread).
 ...
 * Maximum number of logical CPU ID under Linux is 1024 and under Windows is 64.
 * @param cpuId the logical index of Cpu core.
...
/**
 * @brief Converts a physical core in PCT format or list of cores to a logical CPU Id or list Ids.
 * Maximum number of logical CPU ID under Linux is 1024 and under Windows is 64.
 * @param cpuString Specifies the Cpu core in string format (Cpu core id or P:X C:Y T:Z format). P(Package) C(Core) T(Thread).
 * @param pLogicalIds on Success, the list of logical core id; on Fail, an empty string.
 * @param pError Error structure to be populated in the event of failure.
 * @return RSSL_RET_SUCCESS if the maping convertion was successful.
 * @return RSSL_RET_FAILURE when an error received, specific description of binding error see in pError.
 */
