 ...
 *|           Copyright (C) 2015,2020,2022,2024-2026 LSEG. All rights reserved.
 *|-----------------------------------------------------------------------------
 */

/*
 * This is the market by order handler for the rsslConsumer application.
 ...
 * a map containing field list contents, decoding field entries from a 
 * response, closing market by order streams, getting the next stream id,
 * and adding/removing items to/from the item list are also provided.
 */
...
/* non-private stream market by order item info list */
static MarketByOrderItemInfo marketByOrderItemInfoList[MAX_STREAM_ID_RANGE_PER_DOMAIN/2];

/* private stream market by order item info list */
static MarketByOrderItemInfo marketByOrderPSItemInfoList[MAX_STREAM_ID_RANGE_PER_DOMAIN/2];

/* next non-private stream id for item requests */
static RsslInt32 nextStreamId = MARKET_BY_ORDER_STREAM_ID_START;

/* next private stream id for item requests */
static RsslInt32 nextPrivateStreamId = MARKET_BY_ORDER_PRIVATE_STREAM_ID_START;
...
/*
 * Publically visable method used to add a market by order name to the request list
...
void addMarketByOrderItemName(char* itemname, RsslBool isPrivateStream)
{
	...
	if (!isPrivateStream) /* non-private stream */
	{
		...
		{
			snprintf(marketByOrderItemInfoList[nextStreamId - MARKET_BY_ORDER_STREAM_ID_START].itemname, 128, "%s", itemname);
			marketByOrderItemInfoList[nextStreamId - MARKET_BY_ORDER_STREAM_ID_START].nameLength = (RsslUInt32)strlen(itemname);
			marketByOrderItemInfoList[nextStreamId - MARKET_BY_ORDER_STREAM_ID_START].streamId = 0;
			++nextStreamId;
			++itemCount;
		}
		else
	...
	else /* private stream */
	{
		...
		{
			snprintf(marketByOrderPSItemInfoList[nextPrivateStreamId - MARKET_BY_ORDER_PRIVATE_STREAM_ID_START].itemname, 128, "%s", itemname);
			marketByOrderPSItemInfoList[nextPrivateStreamId - MARKET_BY_ORDER_PRIVATE_STREAM_ID_START].nameLength = (RsslUInt32)strlen(itemname);
			marketByOrderPSItemInfoList[nextPrivateStreamId - MARKET_BY_ORDER_PRIVATE_STREAM_ID_START].streamId = 0;
			++nextPrivateStreamId;
 ...
 * isPrivateStream - Flag for private stream
 */
static void removeMarketByOrderItemEntry(RsslChannel *chnl, RsslInt32 streamId, RsslBool isPrivateStream)
{
	int i;
	...
	if (!isPrivateStream) /* non-private stream */
	{
		listIndex = streamId - MARKET_BY_ORDER_STREAM_ID_START;
		for (i = listIndex; i < itemCount - 1; i++)
		{
			strncpy(marketByOrderItemInfoList[i].itemname, marketByOrderItemInfoList[i+1].itemname, 129);
			marketByOrderItemInfoList[i].nameLength = marketByOrderItemInfoList[i+1].nameLength;
			marketByOrderItemInfoList[i].itemState = marketByOrderItemInfoList[i+1].itemState;
			marketByOrderItemInfoList[i].streamId = marketByOrderItemInfoList[i+1].streamId;
		}
		--nextStreamId;
		--itemCount;
	}
	...
	{
		for (i = streamId - MARKET_BY_ORDER_PRIVATE_STREAM_ID_START; i < privateStreamItemCount - 1; i++)
		{
			strncpy(marketByOrderPSItemInfoList[i].itemname, marketByOrderPSItemInfoList[i+1].itemname, 129);
			marketByOrderPSItemInfoList[i].nameLength = marketByOrderPSItemInfoList[i+1].nameLength;
			marketByOrderPSItemInfoList[i].itemState = marketByOrderPSItemInfoList[i+1].itemState;
			marketByOrderPSItemInfoList[i].streamId = marketByOrderPSItemInfoList[i+1].streamId;
		}
		--nextPrivateStreamId;
		--privateStreamItemCount;
	}
 ...
 */
static RsslRet closeMBOItemStream(RsslChannel* chnl, RsslInt32 streamId)
{
	RsslError error;
	RsslBuffer* msgBuf = 0;
...
static RsslRet encodeItemRequest(RsslChannel* chnl, RsslBuffer* msgBuf, RsslInt32 streamId, RsslBool isPrivateStream)
{
	...
	{
		listIndex = streamId - MARKET_BY_ORDER_STREAM_ID_START;
		msg.msgBase.msgKey.name.data = marketByOrderItemInfoList[listIndex].itemname;
		msg.msgBase.msgKey.name.length = marketByOrderItemInfoList[listIndex].nameLength;
	}
	else /* private stream */
	{
		listIndex = streamId - MARKET_BY_ORDER_PRIVATE_STREAM_ID_START;
		msg.msgBase.msgKey.name.data = marketByOrderPSItemInfoList[listIndex].itemname;
		msg.msgBase.msgKey.name.length = marketByOrderPSItemInfoList[listIndex].nameLength;
	}
	...
	if (!isPrivateStream) 
		marketByOrderItemInfoList[listIndex].streamId = streamId;
	else
		marketByOrderPSItemInfoList[listIndex].streamId = streamId;
...
static RsslRet encodeBatchItemRequest(RsslChannel* chnl, RsslBuffer* msgBuf, RsslInt32 streamId, RsslBool isPrivateStream)
{
	...
	if (!isPrivateStream) /* non-private stream */
	{
		...
		{
			itemName.data = marketByOrderItemInfoList[i].itemname;
			itemName.length = (RsslUInt32)strlen(marketByOrderItemInfoList[i].itemname);
			marketByOrderItemInfoList[i].streamId = ++streamId;
	...
	else /* private stream */
	{
		...
		{
			itemName.data = marketByOrderPSItemInfoList[i].itemname;
			itemName.length = (RsslUInt32)strlen(marketByOrderPSItemInfoList[i].itemname);
			marketByOrderPSItemInfoList[i].streamId = ++streamId;
			if((ret = rsslEncodeArrayEntry(&encodeIter, &itemName, 0)) < RSSL_RET_SUCCESS)
			{
				printf("rsslEncodeArrayEntry() failed with return code: %d\n", ret);
				return ret;
...
static RsslRet encodeBatchItemClose(RsslChannel* chnl, RsslBuffer* msgBuf, RsslInt32 batchCloseStreamId)
{
	...
	for(i = 0; i < itemCount; i++)
	{
		...
		if (!rsslIsFinalState(&marketByOrderItemInfoList[i].itemState))
		{
	...
	for(i = 0; i < privateStreamItemCount; i++)
	{
		...
		/* we only want to close a stream if it was not already closed (e.g. rejected by provider, closed via refresh or status, or redirected) */
		if (!rsslIsFinalState(&marketByOrderPSItemInfoList[i].itemState))
		{
			if((ret = rsslEncodeArrayEntry(&encodeIter, 0, (void*)(&itemStreamId))) < RSSL_RET_SUCCESS)
			{
				printf("rsslEncodeArrayEntry() failed with return code: %d\n", ret);
...
RsslRet sendMarketByOrderItemRequests(RsslChannel* chnl)
{
	...
	/* check to see if the provider supports the market by order domain */
	if(getSourceDirectoryCapabilities(RSSL_DMT_MARKET_BY_ORDER) == RSSL_FALSE)
	{
	...
	for (i = 0; i < (MAX_STREAM_ID_RANGE_PER_DOMAIN/2) ; i++)
	{
		...
		marketByOrderItemInfoList[i].itemState.dataState = RSSL_DATA_NO_CHANGE;
		marketByOrderItemInfoList[i].itemState.streamState = RSSL_STREAM_UNSPECIFIED;
	}
	...
	for (i = 0; i < (MAX_STREAM_ID_RANGE_PER_DOMAIN/2) ; i++)
	{
		...
		marketByOrderPSItemInfoList[i].itemState.dataState = RSSL_DATA_NO_CHANGE;
		marketByOrderPSItemInfoList[i].itemState.streamState = RSSL_STREAM_UNSPECIFIED;
	}
	...
	if (itemCount > 0)
	{
		...
		if((loginInfo->SupportBatchRequests & RDM_LOGIN_BATCH_SUPPORT_REQUESTS) && (itemCount > 1))
		{
			...
			if (msgBuf != NULL)
			{
				if(encodeBatchItemRequest(chnl, msgBuf, MARKET_BY_ORDER_BATCH_STREAM_ID_START, RSSL_FALSE) != RSSL_RET_SUCCESS)
				{
					rsslReleaseBuffer(msgBuf, &error); 
		...
		else 
		{
			...
			for (i = 0; i < itemCount; i++)
			{
				...
				{
					/* encode item request */
					if (encodeItemRequest(chnl, msgBuf, (i + MARKET_BY_ORDER_STREAM_ID_START), RSSL_FALSE) != RSSL_RET_SUCCESS)
					{
						rsslReleaseBuffer(msgBuf, &error); 
	...
	if (privateStreamItemCount > 0)
	{
		...
		if((loginInfo->SupportBatchRequests & RDM_LOGIN_BATCH_SUPPORT_REQUESTS) && (privateStreamItemCount > 1))
		{
			...
			if (msgBuf != NULL)
			{
				if(encodeBatchItemRequest(chnl, msgBuf, MARKET_BY_ORDER_BATCH_PRIVATE_STREAM_ID_START, RSSL_TRUE) != RSSL_RET_SUCCESS)
				{
					rsslReleaseBuffer(msgBuf, &error); 
					printf("\nMarket By Order encodeBatchItemRequest() failed\n");
					return RSSL_RET_FAILURE;
		...
		else 
		{
			...
			for (i = 0; i < privateStreamItemCount; i++)
			{
				...
				{
					/* encode item request */
					if (encodeItemRequest(chnl, msgBuf, (i + MARKET_BY_ORDER_PRIVATE_STREAM_ID_START), RSSL_TRUE) != RSSL_RET_SUCCESS)
					{
						rsslReleaseBuffer(msgBuf, &error); 
						printf("\nMarket By Order encodeItemRequest() failed\n");
						return RSSL_RET_FAILURE;
...
RsslRet processMarketByOrderResponse(RsslChannel* chnl, RsslMsg* msg, RsslDecodeIterator* dIter)
{
	...
	MarketByOrderItemInfo *itemInfo = NULL;
	...
	if (msg->msgBase.streamId == MARKET_BY_ORDER_BATCH_STREAM_ID_START ||
		msg->msgBase.streamId == MARKET_BY_ORDER_BATCH_PRIVATE_STREAM_ID_START)
	{
		isBatchRequest = RSSL_TRUE;	// we dont have info list entries for a status response to a batch request
	}
	else if (msg->msgBase.streamId >= MARKET_BY_ORDER_PRIVATE_STREAM_ID_START)
	{
		isPrivateStream = RSSL_TRUE;
		itemInfo = &marketByOrderPSItemInfoList[msg->msgBase.streamId - MARKET_BY_ORDER_PRIVATE_STREAM_ID_START];
	}
	else if (msg->msgBase.streamId >= MARKET_BY_ORDER_STREAM_ID_START)
	{
		itemInfo = &marketByOrderItemInfoList[msg->msgBase.streamId - MARKET_BY_ORDER_STREAM_ID_START];
	}
	else
	{
		printf("\nReceived not valid stream id %i  - closing stream\n", msg->msgBase.streamId);
		return RSSL_RET_FAILURE;
	}
	...
	switch(msg->msgBase.msgClass)
	{
		case RSSL_MC_REFRESH:
			...
			if (!(msg->refreshMsg.flags & RSSL_RFMF_PRIVATE_STREAM)) /* non-private stream */
			{
				...
				if (isPrivateStream)
				{
					...
					/* close stream */
					if (closeMBOItemStream(chnl, itemInfo->streamId) != RSSL_RET_SUCCESS)
						return RSSL_RET_FAILURE;
				
					/* remove private stream entry from list */
					removeMarketByOrderItemEntry(chnl, itemInfo->streamId, RSSL_TRUE);
					return RSSL_RET_SUCCESS;
				}
			}
		...
		case RSSL_MC_STATUS:
			...
			if (msg->statusMsg.flags & RSSL_STMF_HAS_STATE)
    		{
				...
				{
    				if (redirectToMarketByOrderPrivateStream(chnl, itemInfo->streamId) != RSSL_RET_SUCCESS)
						return RSSL_RET_FAILURE;
				}
			}	
			break;
...
/*
 * Publically visable Market By Order close function
 *
 * Close all market by order item streams.
 * chnl - The channel to send an item close to
 */
RsslRet closeMarketByOrderItemStreams(RsslChannel* chnl)
{
	...
	else
	{
		...
		for (i = 0; i < itemCount; i++)
		{
			...
			if (!rsslIsFinalState(&marketByOrderItemInfoList[i].itemState))
			{
				if (closeMBOItemStream(chnl, i + MARKET_BY_ORDER_STREAM_ID_START) != RSSL_RET_SUCCESS)
					return RSSL_RET_FAILURE;
			}
		}
		...
		for (i = 0; i < privateStreamItemCount; i++)
		{
			...
			if (!rsslIsFinalState(&marketByOrderPSItemInfoList[i].itemState))
			{
				if (closeMBOItemStream(chnl, i + MARKET_BY_ORDER_PRIVATE_STREAM_ID_START) != RSSL_RET_SUCCESS)
					return RSSL_RET_FAILURE;
			}
		}
	}
...
RsslRet redirectToMarketByOrderPrivateStream(RsslChannel* chnl, RsslInt32 streamId)
{
	...
	addMarketByOrderItemName(marketByOrderItemInfoList[streamId - MARKET_BY_ORDER_STREAM_ID_START].itemname, RSSL_TRUE);

	/* remove non-private stream entry from list */
	removeMarketByOrderItemEntry(chnl, streamId, RSSL_FALSE);
