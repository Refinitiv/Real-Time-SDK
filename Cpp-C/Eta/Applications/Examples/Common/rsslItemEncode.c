 ...
 * the different data domains (e.g. market price, market by order, symbol list, 
 * market by price and yield curve).
 */
 ...
 * msgBuf - The message buffer to encode the item request close status into
 * streamId - The stream id of the item request close status
 */
 ...
 * msgBuf - The message buffer to encode the item request reject into
 */
RsslRet encodeItemRequestReject(RsslChannel* chnl, RsslInt32 streamId, RsslItemRejectReason reason, RsslBuffer* msgBuf, RsslUInt8 domainType, RsslBool isPrivateStream)
{
	RsslRet ret = 0;
	RsslStatusMsg msg = RSSL_INIT_STATUS_MSG;
	...
	case KEY_ENC_ATTRIB_NOT_SUPPORTED:
		msg.state.code = RSSL_SC_USAGE_ERROR;
		msg.state.streamState = RSSL_STREAM_CLOSED;
		snprintf(stateText, 128, "Item request rejected for stream id %d - this provider does not support key attribute information", streamId);
		msg.state.text.data = stateText;
		msg.state.text.length = (RsslUInt32)strlen(stateText) + 1;
		break;
	case PRIVATE_STREAM_REDIRECT:
		...
		snprintf(stateText, 128, "Standard stream redirect to private for stream id %d - this item must be requested via private stream", streamId);
		msg.state.text.data = stateText;
		msg.state.text.length = (RsslUInt32)strlen(stateText) + 1;
		break;
	case PRIVATE_STREAM_MISMATCH:
		...
		msg.state.code = RSSL_SC_USAGE_ERROR;
		msg.state.streamState = RSSL_STREAM_CLOSED;
		snprintf(stateText, 128, "Rejected reissued request for stream id %d - private stream flag did not match previous request", streamId);
		msg.state.text.data = stateText;
		msg.state.text.length = (RsslUInt32)strlen(stateText) + 1;
		break;
	case BATCH_ITEM_REISSUE:
		...
		msg.state.code = RSSL_SC_USAGE_ERROR;
		msg.state.streamState = RSSL_STREAM_CLOSED;
		snprintf(stateText, 128, "Rejected request for stream id %d - reissue via batch request is not allowed", streamId);
		msg.state.text.data = stateText;
		msg.state.text.length = (RsslUInt32)strlen(stateText) + 1;
		break;
	case ITEM_NOT_SUPPORTED:
		msg.state.streamState = RSSL_STREAM_CLOSED;
		msg.state.code = RSSL_SC_USAGE_ERROR;
		snprintf(stateText, 128, "Item request rejected for stream id %d - item not supported", streamId);
		msg.state.text.data = stateText;
		msg.state.text.length = (RsslUInt32)strlen(stateText) + 1;
		break;
	default:
 ...
 *
 * chnl - The channel to send close status for the orginal batch request or batch close stream to
 ...
 *
 * chnl - The channel to send ack message to
 * ackBuf - The message buffer to encode the ack message into
