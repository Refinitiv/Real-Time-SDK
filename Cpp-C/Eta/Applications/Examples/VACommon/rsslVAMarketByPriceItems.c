...
/* 
 * This handles the generation of VA example market by price data, and shows
...
void initMarketByPriceItemFields(RsslMarketByPriceItem* mbpItem)
{
	...
	for(i = 0; i < MBP_MAX_ORDERS; ++i)
	{
		...
		strcat(order->PRICE_POINT, (i >= MBP_MAX_ORDERS / 2) ? "a" : "b");

		order->ORDER_SIZE.hint = RSSL_RH_EXPONENT0;
		order->ORDER_SIZE.value = 5+i;
		order->ORDER_SIZE.isBlank = RSSL_FALSE;

		order->ORDER_SIDE = (i >= MBP_MAX_ORDERS / 2) ? ORDER_ASK : ORDER_BID;
...
/*
 * Build and encode the message (without payload) for market by price refresh.
 * itemInfo - The item information
 * encodeIter - iterator for encoding the message part
 ...
 * isPrivateStream - Flag indicating if this is a private stream
 * serviceId - The service id of the market by price refresh
 * multiPartNo - The part number of this message for a multi-part refresh
 */
RsslRet encodeMarketByPriceRefreshMsgInit(RsslItemInfo* itemInfo, RsslEncodeIterator *encodeIter, RsslBool isSolicited, RsslInt32 streamId, RsslBool isStreaming, RsslBool isPrivateStream, RsslUInt16 serviceId, int multiPartNo, const char* stateTextStr)
{
	...
	if (isSolicited)
	{
		refreshMsg.flags = RSSL_RFMF_HAS_MSG_KEY | RSSL_RFMF_SOLICITED | RSSL_RFMF_HAS_QOS;
	}
	else
	{
	...
	/* This message initialization method does not set the RSSL_RFMF_REFRESH_COMPLETE
	 * since the application may not know which part is final when this is called.
	 *  The refresh complete flag can be set after encoding is complete.
	 */
	sprintf(stateText, "%s %d", stateTextStr, multiPartNo);
...
/*
 * Encodes the market by price refresh map payload.
 * itemInfo - The item information
 * encodeIter - iterator for encoding the MBP map
...
RsslRet encodeMarketByPriceRefreshMap(RsslItemInfo* itemInfo, RsslEncodeIterator *encodeIter, RsslBool isPrivateStream, RsslDataDictionary* dictionary, int multiPartNo)
{
	...
	map.totalCountHint = MBP_MAX_ORDERS;
...
/*
 * Complete the Market By Price response message encoding
 */
 ...
 * pReactorChannel - The channel to send a market by price refresh to
 * itemInfo - The item information
 * isSolicited - The refresh is solicited if set
...
RsslRet encodeMarketByPriceRefresh(RsslReactorChannel* pReactorChannel, RsslItemInfo* itemInfo, RsslBuffer* msgBuf, RsslBool isSolicited, RsslInt32 streamId, RsslBool isStreaming, RsslBool isPrivateStream, RsslUInt16 serviceId, RsslDataDictionary* dictionary, int multiPartNo)
{
	...
	{
		printf("rsslSetEncodeIteratorBuffer() failed with return code: %d", ret);
		return ret;
	}
	rsslSetEncodeIteratorRWFVersion(&encodeIter, pReactorChannel->majorVersion, pReactorChannel->minorVersion);

	ret = encodeMarketByPriceRefreshMsgInit(itemInfo, &encodeIter, isSolicited, streamId, isStreaming, isPrivateStream, serviceId, multiPartNo, "Refresh Part");
	if (ret < RSSL_RET_SUCCESS)
	{
		printf("encodeMarketByPriceRefreshMsgInit() failed with return code: %d\n", ret);
		return ret;
	}

	ret = encodeMarketByPriceRefreshMap(itemInfo, &encodeIter, isPrivateStream, dictionary, multiPartNo);
	if (ret < RSSL_RET_SUCCESS)
	{
		printf("encodeMarketByPriceRefreshMap() failed with return code: %d\n", ret);
		return ret;
	}
 ...
 * encoding succeeds or failure if encoding fails.
 * pReactorChannel - The channel to send a market by price update to
 * itemInfo - The item information
