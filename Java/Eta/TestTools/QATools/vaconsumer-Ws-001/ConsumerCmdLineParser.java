...
class ConsumerCmdLineParser implements CommandLineParser
{
	...
	private int compressType = CompressionTypes.ZLIB;
	...
	public boolean parseArgs(String[] args)
	{
		try
		{
			...
			while (argsCount < args.length)
			{
				...
				//API QA
				else if ("-compressionType".equals(args[argsCount]))
				{
					// will overwrite connectionArgsParser's connectionList's connectionType based on the flag
					String compressionType = args[++argsCount];
					++argsCount;
					...
					{
						compressType = CompressionTypes.LZ4;
					}
				}
				//END API QA
	...
	public void printUsage()
	{
		System.out.println("Usage: Consumer or\nConsumer [-c <hostname>:<port> <service name> <domain>:<item name>,...] [-bc <hostname>:<port>] [-uname <LoginUsername>] [-view] [-post] [-offpost] [-snapshot] [-runtime <seconds>]" +
						   ...
						   "\n -clientId specifies a unique ID for application making the request to LDP token service, also known as AppKey generated using an AppGenerator.\n" +
						   ...
						   "\n -compressionType valid is ZLib, LZ4.\n" );
	}
}
