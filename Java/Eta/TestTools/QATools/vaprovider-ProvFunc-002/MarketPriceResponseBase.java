...
public abstract class MarketPriceResponseBase extends MsgBaseImpl
{
    ...
    public int encode(EncodeIterator encodeIter)
    {
        ...
        // APIQA:  Adding negative FIDs into market price response
        // Negative FID -32768
        fieldEntry.clear();
        dictionaryEntry = dictionary.entry(-32768);
        if (dictionaryEntry != null)
        {
            fieldEntry.fieldId(-32768);
            fieldEntry.dataType(dictionaryEntry.rwfType());
            tempReal.clear();
            tempReal.value(mpItem.ASK, RealHints.EXPONENT_2);
            ret = fieldEntry.encode(encodeIter, tempReal);
        ...
        // Negative FID -32767
        fieldEntry.clear();
        dictionaryEntry = dictionary.entry(-32767);
        if (dictionaryEntry != null)
        {
            fieldEntry.fieldId(-32767);
            fieldEntry.dataType(dictionaryEntry.rwfType());
            tempReal.clear();
            tempReal.value(mpItem.ASK, RealHints.EXPONENT_2);
            ret = fieldEntry.encode(encodeIter, tempReal);
        ...
        // Negative FID -100
        fieldEntry.clear();
        dictionaryEntry = dictionary.entry(-100);
        if (dictionaryEntry != null)
        {
