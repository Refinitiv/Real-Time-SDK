...
public class ConsPerfConfig
{
	...
	private static final String DEFAULT_PORT = "14002";
	...
	private String _proxyHostName;			/* Address or hostname of the HTTP proxy server */
	private String _proxyPort;				/* Port Number of the HTTP proxy server. */
	private boolean _proxy;					/* Proxy. */
	private String _location;				/* Location endpoint */

	private boolean _sessionMgnt;			/* (optional) Enable Session Management in the reactor. */
	...
	public void init (String[] args, int maxThreads)
	{
		...
		_clientId = CommandLine.value("clientId");
		_password = CommandLine.value("password");
		_clientSecret = CommandLine.value("clientSecret");
		_proxyHostName = CommandLine.value("ph");
		_proxyPort = CommandLine.value("pp");
		_proxy = CommandLine.booleanValue("proxy");
		_sessionMgnt = CommandLine.booleanValue("sessionMgnt");
		_location = CommandLine.value("location");
        try
        {
		...
		if (_useReactor && _username.length() == 0)
		{
			System.err.println("Config Error: Username must be provided");
			System.out.println(CommandLine.optionHelpString());
			System.exit(-1);
		}

		if (_useReactor && _clientId.length() == 0)
		{
			System.err.println("Config Error: clientId must be provided");
			System.out.println(CommandLine.optionHelpString());
			System.exit(-1);
		}

		if (_useReactor && _password.length() == 0)
		{
			System.err.println("Config Error: Password must be provided");
			System.out.println(CommandLine.optionHelpString());
			System.exit(-1);
		}

		if ( _sessionMgnt && !_useReactor && !_useWatchlist)
		{
			System.err.println("Config Error: The -reactor or -watchlist must be set in order to work with -sessionMgnt");
			System.out.println(CommandLine.optionHelpString());
			System.exit(-1);
		}

		if (_sessionMgnt && _portNo.equals(DEFAULT_PORT) && _hostName.equals(DEFAULT_HOST))
		{
			_portNo = "";
			_hostName = "";
		}
		...
		if (_proxy)
		{
			if (_proxyHostName.length() == 0) {
				System.err.println("Config Error: Proxy hostname not provided.\"");
				System.out.println(CommandLine.optionHelpString());
				System.exit(-1);
			}

			if (_proxyPort.length() == 0) {
				System.err.println("Config Error: Proxy port not provided.\"");
				System.out.println(CommandLine.optionHelpString());
				System.exit(-1);
			}
	...
	private void createConfigString()
	{
		...
		_configString = "--- TEST INPUTS ---\n\n" +
            ...
            "                   Hostname: " + _hostName + "\n" +
            "                       Port: " + _portNo + "\n" +
			"                   ClientId: " + (!_clientId.isEmpty() ? "<removed from displaying>" : "null") + "\n" +
			"                   Password: " + (!_password.isEmpty() ? "<removed from displaying>" : "null") + "\n" +
			"   	        ClientSecret: " + (!_clientSecret.isEmpty() ? "<removed from displaying>" : "unspecified") + "\n" +
			"            Proxy Host Name: " + _proxyHostName + "\n" +
	...
	/**
	 *  Client Id used when logging in.
	 *
	 * @return the string
	 */
	...
	/**
	 *  Client secret used when logging in.
	 *
	 * @return the string
	 */
	...
	/**
	 *  Address or hostname of the HTTP proxy server.
	 *
	 * @return the string.
	 */
