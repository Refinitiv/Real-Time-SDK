												...
												ReactorJsonConversionEventCallback, ReactorServiceNameToIdCallback
{
    ...
    private ReactorOAuthCredential _oAuthCredential = ReactorFactory.createReactorOAuthCredential();
    ...
    private void connect()
    {
        ...
        connectOptions.userSpecObject(this);
        ...
        else // use ETA VA Reactor for sending and receiving
        {
            ...
            _connectInfo.enableSessionManagement(_consPerfConfig.sessionMgnt());

            if (!_consPerfConfig.location().isEmpty()) {
                _connectInfo.location(_consPerfConfig.location());
            }
            ...
            if (_consPerfConfig.username() != null && !_consPerfConfig.username().isEmpty())
            {
                _role.rdmLoginRequest().userName().data(_consPerfConfig.username());
            }

            if (_consPerfConfig.password() != null && !_consPerfConfig.password().isEmpty())
            {
                _role.rdmLoginRequest().password().data(_consPerfConfig.password());
                _role.rdmLoginRequest().applyHasPassword();
                _oAuthCredential.password().data(_consPerfConfig.password());
            }

            if (_consPerfConfig.clientId() != null && !_consPerfConfig.clientId().isEmpty())
            {
                _oAuthCredential.clientId().data(_consPerfConfig.clientId());
                _oAuthCredential.takeExclusiveSignOnControl(true);
            }

            if (_consPerfConfig.clientSecret() != null && !_consPerfConfig.clientSecret().isEmpty())
            {
                _oAuthCredential.clientSecret().data(_consPerfConfig.clientSecret());
            }
            ...
            _role.reactorOAuthCredential(_oAuthCredential);
            _role.rdmLoginRequest().attrib().applyHasSingleOpen();
            _role.rdmLoginRequest().attrib().singleOpen(1);
            _role.rdmLoginRequest().attrib().applyHasAllowSuspectData();
            _role.rdmLoginRequest().attrib().allowSuspectData(1);

            _connectOptions.reconnectAttemptLimit(-1);
            _connectOptions.reconnectMaxDelay(5000);
            _connectOptions.reconnectMinDelay(1000);

            if (_consPerfConfig.enableProxy()) {
                connectOptions.tunnelingInfo().HTTPproxy(true);
                connectOptions.tunnelingInfo().HTTPproxyHostName(_consPerfConfig.proxyHostName());
                try {
                    connectOptions.tunnelingInfo().HTTPproxyPort(Integer.parseInt(_consPerfConfig.proxyPort()));
                }
                catch (Exception e) {
	...
	private int writeMsg(Msg msg)
	{
        ...
        {
            if (_reactorChannel.state() == ReactorChannel.State.READY)
            {
	...
	{
		ConsumerThread consumerThread = (ConsumerThread)serviceNameToIdEvent.reactorChannel().userSpecObj();
