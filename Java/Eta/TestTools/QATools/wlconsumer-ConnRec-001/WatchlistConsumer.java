...
public class WatchlistConsumer implements ConsumerCallback
{
	...
	private void initChannelInfo(ChannelInfo chnlInfo)
	{
        ...
        // APIQA
        chnlInfo.connectOptions.reconnectAttemptLimit(CommandLine.intValue("attempLimit")); // attempt to recover forever
        chnlInfo.connectOptions.reconnectMinDelay(CommandLine.intValue("minDelay")); // minDelay second minimum
        chnlInfo.connectOptions.reconnectMaxDelay(CommandLine.intValue("maxDelay")); // maxDalay second maximum
		for( int i = 0; i < CommandLine.intValue("numConnections"); i++ )
		{

			if (i == 0)
			{
				chnlInfo.connectOptions.connectionList().get(i).connectOptions().unifiedNetworkInfo().serviceName(chnlInfo.connectionArg.port());
				chnlInfo.connectOptions.connectionList().get(i).connectOptions().unifiedNetworkInfo().address(chnlInfo.connectionArg.hostname());
			} 
			if (i == 1)
			{
				...
				chnlInfo.connectOptions.connectionList().get(i).connectOptions().unifiedNetworkInfo().serviceName(CommandLine.value("p2"));
				chnlInfo.connectOptions.connectionList().get(i).connectOptions().unifiedNetworkInfo().address(CommandLine.value("h2"));
			}
				if (i == 2)
			{
				ReactorConnectInfo connectInfo1 = ReactorFactory.createReactorConnectInfo();
				chnlInfo.connectOptions.connectionList().add(connectInfo1);
				chnlInfo.connectOptions.connectionList().get(i).connectOptions().unifiedNetworkInfo().serviceName(CommandLine.value("p3"));
				chnlInfo.connectOptions.connectionList().get(i).connectOptions().unifiedNetworkInfo().address(CommandLine.value("h3"));
			} 
			chnlInfo.connectOptions.connectionList().get(i).connectOptions().majorVersion(Codec.majorVersion());
			chnlInfo.connectOptions.connectionList().get(i).connectOptions().minorVersion(Codec.minorVersion());
			chnlInfo.connectOptions.connectionList().get(i).connectOptions().connectionType(chnlInfo.connectionArg.connectionType());
			chnlInfo.connectOptions.connectionList().get(i).connectOptions().unifiedNetworkInfo().interfaceName(chnlInfo.connectionArg.interfaceName());
			chnlInfo.connectOptions.connectionList().get(i).connectOptions().userSpecObject(chnlInfo);
			chnlInfo.connectOptions.connectionList().get(i).connectOptions().guaranteedOutputBuffers(1000);
		}
			// END APIQA
