...
public class WatchlistConsumer implements ConsumerCallback
{
    ...
    int reissuItem(Reactor reactor, ReactorChannel channel, int userStreamIndex, boolean pauseFlag)
    {
    ...
    public int defaultMsgCallback(ReactorMsgEvent event)
    {
        ...
        switch (msg.msgClass())
        {
            ...
            case MsgClasses.UPDATE:
                ...
                eventReceived++;
                if (eventReceived == 5)
                    reissuItem(reactor, event.reactorChannel(), 0, true);
                if (eventReceived == 10)
                    reissuItem(reactor, event.reactorChannel(), 1, true);
                if (eventReceived == 25)
                    reissuItem(reactor, event.reactorChannel(), 0, false);
                if (eventReceived == 30)
                    reissuItem(reactor, event.reactorChannel(), 1, false);
                // END APIQA
                break;
