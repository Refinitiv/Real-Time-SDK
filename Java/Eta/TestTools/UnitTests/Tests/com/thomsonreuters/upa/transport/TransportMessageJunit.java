...
public class TransportMessageJunit
{
                    ...
                    if (keySet != null)
                    {
                        ...
                        while (iter.hasNext())
                        {
                            ...
                            if (key.isAcceptable())
                            {
                                if (DEBUG)
                                    System.out
                                            .println("UpajServer: processSelector() accepting a connection");
                            ...
                            if (key.isReadable())
                            {
                                ...
                                if (DEBUG)
                                    System.out
                                            .println("UpajServer: processSelector() channel is readable, channelState="
                                                    + channel.state());
    ...
    class UpajClient implements Runnable
    {
         ...
         * .
         */
        public UpajClient(int id, int priority, 
        ...
        private int writePackedMessages(PackedTestArgs args)
        {
            ...
            do
            {
                ...
                if(!_running)
                {
                    System.out.println("UpajClient id=" + _id + ": writePackedMessages(): write() retVal=" + retVal + " while run time expired.");
                    try
                    {
                        if (DEBUG)
                            Thread.sleep(250);
            ...
            if (retVal >= TransportReturnCodes.SUCCESS)
            {
                ...
                if (DEBUG)
                    System.out.println("UpajClient id=" + _id + ": writePackedMessages(): _bytesWritten="
        ...
        private int writeMessage(ByteBuffer dataBuf)
        {
            ...
            do
            {
                ...
                if(!_running)
                {
                    System.out.println("UpajClient id=" + _id + ": writeMessage(): write() retVal=" + retVal + " while run time expired.");
                    try
                    {
                        if (DEBUG)
                            Thread.sleep(250);
            ...
            if (retVal >= TransportReturnCodes.SUCCESS)
            {
                ...
                if (DEBUG)
                    System.out.println("UpajClient id=" + _id + ": writeMessage(): _bytesWritten="
                            + _bytesWritten + " _uncompressedBytesWritten="
                            + _uncompressedBytesWritten 
                            + " userBytes=" + dataBuf.limit());
        ...
        public void run()
        {
            ...
            _runningState = RunningState.TERMINATED;
            System.out.println("UpajClient id=" + _id + ": run() complete. messageCount=" + _messageCount);
    ...
    public Channel startClientChannel(int guaranteedOutputBuffers, 
            boolean blocking, boolean writeLocking, int compressionType)
    {
            ...
            Channel channel)
    {
        ...
        {
            stillRunning = false;
            if (upajClient.state() == RunningState.RUNNING)
                stillRunning = true;
    ...
    public class PackedTestArgs implements DataHandler
    {
    ...
    public class TestArgs implements DataHandler
    {
        // set default test args
        int runTime = 30;
        int guaranteedOutputBuffers = 7000;
    ...
    public void testRunner(String testName, TestArgs args)
    {
        ...
        UpajServer server = new UpajServer(bindOptions, acceptOptions, args.globalLocking, args);
        ...
        else
        {
            ...
            UpajClient upajClient = new UpajClient(1, // upajClientCount
                                                   0, // priority
                                                   clientChannel, 
                                                   args.globalLocking);
            Thread clientThread = new Thread(upajClient);
            clientThread.start();
            ...
            for (int testMessageSize : args.messageSizes)
            {
                ...
                upajClient.populateMessage(msgData, msgData.capacity(), args.messageContent);
                int writeReturn = upajClient.writeMessage(msgData);
                ...
                if (args.debug)
                    System.out.println("writeArgs.bytesWritten()=" + upajClient._writeArgs.bytesWritten()
                                       + " writeArgs.uncompressedBytesWritten=" + upajClient._writeArgs.uncompressedBytesWritten());

                if (args.expectedTotalBytes != -1)
                {
                    assertTrue(args.expectedTotalBytes == upajClient._writeArgs.bytesWritten());
                }
                if (args.expectedUncompressedBytes != -1)
                {
                    assertTrue(args.expectedUncompressedBytes == upajClient._writeArgs.uncompressedBytesWritten());
                }
            ...
            terminateServerAndClients(serverThread, server, clientThread, upajClient, clientChannel);
    ...
    public void testRunnerPacked(String testName, PackedTestArgs args)
    {
        ...
        else
        {
            ...
            clientThread.start();
                ...
                int writeReturn = upajClient.writePackedMessages(args);
                if (writeReturn < TransportReturnCodes.SUCCESS)
                {
