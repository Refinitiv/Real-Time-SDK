...
{
    /* Pointer array of RsslFieldSetDef definitions. This is allocated with rsslAllocateFieldSetDefDb. */
    FieldSetDefImpl[] _definitions = new FieldSetDefImpl[MAX_LOCAL_ID + 1];
    ...
    Buffer tempFidArray = CodecFactory.createBuffer();
        ...
        while ((ret = vectorEntry.decode(iter)) != CodecReturnCodes.END_OF_CONTAINER)
        {
            ...
            while ((ret = elemEntry.decode(iter)) != CodecReturnCodes.END_OF_CONTAINER)
            {
                ...
                if (elemEntry._name.equals(ElementNames.SET_NUMENTRIES))
                {
                    ...
                    {
                        tempSetDefEntry[i] = CodecFactory.createFieldSetDefEntry();
                    }
                ...
                else if (elemEntry._name.equals(ElementNames.SET_FIDS))
                {
                    ...
                    {
                        setError(error, "Cannot decode '" + ElementNames.SET_FIDS.toString() + "' element.");
                        return CodecReturnCodes.FAILURE;
                    }
                    ...
                    else
                    {
                        if (arr.decode(iter) < 0)
                        {
                            setError(error, "Cannot decode '" + ElementNames.SET_FIDS.toString() + "' array.");
                            return CodecReturnCodes.FAILURE;
                        }
            ...
            _definitions[(int)newSetDef.setId()] = (FieldSetDefImpl)CodecFactory.createFieldSetDef();
        ...
        if (verbosity > Dictionary.VerbosityValues.INFO)
        {
            while (curSetDef <= maxSetId)
            {
                if (_definitions[(int)curSetDef] != null && _definitions[(int)curSetDef]._setId != BLANK_ID)
                {
                    ...
                    {
                        encArrayEntry.clear();
                        tmpInt.value(_definitions[(int)curSetDef]._entries[i].fieldId());
    ...
    public int addSetDef(FieldSetDef setDef, Error error)
    {
        ...
        FieldSetDefImpl newDef = (FieldSetDefImpl)CodecFactory.createFieldSetDef();
        ...
        {
            newDef.entries()[i] = CodecFactory.createFieldSetDefEntry();
