            ...
            try
            {
                ...
                while (_timerEventQueue.hasNext())
                {
                    ...
                    if (System.nanoTime() >= event.timeout())
                    {
                        ...
                        if (event.eventType() == WorkerEventTypes.START_OPEN_TIMER)
                        {
                            eventType = WorkerEventTypes.TUNNEL_STREAM_OPEN_TIMEOUT;
                        }
                        else if (event.eventType() == WorkerEventTypes.START_WATCHLIST_TIMER)
                        {
                ...
                // check if initialization timeout occurred and handle connection recovery
                ...
                while (_initChannelQueue.hasNext())
                {
                    ...
                    if (reactorChannel.state() == ReactorChannel.State.DOWN_RECONNECTING)                    	
                    { 
                    	if (!reactorChannel.connectionRecoveryHandler().recoveryAttemptLimitReached())
                    	{
	                    	if (reactorChannel.connectionRecoveryHandler().recoverConnection(reactorChannel, _selector))
	                    	{
	                    		// channel.init is complete,
	                            // save the channel's negotiated ping timeout
	                            reactorChannel.pingHandler().initPingHandler(reactorChannel.channel().pingTimeout());
	                            ...
	                            // set ReactorChannel state to UP
	                            reactorChannel.state(ReactorChannel.State.UP);

	                    		// send channel up event
	                            sendWorkerEvent(reactorChannel, WorkerEventTypes.CHANNEL_UP,
                                        ReactorReturnCodes.SUCCESS, null, null);	                    		
	                    	}
                    	}
                    	else // connection recovery attempt limit reached
                    	{
                    	    sendWorkerEvent(reactorChannel, WorkerEventTypes.CHANNEL_DOWN,
                                            ReactorReturnCodes.FAILURE, "Worker.run",
                                            "connection recovery attempt limit reached");
                    		_initChannelQueue.remove(reactorChannel);
                    	}
                    }
                }
                
                // handle pings and connection recovery
    ...
    private void processWorkerEvent()
    {
        ...
        switch (eventType)
        {
            case CHANNEL_INIT:
                ...
                break;
            case CHANNEL_DOWN:
            	// close if server channel or retries exhausted
            	if (reactorChannel.server() != null || event.reactorChannel().connectionRecoveryHandler().recoveryAttemptLimitReached())
            	{
                    processChannelClose(reactorChannel);
            	}
            	// otherwise allow channel to go into connection recovery for client channel
            	break;
            case CHANNEL_CLOSE:
                processChannelClose(reactorChannel);
                break;
