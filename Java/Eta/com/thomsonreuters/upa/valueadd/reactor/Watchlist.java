...
class Watchlist extends VaNode
{
    ...
    HashMap<Integer,WlRequest> _streamIdtoWlRequestTable = new HashMap<Integer,WlRequest>();
    HashMap<Integer,WlStream> _streamIdtoWlStreamTable = new HashMap<Integer,WlStream>();
    ...
    Watchlist(ReactorChannel reactorChannel, ConsumerRole consumerRole)
    {
        ...
        _tempByteBuffer1 = ByteBuffer.allocate(6144);
        _tempBuffer1.data(_tempByteBuffer1);
        _tempByteBuffer2 = ByteBuffer.allocate(6144);
        _tempBuffer2.data(_tempByteBuffer2);
    }
    
    HashMap<Integer,WlStream> streamIdtoWlStreamTable()
    {
        return _streamIdtoWlStreamTable;
    }
    
    HashMap<Integer,WlRequest> streamIdtoWlRequestTable()
    {
    ...
    {
        WlRequest wlRequest = _streamIdtoWlRequestTable.get(msg.streamId());
        ...
        if (msg.msgClass() == MsgClasses.REQUEST)
        {
            ...
            if (ret >= ReactorReturnCodes.SUCCESS)
            {
                ...
                if (!isReissue)
                {
                    _streamIdtoWlRequestTable.put(msg.streamId(), wlRequest);
                }
            }
            else // submit failed
            {
    ...
    int nextStreamId()
    {
        ...
        while (_streamIdtoWlStreamTable.containsKey(nextStreamId))
        {
