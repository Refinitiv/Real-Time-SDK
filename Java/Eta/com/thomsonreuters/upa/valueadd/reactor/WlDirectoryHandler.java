...
public class WlDirectoryHandler implements WlHandler
{
    ...
    // List of Request Messages to assemble and fanout when dispatch is called
    LinkedList<RequestMsg> _requestDispatchList = new LinkedList<RequestMsg>();
    ...
    WlDirectoryHandler(Watchlist watchlist)
    {
        ...
        _directoryRequest.streamId(_watchlist.nextStreamId());
        
        // Create Default Stream
        if (_stream == null)
        {
            if (!_watchlist.streamIdtoWlStreamTable().containsKey(_directoryRequest.streamId()))
            {
                // create stream
                _stream = ReactorFactory.createWlStream();
                _stream.handler(this);
                _stream.watchlist(_watchlist);
                _stream.streamId(_directoryRequest.streamId());
                _stream.domainType(DomainTypes.SOURCE);
            }
        }
        
        // add stream to watchlist table
        _watchlist.streamIdtoWlStreamTable().put(_directoryRequest.streamId(), _stream);
    }
    ...
    public int submitRequest(WlRequest wlRequest, RequestMsg requestMsg, boolean isReissue, ReactorSubmitOptions submitOptions, ReactorErrorInfo errorInfo)
    {
        ...
        {
            _requestDispatchList.add(requestMsg);
            
            if (_requestDispatchList.size() == 1)
            {
	        ...
	        // We have our refresh already, set state to Open
    ...
    public int submitMsg(WlRequest wlRequest, Msg msg, ReactorSubmitOptions submitOptions, ReactorErrorInfo errorInfo)
    {
        ...
        switch (msg.msgClass())
        {
            case MsgClasses.CLOSE:
                // Remove refresh out of the dispatch queue list
                for (int i = 0; i < _requestDispatchList.size(); ++i)
                {
                	if (_requestDispatchList.get(i).streamId() == msg.streamId())
                	{
                		_requestDispatchList.remove(i);
                	}
                }
    ...
    public void fillDirectoryRefreshFromRequestMsg(DirectoryRefresh directoryRefresh, RequestMsg requestMsg)
    {
	        ...
	        directoryRefresh.flags(requestMsg.flags());
	        directoryRefresh.state(_directoryRefresh.state());
    ...
    private void fillDirectoryRefreshServiceListFromCache(DirectoryRefresh directoryRefresh) {
    	...
    	if (directoryRefresh.checkHasServiceId())
    	{
    		if (_serviceCache.service(directoryRefresh.serviceId()) != null)
    		{
    			...
    			setFilterFlags(directoryRefresh.filter(), directoryRefresh.serviceList().get(0));
    		}
    	}
    	else
    	{
    		for (int i = 0; i < serviceList().size(); ++i)
    		{
    			...
    			setFilterFlags(directoryRefresh.filter(), service);
    			directoryRefresh.serviceList().add(service);
    		}
    	}
	}
    
    int setFilterFlags(long filter, Service service)
    {
    	int retNumFilters = 0;
    	...
    	{    	 
        	...
        	for (int i = 0; i < serviceList().size(); ++i) 
        	{
        				...
        				usrRequest.requestMsg().msgKey().serviceId() == 0)
        		{
            		...
            		_directoryUpdate.serviceList().get(i).action(MapEntryActions.DELETE);
        		}
        	}
    ...
    int readRefreshMsg(WlStream wlStream, DecodeIterator dIter, Msg msg, ReactorErrorInfo errorInfo)
    {
        ...
        if (((RefreshMsg)msg).checkClearCache())
        {
            ...
            if (!((RefreshMsg)msg).checkSolicited())
            {
                _watchlist.itemHandler().allServicesDeleted();
            }
        }
        ...
        if (ret == ReactorCallbackReturnCodes.SUCCESS)
        {
            ...
            if (serviceList != null)
            {
                ret = _serviceCache.processServiceList(serviceList, errorInfo);
            }
        }
    ...
    int readUpdateMsg(WlStream wlStream, DecodeIterator dIter, Msg msg, ReactorErrorInfo errorInfo)
    {
        ...
        for (int i = 0; i < wlStream.userRequestList().size(); i++)
        {
                ...
                wlRequest.state() == WlRequest.State.REFRESH_PENDING)
            {
        		...
        		for (int j = 0; j < _directoryUpdate.serviceList().size(); ++j)
        		{
        			// If requested service...
        			if (!wlRequest.requestMsg().msgKey().checkHasServiceId() ||
        					_directoryUpdate.serviceList().get(j).serviceId() == wlRequest.requestMsg().msgKey().serviceId())
        			{
            			// ... and if DELETE or ADD action, keep service ...
            			if (_directoryUpdate.serviceList().get(j).action() == MapEntryActions.DELETE || 
            					_directoryUpdate.serviceList().get(j).action() == MapEntryActions.ADD)
            			{
            				setFilterFlags(wlRequest.requestMsg().msgKey().filter(), _directoryUpdate.serviceList().get(j));
            				continue;
            			}
            			// ... or if requested filters exist, flip flags and keep service, otherwise remove service
            			if (setFilterFlags(wlRequest.requestMsg().msgKey().filter(), _directoryUpdate.serviceList().get(j)) == 0)
            			{
            				_directoryUpdate.serviceList().remove(j--);
            			}
        			}
        			else
        			{
        		...
        		if (_directoryUpdate.serviceList().size() > 0)
        		{
                    ...
                    // callback user
                    if ((ret = callbackUser("WlDirectoryHandler.readUpdateMsg", msg, _directoryUpdate, errorInfo)) < ReactorCallbackReturnCodes.SUCCESS)
                     {
    ...
    int dispatch(ReactorErrorInfo errorInfo)
    {
        ...
        if (!_serviceCache._serviceList.isEmpty())
        {
	        ...
	        while ((requestMsg = _requestDispatchList.poll()) != null)
	        {
	        	newDirectoryRefresh.clear();
	        	_watchlist.convertCodecToRDMMsg(requestMsg, newDirectoryRefresh);
	        	
	        	fillDirectoryRefreshFromRequestMsg(newDirectoryRefresh, requestMsg);
	            ...
	            for (int i = 0; i < _stream.userRequestList().size(); i++)
	            {
	                    ...
	                    wlRequest.state() == WlRequest.State.REFRESH_PENDING)
	                {
	                	...
	                	requestMsg.streamId(wlRequest.requestMsg().streamId());
	                	newDirectoryRefresh.streamId(wlRequest.requestMsg().streamId());
	                	
	                	requestMsg.msgKey().filter(wlRequest.requestMsg().msgKey().filter());
	                	newDirectoryRefresh.filter(wlRequest.requestMsg().msgKey().filter());
	                	
	                	fillDirectoryRefreshServiceListFromCache(newDirectoryRefresh);
	                	
	                    // callback user
	                    if ((ret = callbackUser("WlDirectoryHandler.dispatch", requestMsg, newDirectoryRefresh, errorInfo)) < ReactorCallbackReturnCodes.SUCCESS)
	                     {
	                        // put Directory Refresh services back into pool since we are finished with them
	                        for (int j = 0; j < newDirectoryRefresh.serviceList().size(); ++j)
	                        {
    ...
    {
        if (_watchlist.role()._directoryRequest != null)
        {
        	if (!_watchlist.streamIdtoWlRequestTable().containsKey(_watchlist.role().rdmDirectoryRequest().streamId()))
        	{
                ...
                _watchlist.streamIdtoWlRequestTable().put(_watchlist.role().rdmDirectoryRequest().streamId(), wlRequest);
        ...
        if (_stream.state().streamState() != StreamStates.OPEN && _stream.state().streamState() != StreamStates.CLOSED)
        {
            _tempMsg.clear();
