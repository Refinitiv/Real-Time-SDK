...
class WlLoginHandler implements WlHandler
{
    ...
    public int submitRequest(WlRequest wlRequest, RequestMsg requestMsg, boolean isReissue, ReactorSubmitOptions submitOptions, ReactorErrorInfo errorInfo)
    {
        ...
        if (_loginRequest != null &&
            requestMsg.streamId() != _loginRequest.streamId() &&  
            	_stream != null && _stream.state().streamState() != StreamStates.CLOSED)
        {
        ...
        if (_stream == null)
        {
            if (!_watchlist.streamIdtoWlStreamTable().containsKey(requestMsg.streamId()))
            {
                // create stream
        ...
        if (!isReissue)
        {
            ...
            _watchlist.streamIdtoWlStreamTable().put(requestMsg.streamId(), _stream);               
        }
    ...
    public int submitMsg(WlRequest wlRequest, Msg msg, ReactorSubmitOptions submitOptions, ReactorErrorInfo errorInfo)
    {
        ...
        switch (msg.msgClass())
        {
            case MsgClasses.CLOSE:
                ...
                int ret2 = _watchlist.itemHandler().loginStreamClosed();
                ...
                // remove watchlist request from table
                WlRequest wlRequestTable = _watchlist.streamIdtoWlRequestTable().remove(msg.streamId());
                wlRequestTable.returnToPool();
    ...
    int handleReissue(LoginRequest loginRequest, ReactorErrorInfo errorInfo)
    {
        ...
        {
            if (_loginRefresh.checkHasFeatures() &&
                _loginRefresh.features().checkHasSupportOptimizedPauseResume() &&
            ...
            else
            {
                return _watchlist.reactor().populateErrorInfo(errorInfo,
                                                              ReactorReturnCodes.INVALID_USAGE,
                                                              "WlLoginHandler.handleReissue",
                                                              "Pause not supported by provider");
            }
        }
        else // not pause
        {
    ...
    int validateReissue(LoginRequest loginRequest, ReactorErrorInfo errorInfo)
    {
        ...
        if (_loginRequest.checkHasUserNameType() == loginRequest.checkHasUserNameType())
        {
            if (_loginRequest.checkHasUserNameType() &&
                (loginRequest.userNameType() != _loginRequest.userNameType() ||
                 (_loginRequest.userNameType() != Login.UserIdTypes.TOKEN && !_loginRequest.userName().equals(loginRequest.userName()))))
            {
                return _watchlist.reactor().populateErrorInfo(errorInfo,
                                                              ReactorReturnCodes.INVALID_USAGE,
                                                              "WlLoginHandler.validateReissue",
        ...
        if (_loginRequest.checkHasInstanceId() == loginRequest.checkHasInstanceId())
        {
            if (_loginRequest.checkHasInstanceId() &&
                _loginRequest.instanceId() != loginRequest.instanceId())
            {
                return _watchlist.reactor().populateErrorInfo(errorInfo,
                                                              ReactorReturnCodes.INVALID_USAGE,
                                                              "WlLoginHandler.validateReissue",
        ...
        if (_loginRequest.checkHasPassword() == loginRequest.checkHasPassword())
        {
            if (_loginRequest.checkHasPassword() &&
                _loginRequest.password() != loginRequest.password())
            {
                return _watchlist.reactor().populateErrorInfo(errorInfo,
                                                              ReactorReturnCodes.INVALID_USAGE,
                                                              "WlLoginHandler.validateReissue",
        ...
        if (_loginRequest.checkHasAttrib() == loginRequest.checkHasAttrib())
        {
            ...
            {
                if (_loginRequest.attrib().checkHasApplicationId() == loginRequest.attrib().checkHasApplicationId())
                {
                    if (_loginRequest.attrib().checkHasApplicationId() &&
                        !_loginRequest.attrib().applicationId().equals(loginRequest.attrib().applicationId()))
                    {
                        return _watchlist.reactor().populateErrorInfo(errorInfo,
                                                                      ReactorReturnCodes.INVALID_USAGE,
                                                                      "WlLoginHandler.validateReissue",
                ...
                if (_loginRequest.attrib().checkHasApplicationName() == loginRequest.attrib().checkHasApplicationName())
                {
                    if (_loginRequest.attrib().checkHasApplicationName() &&
                        !_loginRequest.attrib().applicationName().equals(loginRequest.attrib().applicationName()))
                    {
                        return _watchlist.reactor().populateErrorInfo(errorInfo,
                                                                      ReactorReturnCodes.INVALID_USAGE,
                                                                      "WlLoginHandler.validateReissue",
                ...
                if (_loginRequest.attrib().checkHasPosition() == loginRequest.attrib().checkHasPosition())
                {
                    if (_loginRequest.attrib().checkHasPosition() &&
                        !_loginRequest.attrib().position().equals(loginRequest.attrib().position()))
                    {
                        return _watchlist.reactor().populateErrorInfo(errorInfo,
                                                                      ReactorReturnCodes.INVALID_USAGE,
                                                                      "WlLoginHandler.validateReissue",
                ...
                if (_loginRequest.attrib().checkHasSingleOpen() == loginRequest.attrib().checkHasSingleOpen())
                {
                    if (_loginRequest.attrib().checkHasSingleOpen() &&
                        _loginRequest.attrib().singleOpen() != loginRequest.attrib().singleOpen())
                    {
                        return _watchlist.reactor().populateErrorInfo(errorInfo,
                                                                      ReactorReturnCodes.INVALID_USAGE,
                                                                      "WlLoginHandler.validateReissue",
                ...
                if (_loginRequest.attrib().checkHasAllowSuspectData() == loginRequest.attrib().checkHasAllowSuspectData())
                {
                    if (_loginRequest.attrib().checkHasAllowSuspectData() &&
                        _loginRequest.attrib().allowSuspectData() != loginRequest.attrib().allowSuspectData())
                    {
                        return _watchlist.reactor().populateErrorInfo(errorInfo,
                                                                      ReactorReturnCodes.INVALID_USAGE,
                                                                      "WlLoginHandler.validateReissue",
                ...
                if (_loginRequest.attrib().checkHasProvidePermissionExpressions() == loginRequest.attrib().checkHasProvidePermissionExpressions())
                {
                    if (_loginRequest.attrib().checkHasProvidePermissionExpressions() &&
                        _loginRequest.attrib().providePermissionExpressions() != loginRequest.attrib().providePermissionExpressions())
                    {
                        return _watchlist.reactor().populateErrorInfo(errorInfo,
                                                                      ReactorReturnCodes.INVALID_USAGE,
                                                                      "WlLoginHandler.validateReissue",
                ...
                if (_loginRequest.attrib().checkHasProvidePermissionProfile() == loginRequest.attrib().checkHasProvidePermissionProfile())
                {
                    if (_loginRequest.attrib().checkHasProvidePermissionProfile() &&
                        _loginRequest.attrib().providePermissionProfile() != loginRequest.attrib().providePermissionProfile())
                    {
                        return _watchlist.reactor().populateErrorInfo(errorInfo,
                                                                      ReactorReturnCodes.INVALID_USAGE,
                                                                      "WlLoginHandler.validateReissue",
        ...
        if (ret == ReactorReturnCodes.SUCCESS)
        {
            switch (wlStream.state().streamState())
            {
                ...
                case StreamStates.REDIRECTED:
                    ...
                    ret2 = _watchlist.itemHandler().loginStreamClosed();
                    ...
                    // recover stream if state is CLOSED_RECOVER
                    if (wlStream.state().streamState() == StreamStates.CLOSED_RECOVER)
                    {
                        _tempMsg.clear();
                        _watchlist.convertRDMToCodecMsg(_loginRequest, _tempMsg);
                        ret = wlStream.sendMsg(_tempMsg, _submitOptions, errorInfo);
                    }
                    else if (wlStream.state().streamState() == StreamStates.CLOSED || wlStream.state().streamState() == StreamStates.REDIRECTED)
                    {
                        // remove watchlist request from table if state is closed
                        WlRequest wlRequest = _watchlist.streamIdtoWlRequestTable().remove(msg.streamId());
                ...
                case StreamStates.OPEN:
                    if (wlStream.state().dataState() == DataStates.OK)
                    {
                        ...
                        ret1 = _watchlist.directoryHandler().loginStreamOpen(errorInfo);
    ...
    int readRefreshMsg(WlStream wlStream, DecodeIterator dIter, Msg msg, ReactorErrorInfo errorInfo)
    {
        ...
        // SingleOpen and AllowSuspectData must match the user's request, regardless of provider support.
        // SupportOptimizedPauseResume must be passed on from the provider, but must NOT pass on SupportPauseResume.
        ...
        _loginRefresh.attrib().allowSuspectData(supportAllowSuspectData() ? 1 : 0);
        _loginRefresh.features().applyHasSupportBatchRequests();
        _loginRefresh.features().supportBatchRequests(Login.BatchSupportFlags.SUPPORT_REQUESTS);
        _loginRefresh.features().applyHasSupportEnhancedSymbolList();
        _loginRefresh.features().supportEnhancedSymbolList(Login.EnhancedSymbolListSupportFlags.DATA_STREAMS);
        ...
        // call back user
        return callbackUser("WlLoginHandler.readRefreshMsg", msg, _loginRefresh, errorInfo);
    }
    ...
    {
        // call back user
        return callbackUser("WlLoginHandler.readGenericMsg", msg, null, errorInfo);
    }
    ...
    int readAckMsg(WlStream wlStream, DecodeIterator dIter, Msg msg, ReactorErrorInfo errorInfo)
    {
        ...
        if (wlStream.handlePostAck(msg))
        {
            ...
            {
                ret = _watchlist.itemHandler().callbackUser("WlLoginHandler.readAckMsg", msg, null, errorInfo);
            }
            else
            {
                ret = callbackUser("WlLoginHandler.readAckMsg", msg, null, errorInfo);                
            }
        }
    ...
    {    	
        ...
        else
        {
        	if (_watchlist._role != null &&
        		...
        		return (_watchlist._role._loginRequest.attrib().singleOpen() > 0 ? true: false); 
        	else
        		return false;
        }
    }

    /* Returns whether or not watchlist login stream supports allow suspect data. */
    boolean supportAllowSuspectData()
    {
        ...
        {
            return (_loginRequest.attrib().allowSuspectData() > 0 ? true : false);
        }
        else
        {   
        	if (_watchlist._role != null &&
        		_watchlist._role._loginRequest != null &&
        		_watchlist._role._loginRequest.attrib() != null)	
        		return (_watchlist._role._loginRequest.attrib().allowSuspectData() > 0 ? true: false);            
        	else
        		return false;
        }        
    }
    ...
    {
        if ( _userloginStreamOpen == false) return false;
    ...
    boolean supportOptimizedPauseResume()
    {
        if (_loginRefresh != null &&
            _loginRefresh.features().checkHasSupportOptimizedPauseResume())
        {
            return (_loginRefresh.features().supportOptimizedPauseResume() > 0 ? true : false);
        }
        else
        {
            return false;
    ...
    boolean supportViewRequests()
    {
        ...
        {
            return (_loginRefresh.features().supportViewRequests() > 0 ? true : false);
        }
        else
        {
            return false;
    ...
    boolean supportPost()
    {
        ...
        {
            return (_loginRefresh.features().supportOMMPost() > 0 ? true : false);
        }
        else
        {
            return false;
    ...
    void channelDown()
    {
        ...
        if (_stream != null)
        {
            ...
            callbackUser("WlLoginHandler.channelDown", _statusMsg, _loginStatus, _errorInfo);
        }
    }
    ...
    void channelUp(ReactorErrorInfo errorInfo)
    {
        ...
        if (_loginRequest == null && _watchlist.role().rdmLoginRequest() != null)
        {
            newRequest = true;
        ...
        if (_loginRequest != null && _stream != null)
        {
            ...
            if (_stream.sendMsg(_tempMsg, _submitOptions, errorInfo) >= ReactorReturnCodes.SUCCESS)
            {
                ...
                if (newRequest)
                {
                    ...
                    _watchlist.streamIdtoWlRequestTable().put(_loginRequest.streamId(), wlRequest);
                }
                
                if (newStream)
                {
                    // add stream to watchlist table
                    _watchlist.streamIdtoWlStreamTable().put(_loginRequest.streamId(), _stream);
                }
            }
            else // sendMsg failed
            {
