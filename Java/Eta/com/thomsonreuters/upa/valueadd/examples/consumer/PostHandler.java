...
import com.thomsonreuters.upa.examples.rdm.marketprice.MarketPriceRefresh;
import com.thomsonreuters.upa.examples.rdm.marketprice.MarketPriceUpdate;
...
class PostHandler
{
	...
	private final int TRANSPORT_BUFFER_SIZE_DATA_POST = 5000;
    ...
    {
        // get a buffer for the item request
        TransportBuffer msgBuf = chnl.getBuffer(TRANSPORT_BUFFER_SIZE_DATA_POST, false, errorInfo);
        if (msgBuf == null)
            return CodecReturnCodes.FAILURE;
     ...
     */
    private int encodePostWithData(ReactorChannel chnl, TransportBuffer msgBuf)
    {
        int ret = 0;
        ...
        // encode message
        encIter.setBufferAndRWFVersion(msgBuf, chnl.majorVersion(), Codec.minorVersion());
