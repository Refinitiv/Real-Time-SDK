...
public class Module_2_Login
{
    ...
    public static void main(String[] args)
    {
        ...
        */
        ...
        while (true)
        {
             ...
             * the NIP application pushes the Update Message content to ADH
             */
            ...
            try
            {
                ...
                if (keyIter.hasNext())
                {
                    ...
                    if (key.isReadable())
                    {
                        ...
                        while (retCode > TransportReturnCodes.SUCCESS)
                        {
                             ...
                             * Either schedule another call to read or loop on read until retCode == _RET_SUCCESS
                ...
                if (System.currentTimeMillis() >= upaRuntime)
                {
                    ...
                    if ((retCode = closeLoginStream(channel, maxMsgSize, encodeIter)) < TransportReturnCodes.SUCCESS) /* (retval > TransportReturnCodes.SUCCESS) or (retval < _RET_SUCCESS) */
                    {
    ...
    public static int sendMessage(Channel channel, TransportBuffer msgBuf)
    {
        ...
        if ((retCode = channel.write(msgBuf, writeArgs, error)) == TransportReturnCodes.WRITE_CALL_AGAIN)
        {
            ...
            /* call flush and write again - breaking out if the return code is something other than TransportReturnCodes.CALL_AGAIN (write call again) */
