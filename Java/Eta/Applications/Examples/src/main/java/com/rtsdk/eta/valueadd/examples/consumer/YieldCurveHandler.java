...
import com.rtsdk.eta.valueadd.examples.consumer.StreamIdWatchList.StreamIdKey;
import com.rtsdk.eta.valueadd.examples.consumer.StreamIdWatchList.WatchListEntry;
...
import com.rtsdk.eta.shared.rdm.marketprice.MarketPriceClose;
import com.rtsdk.eta.shared.rdm.yieldcurve.YieldCurveRequest;
 ...
 * yield curve streams, and adding/removing items to/from the item list are
 * also provided. Methods for decoding a field entry from a response, and
 * closing market price streams are also provided.
 */
class YieldCurveHandler
{
	...
	private ReactorChannel redirectChnl;
	...
	private com.rtsdk.eta.codec.Date fidDateValue = CodecFactory.createDate();
	...
	YieldCurveHandler(StreamIdWatchList watchList)
	{
	 ...
	 */
	int sendItemRequests(ReactorChannel chnl, List<String> itemNames,
	...
	private int encodeAndSendRequest(ReactorChannel chnl, YieldCurveRequest yieldCurveRequest, ReactorErrorInfo errorInfo)
	{
	...
	private int handleStatus(Msg msg, ReactorErrorInfo errorInfo)
	{
	...
	private int handleUpdate(Msg msg, DecodeIterator dIter, DataDictionary dictionary, CacheInfo cacheInfo)
	{
	...
	private int handleRefresh(Msg msg, DecodeIterator dIter, DataDictionary dictionary, CacheInfo cacheInfo, ReactorErrorInfo errorInfo)
	{
		RefreshMsg refreshMsg = (RefreshMsg) msg;
		WatchListEntry wle = watchList.get(msg.streamId());
