...
/**
 * This is the dictionary handler for the UPA NewsViewer application. It
 * provides methods for loading the field/enumType dictionaries from a file and
 * sending requests for those dictionaries to a provider. Methods for processing
 * the dictionary response and closing a dictionary stream are also provided.
 */
public class DictionaryHandler
{
    ...
    public void loadDictionary()
    {
        ...
        {
            System.out
                    .println("Unable to load field dictionary.  Will attempt to download from provider.\n\tText: "
                            + error.text());
        }
        else
        {
        ...
        {
            System.out
                    .println("Unable to load enum dictionary.  Will attempt to download from provider.\n\tText: "
                            + error.text());
        }
        else
        {
     ...
     */
    private int handleDictRefresh(Msg msg, DecodeIterator dIter,
            com.thomsonreuters.upa.transport.Error error)
    {
        ...
        if (dictionaryRefresh.streamId() == fieldDictionaryStreamId)
        {
            ...
            ret = dictionary
                    .decodeFieldDictionary(dIter, Dictionary.VerbosityValues.VERBOSE, error);
            if (ret != CodecReturnCodes.SUCCESS)
            {
                return ret;
            }
        ...
        else if (dictionaryRefresh.streamId() == enumDictionaryStreamId)
        {
            ...
            ret = dictionary.decodeEnumTypeDictionary(dIter, Dictionary.VerbosityValues.VERBOSE,
                                                      error);
            if (ret != CodecReturnCodes.SUCCESS)
            {
                return ret;
            ...
            if (dictionaryRefresh.checkRefreshComplete())
            {
                ...
                if (!isFieldDictionaryLoaded())
                    System.out
                            .println("Enumerated Types Dictionary complete, waiting for Field Dictionary...");
            }
        }
        else
        {
