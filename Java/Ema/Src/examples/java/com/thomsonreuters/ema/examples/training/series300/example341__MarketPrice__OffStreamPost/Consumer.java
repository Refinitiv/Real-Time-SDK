...
package com.thomsonreuters.ema.examples.training.series300.example341__MarketPrice__OffStreamPost;
...
class AppClient implements OmmConsumerClient
{
	public void onRefreshMsg(RefreshMsg refreshMsg, OmmConsumerEvent event)
	{
		...
		if(refreshMsg.hasMsgKey())
			System.out.println("Item Name: " + refreshMsg.name() + " Service Name: " + ( refreshMsg.hasServiceName() ? refreshMsg.serviceName() : "not set" ) );
		...
		if ( refreshMsg.domainType() == EmaRdm.MMT_LOGIN && 
				...
				refreshMsg.state().dataState() == OmmState.DataState.OK )
			{
				...
				((OmmConsumer)event.closure()).submit( postMsg.postId( 1 ).serviceId( 1 )
	...
	public void onUpdateMsg(UpdateMsg updateMsg, OmmConsumerEvent event) 
	{
		...
		if(updateMsg.hasMsgKey())
			System.out.println("Item Name: " + updateMsg.name() + " Service Name: " + ( updateMsg.hasServiceName() ? updateMsg.serviceName() : "not set" ) );
	...
	public void onStatusMsg(StatusMsg statusMsg, OmmConsumerEvent event) 
	{
		...
		if(statusMsg.hasMsgKey())
			System.out.println("Item Name: " + statusMsg.name() + " Service Name: " + ( statusMsg.hasServiceName() ? statusMsg.serviceName() : "not set" ) );
	...
	{
		System.out.println("Received Ack Msg. Item Handle: " + event.handle() + " Closure: " + event.closure());
