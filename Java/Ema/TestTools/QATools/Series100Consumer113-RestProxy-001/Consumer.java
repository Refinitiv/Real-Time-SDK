...
public class Consumer {
	...
	static String restProxyHostName_pc;
	static String restProxyPort_pc;
	
	static Map createProgrammaticConfig(Map configDb, String consumerName)
	{
		Map elementMap = EmaFactory.createMap();
		ElementList elementList = EmaFactory.createElementList();
		ElementList consumerElementList = EmaFactory.createElementList();
		
		if (restProxyHostName_pc != null && restProxyHostName_pc.length() > 0)
		{
			consumerElementList.add(EmaFactory.createElementEntry().ascii("RestProxyHostName", restProxyHostName_pc));
		}
		if (restProxyPort_pc != null && restProxyPort_pc.length() > 0)
		{
			consumerElementList.add(EmaFactory.createElementEntry().ascii("RestProxyPort", restProxyPort_pc));
		}
		
		elementMap.add(EmaFactory.createMapEntry().keyAscii(consumerName, MapEntry.MapAction.ADD, consumerElementList));
	...
	static void printHelp()
	{
		System.out.println("\nOptions:\n" + "  -?\tShows this usage\n"
						   ...
						   + "  Optional parameters of proxy server for Rest requests. Functional way."
						   + "  -restProxyHost Proxy host name (optional). Used for Rest requests only: service discovery, auth."
						   + "  -restProxyPort Proxy port number (optional). Used for Rest requests only: service discovery, auth."
						   + "  -restProxyUserName User name on proxy server (optional). Used for Rest requests only: service discovery, auth."
						   + "  -restProxyPasswd Password on proxy server (optional). Used for Rest requests only: service discovery, auth."
						   + "  -restProxyDomain Proxy Domain (optional). Used for Rest requests only: service discovery, auth."
						   + "  Optional parameters of proxy server for Rest requests. Program config way."
						   + "  -prestProxyHost Proxy host name (optional). Used for Rest requests only: service discovery, auth."
						   + "  -prestProxyPort Proxy port number (optional). Used for Rest requests only: service discovery, auth."
		...
		try
		{
			...
			while (argsCount < args.length)
			{
				...
				else if ("-restProxyHost".equals(args[argsCount]))
				{
					if (argsCount < (args.length-1) )
					{
						config.restProxyHostName(args[++argsCount]);
					}
					++argsCount;
				}
				...
				else if ("-restProxyPasswd".equals(args[argsCount]))
				{
					if (argsCount < (args.length-1) )
					{
						config.restProxyPasswd(args[++argsCount]);
					}
					++argsCount;
				}
				else if ("-restProxyDomain".equals(args[argsCount]))
				{
					if (argsCount < (args.length-1) )
					{
						config.restProxyDomain(args[++argsCount]);
					}
					++argsCount;
				}
				else if ("-prestProxyHost".equals(args[argsCount]))
				{
					if (argsCount < (args.length-1) )
					{
						restProxyHostName_pc = args[++argsCount];
					}
					++argsCount;
				}
				else if ("-prestProxyPort".equals(args[argsCount]))
				{
					if (argsCount < (args.length-1) )
					{
						restProxyPort_pc = args[++argsCount];
					}
					++argsCount;
				}
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			if ((restProxyHostName_pc != null && restProxyHostName_pc.length() > 0) ||
					(restProxyPort_pc != null && restProxyPort_pc.length() > 0))
			{
				String consumerName = connectWebSocket ? "Consumer_5" : "Consumer_4";
