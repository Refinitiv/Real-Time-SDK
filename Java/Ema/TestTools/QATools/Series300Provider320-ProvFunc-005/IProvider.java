        ...
        try
        {
            if (reqMsg.name().equals("RWFFld"))
            {
                ...
                while (!result)
                {
                    ...
                    // APIQA
                    event.provider().submit(refreshMsg.name(reqMsg.name()).serviceName("INVALID_SERVICE").domainType(EmaRdm.MMT_DICTIONARY).filter(reqMsg.filter()).payload(series).complete(result)
