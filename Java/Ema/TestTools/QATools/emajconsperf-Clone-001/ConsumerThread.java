...
public class ConsumerThread implements Runnable, OmmConsumerClient
{
	...
	/* Message cloning variables */
	private LinkedBlockingDeque<Msg> _messageQueue;
	private LinkedBlockingDeque<RefreshMsgWrapper> _refreshMsgPool;
	private LinkedBlockingDeque<UpdateMsg> _updateMsgPool;
	private LinkedBlockingDeque<StatusMsgWrapper> _statusMsgPool;
	private MessageQueueThread _messageQueueThread;
	private ExecutorService _messageQueueExecutor;
	
	class BaseWrapper implements Msg
	{
		Msg msg;
		
		@Override
		public String codeAsString() {
			return msg.codeAsString();
		}
		@Override
		public int dataType() {
			return msg.dataType();
		...
		public boolean hasNameType() {
			return msg.hasNameType();
		}
		@Override
		public boolean hasServiceId() {
			return msg.hasServiceId();
		}
		@Override
		...
		public boolean hasFilter() {
			return msg.hasFilter();
		}
		@Override
		public boolean hasExtendedHeader() {
			return msg.hasExtendedHeader();
		}
		@Override
		public int streamId() {
			return msg.streamId();
	...
	class RefreshMsgWrapper extends BaseWrapper
	{
		public RefreshMsgWrapper(RefreshMsg refreshMsg)
		{
			msg = refreshMsg;
	...
	class StatusMsgWrapper extends BaseWrapper
	{
		public StatusMsgWrapper(StatusMsg statusMsg)
		{
			msg = statusMsg;
	...
	class MessageQueueThread implements Runnable
	{
		...
		{
			_exit = true;
			_messageQueueExecutor.shutdown();
			try {
				_messageQueueExecutor.awaitTermination(5, TimeUnit.SECONDS);
				System.out.println("\nMessageQueueThread " + Thread.currentThread().getId()+ " exiting...");
			...
			while(!_exit)
			{
				...
				if(queuedMessage != null)
				{
					...
					case DataType.DataTypes.REFRESH_MSG:
						RefreshMsgWrapper refreshMsgWrapper = (RefreshMsgWrapper)queuedMessage;
						
						processRefreshMsg(refreshMsgWrapper.refreshMsg, refreshMsgWrapper.handle, refreshMsgWrapper.closure);
						
						if(_refreshMsgPool.size() < _consPerfConfig.refreshMsgPoolSize()) // Checks to limit the size of the pool
							_refreshMsgPool.add(refreshMsgWrapper);
						else
							refreshMsgWrapper = null;
					...
					case DataType.DataTypes.UPDATE_MSG:
						...
						if(_updateMsgPool.size() < _consPerfConfig.updateMsgPoolSize()) // Checks to limit the size of the pool
					...
					case DataType.DataTypes.STATUS_MSG:
						StatusMsgWrapper statusMsgWrapper = (StatusMsgWrapper)queuedMessage;
						
						processStatusMsg(statusMsgWrapper.statusMsg, statusMsgWrapper.closure);
						
						if(_statusMsgPool.size() < _consPerfConfig.statusMsgPoolSize()) // Checks to limit the size of the pool
							_statusMsgPool.add(statusMsgWrapper);
						else
							statusMsgWrapper = null;
						  ...
						  ShutdownCallback shutdownCallback)
	{
		...
		if(consConfig.enableMessageQueue())
		{
			...
			_refreshMsgPool = new LinkedBlockingDeque<RefreshMsgWrapper>();
			_updateMsgPool = new LinkedBlockingDeque<UpdateMsg>();
			_statusMsgPool = new LinkedBlockingDeque<StatusMsgWrapper>();
			
			for(int i = 0; i < consConfig.refreshMsgPoolSize(); i++)
			{
				RefreshMsgWrapper refreshMsg = new RefreshMsgWrapper(EmaFactory.createRefreshMsg(consConfig.cloneMsgBufferSize()));
				_refreshMsgPool.add(refreshMsg);
			}
			
			for(int i = 0; i < consConfig.updateMsgPoolSize(); i++)
			{
				_updateMsgPool.add(EmaFactory.createUpdateMsg(consConfig.cloneMsgBufferSize()));
			}
			
			for(int i = 0; i < consConfig.statusMsgPoolSize(); i++)
			{
				StatusMsgWrapper statusMsg = new StatusMsgWrapper(EmaFactory.createStatusMsg(consConfig.cloneMsgBufferSize()));
				_statusMsgPool.add(statusMsg);
			}
			
			_messageQueueThread = new MessageQueueThread();
			
			_messageQueueExecutor = Executors.newSingleThreadExecutor();
			
			System.out.println("\nExecuting MessageQueueThread " + Thread.currentThread().getId());
			_messageQueueExecutor.execute(_messageQueueThread);
		}
	}
	...
	private void shutdownMessageQueueThread()
	{
    ...
    RefreshMsgWrapper getRefreshMsg()
	{
		RefreshMsgWrapper refershMsg = _refreshMsgPool.poll();
		if(refershMsg == null)
		{
			refershMsg = new RefreshMsgWrapper(EmaFactory.createRefreshMsg(_consPerfConfig.cloneMsgBufferSize()));
		}
	...
	UpdateMsg getUpdateMsg()
	{
		...
		if(updateMsg == null)
		{
			updateMsg = EmaFactory.createUpdateMsg(_consPerfConfig.cloneMsgBufferSize());
		}
		
		return updateMsg;
	}
	
	StatusMsgWrapper getStatusMsg()
	{
		StatusMsgWrapper statusMsg = _statusMsgPool.poll();
		if(statusMsg == null)
		{
			statusMsg = new StatusMsgWrapper(EmaFactory.createStatusMsg(_consPerfConfig.cloneMsgBufferSize()));
		}
		
		return statusMsg;
	}
	
	private void processRefreshMsg(RefreshMsg refreshMsg, long handle, Object closure)
	{
		...
		if (_consThreadInfo._stats.imageRetrievalEndTime() == 0)
		{
			...
			if (refreshMsg.complete())
			{
				if (!_consPerfConfig.primeJVM())
				{
					((ItemInfo)closure).clientHandle(handle);
	                if (refreshMsg.state().dataState() == OmmState.DataState.OK)
	                {
                        _itemRequestList[((ItemInfo)closure).itemId()].requestState(ItemRequestState.HAS_REFRESH);
				...
				else // JVM priming enabled
				{
                    ...
                    if (_JVMPrimingRefreshCount < _requestListSize)
                    {
                        ...
                        _itemRequestList[((ItemInfo)closure).itemId()].requestState(ItemRequestState.NOT_REQUESTED);
                        if (_JVMPrimingRefreshCount == _requestListSize)
                        {
	...
	public void onRefreshMsg(com.refinitiv.ema.access.RefreshMsg refreshMsg, OmmConsumerEvent consumerEvent)
	{
		...
		{
			RefreshMsgWrapper copyRefreshMsg = getRefreshMsg();
			copyRefreshMsg.handle = consumerEvent.handle();
			copyRefreshMsg.closure = consumerEvent.closure();
			refreshMsg.copy(copyRefreshMsg.refreshMsg);
			_messageQueue.add(copyRefreshMsg);
		}
	}
	
	private void processUpdateMsg(UpdateMsg updateMsg)
	{
		if (_consThreadInfo._stats.imageRetrievalEndTime() > 0)
		{
	...
	private void processStatusMsg(StatusMsg statusMsg, Object closure)
	{
				...
				|| statusMsg.state().streamState() == OmmState.StreamState.CLOSED_REDIRECTED))
		{
			shutdownConsumer("Received unexpected final state for item: " + statusMsg.state().toString()+
        			_itemRequestList[((ItemInfo)closure).itemId()].itemName());
    		return;
		}
	}
	...
	public void onStatusMsg(StatusMsg statusMsg, OmmConsumerEvent consumerEvent)
	{
		...
		{
			StatusMsgWrapper copyStatusMsg = getStatusMsg();
			copyStatusMsg.closure = consumerEvent.closure();
			statusMsg.copy(copyStatusMsg.statusMsg);
