...
public class ConsPerfConfig
{
	...
	private boolean _enableMessageQueue; 		/* Enable message queue for decoding messages by a separate thread. */
	private int _cloneMsgBufferSize; 		/* The size of cloned message buffer */
	private int _refreshMsgPoolSize; 		/* Maximum number of refresh message in its pool */
	private int _statusMsgPoolSize; 		/* Maximum number of status message in its pool */
	private int _updateMsgPoolSize; 		/* Maximum number of update message in its pool */
	...
	public void init (String[] args, int maxThreads)
	{
        ...
        _enableMessageQueue = CommandLine.booleanValue("enableMessageQueue");
        ...
        try
        {
        	...
        	_cloneMsgBufferSize = CommandLine.intValue("cloneMsgBufSize");
        	_refreshMsgPoolSize = CommandLine.intValue("refreshMsgPoolSize");
        	_statusMsgPoolSize = CommandLine.intValue("statusMsgPoolSize");
        	_updateMsgPoolSize = CommandLine.intValue("updateMsgPoolSize");
        }
		...
		if(_cloneMsgBufferSize < 1)
		{
			System.err.println("Config Error: clone msg buffer size cannot be less than 1.");
			System.out.println(CommandLine.optionHelpString());
			System.exit(-1);
		}
		
		if(_refreshMsgPoolSize < 1)
		{
			System.err.println("Config Error: refresh message pool size cannot be less than 1.");
			System.out.println(CommandLine.optionHelpString());
			System.exit(-1);
		}
		
		if(_statusMsgPoolSize < 1)
		{
			System.err.println("Config Error: status message pool size cannot be less than 1.");
			System.out.println(CommandLine.optionHelpString());
			System.exit(-1);
		}
		
		if(_updateMsgPoolSize < 1)
		{
			System.err.println("Config Error: update message pool size cannot be less than 1.");
			System.out.println(CommandLine.optionHelpString());
			System.exit(-1);
		}
	...
	private void createConfigString()
	{
		...
		_configString = "--- TEST INPUTS ---\n\n" +
				...
				"      EnableMessageQueue: " + _enableMessageQueue + "\n";
				
				if(_enableMessageQueue)
				{
					_configString += 
				"  Cloned msg buffer size: " + _cloneMsgBufferSize + "\n" +
				"   Refresh msg pool size: " + _refreshMsgPoolSize + "\n" +
				"    Status msg pool size: " + _statusMsgPoolSize + "\n" +
				"    Update msg pool size: " + _updateMsgPoolSize + "\n";
				}
	}
    ...
    /* Checks whether message queue is enabled.
     * @return true, if message queue is enabled
     */
    public boolean enableMessageQueue()
    {
    	return _enableMessageQueue;
    }
    
    /* The cloned message buffer size
     * @return the cloned message buffer size
     */
    public int cloneMsgBufferSize()
    {
    	return _cloneMsgBufferSize;
    }
    
    /* The maximum refresh message pool size
     * @return the refresh message pool size
     */
    public int refreshMsgPoolSize()
    {
    	return _refreshMsgPoolSize;
    }
    
    /* The maximum status message pool size
     * @return the status message pool size
     */
    public int statusMsgPoolSize()
    {
    	return _statusMsgPoolSize;
    }
    
    /* The maximum update message pool size
     * @return the update message pool size
     */
    public int updateMsgPoolSize()
    {
    	return _updateMsgPoolSize;
    }
}
