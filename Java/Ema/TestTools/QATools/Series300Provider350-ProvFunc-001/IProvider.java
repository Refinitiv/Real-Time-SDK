...
package com.rtsdk.ema.examples.training.iprovider.series300.example350__Dictionary__Streaming;
    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            long rwfFld = provider
                    .registerClient(EmaFactory.createReqMsg().name("RWFFld").filter(appClient.filter).interestAfterRefresh(false).serviceName("DIRECT_FEED").domainType(EmaRdm.MMT_DICTIONARY),
                                    appClient);
            long rwfEnum = provider
                    .registerClient(EmaFactory.createReqMsg().name("RWFEnum").filter(appClient.filter).interestAfterRefresh(false).serviceName("DIRECT_FEED").domainType(EmaRdm.MMT_DICTIONARY),
                                    appClient);
            // END APIQA
