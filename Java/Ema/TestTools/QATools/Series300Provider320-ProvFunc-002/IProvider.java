            ...
            for (int i = 0; i < 60; i++)
            {
                ...
                if (i == 60)
                {
                    provider.submit(EmaFactory.createStatusMsg().name("RWFFld").serviceName("DIRECT_FEED").domainType(EmaRdm.MMT_DICTIONARY)
                                            .state(OmmState.StreamState.CLOSED, OmmState.DataState.SUSPECT, OmmState.StatusCode.NONE, "Closed Status on dictionary"), appClient.itemHandle);
                }
                if (i == 6)
                {
                    provider.submit(EmaFactory.createUpdateMsg().name("RWFFld").serviceName("DIRECT_FEED").domainType(EmaRdm.MMT_DICTIONARY), appClient.itemHandle);
                }
            }
        }
