		...
		try
		{
			...
			switch(Consumer._TEST)
			{
				...
				case 11:
					...
					consumer.registerClient(EmaFactory.createReqMsg().serviceListName("SVG1").name("LSEG.L").qos(Timeliness.REALTIME, Rate.TICK_BY_TICK), appClient);
					break;
				case 12:
					System.out.println("***APIQA TEST 12 : ReqMsg.qos(Timeliness.BEST_DELAYED_TIMELINESS, Rate.TICK_BY_TICK).***"); 
					consumer.registerClient(EmaFactory.createReqMsg().serviceListName("SVG1").name("LSEG.L").qos(Timeliness.BEST_DELAYED_TIMELINESS, Rate.TICK_BY_TICK), appClient);
					break;
				case 21:
					System.out.println("***APIQA TEST 21 : ReqMsg.qos(Timeliness.REALTIME, Rate.JIT_CONFLATED).***"); 
					consumer.registerClient(EmaFactory.createReqMsg().serviceListName("SVG1").name("LSEG.L").qos(Timeliness.REALTIME, Rate.JIT_CONFLATED), appClient);	
					break;
