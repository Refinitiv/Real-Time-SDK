    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            long rwfFld = provider
                    .registerClient(EmaFactory.createReqMsg().name("RWFFld").filter(appClient.filter).serviceName("INVALID").domainType(EmaRdm.MMT_DICTIONARY).interestAfterRefresh(false), appClient);

            long rwfEnum = provider
                    .registerClient(EmaFactory.createReqMsg().name("RWFEnum").interestAfterRefresh(false).filter(appClient.filter).serviceName("INVALID").domainType(EmaRdm.MMT_DICTIONARY), appClient);
            long rwfFld1 = provider.registerClient(EmaFactory.createReqMsg().name("RWFFld").filter(appClient.filter).serviceId(999).domainType(EmaRdm.MMT_DICTIONARY).interestAfterRefresh(false),
                                                   appClient);

            long rwfEnum1 = provider.registerClient(EmaFactory.createReqMsg().name("RWFEnum").interestAfterRefresh(false).filter(appClient.filter).serviceId(999).domainType(EmaRdm.MMT_DICTIONARY),
                                                    appClient);
