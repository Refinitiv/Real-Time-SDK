		...
		try
		{
			...
			switch(Consumer._TEST)
			{
				...
				case 0:
					...
					consumer.registerClient(EmaFactory.createReqMsg().serviceName(Consumer._serviceName).name("LSEG.L").payload(view1), appClient, 1);
					consumer.registerClient(EmaFactory.createReqMsg().serviceName(Consumer._serviceName).name("LSEG.L").payload(view2), appClient, 2);
					break;
				case 11:
					System.out.println("***APIQA TEST 11 : ReqMsg.qos(Timeliness.REALTIME, Rate.TICK_BY_TICK) & view1(2,22,25) view2(2,30,31).***"); 
					consumer.registerClient(EmaFactory.createReqMsg().serviceName(Consumer._serviceName).name("LSEG.L").payload(view1).qos(Timeliness.REALTIME, Rate.TICK_BY_TICK), appClient, 1);
					consumer.registerClient(EmaFactory.createReqMsg().serviceName(Consumer._serviceName).name("LSEG.L").payload(view2).qos(Timeliness.REALTIME, Rate.TICK_BY_TICK), appClient, 2);
					break;
				case 12:
					...
					consumer.registerClient(EmaFactory.createReqMsg().serviceName(Consumer._serviceName).name("LSEG.L").payload(view1).qos(Timeliness.BEST_DELAYED_TIMELINESS, Rate.TICK_BY_TICK), appClient, 1);
					consumer.registerClient(EmaFactory.createReqMsg().serviceName(Consumer._serviceName).name("LSEG.L").payload(view2).qos(Timeliness.BEST_DELAYED_TIMELINESS, Rate.TICK_BY_TICK), appClient, 2);
					break;
				case 21:
					System.out.println("***APIQA TEST 21 : ReqMsg.qos(Timeliness.REALTIME, Rate.JIT_CONFLATED) & view1(2,22,25) view2(2,30,31).***"); 
					consumer.registerClient(EmaFactory.createReqMsg().serviceName(Consumer._serviceName).name("LSEG.L").payload(view1).qos(Timeliness.REALTIME, Rate.JIT_CONFLATED), appClient, 1);	
					consumer.registerClient(EmaFactory.createReqMsg().serviceName(Consumer._serviceName).name("LSEG.L").payload(view2).qos(Timeliness.REALTIME, Rate.JIT_CONFLATED), appClient, 2);
					break;
