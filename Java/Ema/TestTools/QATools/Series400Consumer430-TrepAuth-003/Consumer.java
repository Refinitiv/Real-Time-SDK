    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < 120; i++)
            {
                ...
                if (i == 3)
                {
                    System.out.println("SEND PAUSE USING first TOKEN");
                    loginReq.clear().name(authenticationToken).nameType(EmaRdm.USER_AUTH_TOKEN).applicationId(appId).pause(true);
                    consumer.reissue(loginReq.message(), appLoginClient.handle);
                }
                if (i == 10)
                {
                    System.out.println("Renew new TOKEN with PAUSE falg");
                    loginReq.clear().name(newauthenticationToken).nameType(EmaRdm.USER_AUTH_TOKEN).applicationId(appId).pause(true);
                    consumer.reissue(loginReq.message(), appLoginClient.handle);
                }
                if (i == 13)
                {
                    System.out.println("SEND RESUME USING second TOKEN");
                    loginReq.clear().name(newauthenticationToken).nameType(EmaRdm.USER_AUTH_TOKEN).applicationId(appId).pause(false);
                    consumer.reissue(loginReq.message(), appLoginClient.handle);
                }
            }
