...
public class ConsumerThread implements Runnable, OmmConsumerClient
{
	...
	protected void initializeOmmConsumer()
	{
		try
		{
			...
			if (!_consPerfConfig.username().isEmpty() &&
					!_consPerfConfig.clientId().isEmpty() &&
					!_consPerfConfig.password().isEmpty())
			{
				_ommConfig.username(_consPerfConfig.username())
						.password(_consPerfConfig.password())
						.clientId(_consPerfConfig.clientId());
			}

			if (!_consPerfConfig.clientSecret().isEmpty()) {
				_ommConfig.clientSecret(_consPerfConfig.clientSecret());
			}
