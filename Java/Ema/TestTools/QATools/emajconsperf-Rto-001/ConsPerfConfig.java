...
public class ConsPerfConfig
{
	...
	
    {
		...
		CommandLine.addOption("clientSecret",  "","Client secret to use in login request");
    }
	...
	public void init (String[] args, int maxThreads)
	{
		...
		String info = "Config Error: username, clientId, password must is required for RTO connection";
		if (!_username.isEmpty()) {
			if (_clientId.isEmpty() || _password.isEmpty()) {
		...
		if (!_password.isEmpty()) {
			if (_username.isEmpty() || _clientId.isEmpty()) {
				System.err.println(info);
				System.out.println(CommandLine.optionHelpString());
				System.exit(-1);
			}
