...
import static com.thomsonreuters.upa.codec.StateCodes.USAGE_ERROR;
...
class AppClient implements OmmProviderClient
{
	...
	void processLoginRequest(ReqMsg reqMsg, OmmProviderEvent event)
	{
		...
		event.provider().submit(EmaFactory.createStatusMsg().domainType(EmaRdm.MMT_LOGIN).
				name(reqMsg.name()).nameType(EmaRdm.USER_NAME).
				state(OmmState.StreamState.CLOSED_RECOVER, OmmState.DataState.SUSPECT, USAGE_ERROR, "Login request rejected, recover is on, request does not contain user name"),
				event.handle() );
	}
