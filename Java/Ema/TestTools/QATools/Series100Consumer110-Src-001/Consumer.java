...
public class Consumer
{
    public static void main(String[] args)
    {
        try
        {
            ...
            OmmConsumer consumer = EmaFactory.createOmmConsumer(config.addAdminMsg(reqMsg.clear().domainType(EmaRdm.MMT_DIRECTORY).filter(29).interestAfterRefresh(true).serviceName("DIRECT_FEED")));
            ...
            long itemHandle = consumer.registerClient(reqMsg.clear().serviceName("DIRECT_FEED").name("JPY="), appClient, closure);
            long dirHandle = consumer.registerClient(reqMsg.clear().domainType(EmaRdm.MMT_DIRECTORY).interestAfterRefresh(true).filter(29).serviceName("DIRECT_FEED"), appClient, closure);
            Thread.sleep(60000);
