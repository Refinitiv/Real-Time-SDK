...
public class Consumer 
{
	...
	public static void main(String[] args)
	{
		...
		try
		{
            ...
            batchArray.add(EmaFactory.createOmmArrayEntry().ascii("LSEG.L"));
			...
			switch(Consumer._TEST)
			{
				default:
				case 0:
					System.out.println("***APIQA TEST 0 : ReqMsg does NOT set qos.***"); 
					consumer.registerClient(EmaFactory.createReqMsg().serviceListName("SVG1").payload(batchView), appClient);
					break;
				case 11:
					System.out.println("***APIQA TEST 11 : ReqMsg.qos(Timeliness.REALTIME, Rate.TICK_BY_TICK).***"); 
					consumer.registerClient(EmaFactory.createReqMsg().serviceListName("SVG1").payload(batchView).qos(Timeliness.REALTIME, Rate.TICK_BY_TICK), appClient);
					break;
				case 12:
					System.out.println("***APIQA TEST 12 : ReqMsg.qos(Timeliness.BEST_DELAYED_TIMELINESS, Rate.TICK_BY_TICK).***"); 
					consumer.registerClient(EmaFactory.createReqMsg().serviceListName("SVG1").payload(batchView).qos(Timeliness.BEST_DELAYED_TIMELINESS, Rate.TICK_BY_TICK), appClient);
					break;
				case 21:
					System.out.println("***APIQA TEST 21 : ReqMsg.qos(Timeliness.REALTIME, Rate.JIT_CONFLATED).***"); 
					consumer.registerClient(EmaFactory.createReqMsg().serviceListName("SVG1").payload(batchView).qos(Timeliness.REALTIME, Rate.JIT_CONFLATED), appClient);	
					break;
