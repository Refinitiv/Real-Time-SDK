...
package com.refinitiv.ema.examples.training.iprovider.series300.ex332_Dictionary_UserControl;
...
class AppClient implements OmmProviderClient
{
	...
	{
		event.provider().submit( refreshMsg.clear().domainType(EmaRdm.MMT_LOGIN).name(reqMsg.name()).
...
public class IProvider
{
	...
	static void printHelp()
	{
		System.out.println("\nOptions:\n" + "  -?\tShows this usage\n" 
				+ "  -p port number.\n"
				+ "  -delayDict Time to send ditionary response in milliseconds (default 0).\n"
			    + "\n");
	}
	...
	try
	{
		...
		while (argsCount < args.length)
		{
			...
			{
				IProvider._delayDict = Integer.parseInt(args[++argsCount]);
				++argsCount;
			}
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			provider = EmaFactory.createOmmProvider(EmaFactory.createOmmIProviderConfig().
					adminControlDictionary(OmmIProviderConfig.AdminControl.USER_CONTROL).port(IProvider._portNumber), appClient);
