    ...
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < 120; i++)
            {
                ...
                if (i == 10)
                {
                    System.out.println("SEND RESUME USING first TOKEN");
                    loginReq.clear().name(authenticationToken).nameType(EmaRdm.USER_AUTH_TOKEN).applicationId(appId).pause(false);
                    consumer.reissue(loginReq.message(), appLoginClient.handle);
                }
                if (i == 53)
                {
                    System.out.println("Renew new TOKEN");
                    loginReq.clear().name(newauthenticationToken).nameType(EmaRdm.USER_AUTH_TOKEN).applicationId(appId);
                    consumer.reissue(loginReq.message(), appLoginClient.handle);
                }
            }
