...
package com.rtsdk.ema.examples.training.consumer.series300.ex332_Dictionary_Streaming;
...
public class Consumer
{
    public static void main(String[] args)
    {
        ...
        try
        {
            ...
            consumer = EmaFactory.createOmmConsumer(EmaFactory.createOmmConsumerConfig().host("localhost:14002").username("user"), appErrorClient);

            ReqMsg reqMsg = EmaFactory.createReqMsg();

            long fldHandle = consumer.registerClient(reqMsg.domainType(EmaRdm.MMT_DICTIONARY).name("RWFFld").filter(EmaRdm.DICTIONARY_NORMAL), appClient);
            // APIQA user invalid dictionrary name
            long enumHandle = consumer.registerClient(reqMsg.clear().domainType(EmaRdm.MMT_DICTIONARY).name("RWFEnum1").filter(EmaRdm.DICTIONARY_NORMAL), appClient);
            // END APIQA
