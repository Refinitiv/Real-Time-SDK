...
class AckMsgImpl extends MsgImpl implements AckMsg
{
	...
	private final static String COPY_METHOD_NAME = AckMsgImpl.class.getCanonicalName() + ".copy(AckMsg destAckMsg)";
	...
	public AckMsgImpl(int initialSize) 
	{
    	super(DataTypes.ACK_MSG, com.refinitiv.ema.access.EmaObjectManager.GlobalObjectManager);
	...
	void setMessageAttributes(AckMsg other)
	{
		MsgImpl otherMsgImpl = (MsgImpl)other;
	...
	{
		boolean copiedMsgBuffer = super.copy(destAckMsg, COPY_METHOD_NAME);
		...
		if(copiedMsgBuffer)
		{
			...
			{	
				...
				if (this.hasServiceName())
					destAckMsg.serviceName(this.serviceName());
				
				/* The name must be set as it can be generated by the watchlist.*/
				if(hasName())
					destAckMsg.name(name());
			}
			else
			{
				/* There were some changes with the message properties */
				((AckMsgImpl)destAckMsg).setMessageAttributes(this);
			}
		}
		else
		{
