...
class PostMsgImpl extends MsgImpl implements PostMsg
{
	...
	private final static String COPY_METHOD_NAME = PostMsgImpl.class.getCanonicalName() + ".copy(PostMsg destPostMsg)";
	...
	public PostMsgImpl(int initialSize) 
	{
    	super(DataTypes.POST_MSG, com.refinitiv.ema.access.EmaObjectManager.GlobalObjectManager);
	...
	void setMessageAttributes(PostMsg other)
	{
		MsgImpl otherMsgImpl = (MsgImpl)other;
	...
	{
		boolean copiedMsgBuffer = super.copy(destPostMsg, COPY_METHOD_NAME);
		...
		if(copiedMsgBuffer)
		{
			...
			{	
				...
				if (this.hasServiceName())
					destPostMsg.serviceName(this.serviceName());
				
				/* The name must be set as it can be generated by the watchlist.*/
				if(hasName())
					destPostMsg.name(name());
			}
			else
			{
				/* There were some changes with the message properties */
				((PostMsgImpl)destPostMsg).setMessageAttributes(this);
			}
		}
		else
		{
