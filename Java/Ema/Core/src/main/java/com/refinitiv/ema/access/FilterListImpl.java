 ...
 *|           Copyright (C) 2020,2023-2024,2026 LSEG. All rights reserved.
 *|-----------------------------------------------------------------------------
 */
...
class FilterListImpl extends CollectionDataImpl implements FilterList
{
	...
	void fillCollection()
	{
		...
		int retCode;
		while ((retCode  = filterEntry._rsslFilterEntry.decode(_rsslDecodeIter)) != com.refinitiv.eta.codec.CodecReturnCodes.END_OF_CONTAINER)
		{
			switch(retCode)
			{
			case com.refinitiv.eta.codec.CodecReturnCodes.SUCCESS :
				int rsslContainerType = (filterEntry._rsslFilterEntry.action() == FilterEntryActions.CLEAR) ?  com.refinitiv.eta.codec.DataTypes.NO_DATA : (filterEntry._rsslFilterEntry.checkHasContainerType() ?
	...
	Buffer encodedData()
	{
		...
		if (_filterListCollection.isEmpty())
		{
			_rsslFilterList.containerType(com.refinitiv.eta.codec.DataTypes.NO_DATA);
		}
		else
		{    
		...
		for (com.refinitiv.ema.access.FilterEntry entry  : _filterListCollection)
		{
	...
	private FilterEntryImpl filterEntryInstance()
	{
        ...
        if(retData == null)
        {
        	retData = new FilterEntryImpl(com.refinitiv.eta.codec.CodecFactory.createFilterEntry(), noDataInstance(), _objManager);
