...
import com.rtsdk.ema.access.OmmBaseImpl.OmmImplState;
...
import com.rtsdk.eta.transport.WriteFlags;
...
import com.rtsdk.eta.valueadd.reactor.ConsumerWatchlistOptions;
import com.rtsdk.eta.valueadd.reactor.DictionaryDownloadModes;
...
import com.rtsdk.eta.valueadd.reactor.ReactorChannelEventCallback;
import com.rtsdk.eta.valueadd.reactor.ReactorChannelEventTypes;
import com.rtsdk.eta.valueadd.reactor.ReactorChannelInfo;
...
import com.rtsdk.eta.valueadd.reactor.ReactorOAuthCredential;
import com.rtsdk.eta.valueadd.reactor.ReactorReturnCodes;
import com.rtsdk.eta.valueadd.reactor.ReactorRole;
...
class ChannelCallbackClient<T> implements ReactorChannelEventCallback
{
	...
	public int reactorChannelEventCallback(ReactorChannelEvent event)
	{
		...
		switch(event.eventType())
		{
    		...
    		case ReactorChannelEventTypes.CHANNEL_UP:
    		{
    	        ...
    	        if (rsslReactorChannel.ioctl(com.rtsdk.eta.transport.IoctlCodes.SYSTEM_WRITE_BUFFERS, sendBufSize, rsslReactorErrorInfo) != ReactorReturnCodes.SUCCESS)
                {
    	        	if (_baseImpl.loggerClient().isErrorEnabled())
    	        	{
	    	        	StringBuilder temp = _baseImpl.strBuilder();
                ...
                if (rsslReactorChannel.ioctl(com.rtsdk.eta.transport.IoctlCodes.SYSTEM_READ_BUFFERS, rcvBufSize, rsslReactorErrorInfo) != ReactorReturnCodes.SUCCESS)
                {
                	if (_baseImpl.loggerClient().isErrorEnabled())
    	        	{
	    	        	StringBuilder temp = _baseImpl.strBuilder();
                ...
                {
	                if (rsslReactorChannel.ioctl(com.rtsdk.eta.transport.IoctlCodes.COMPRESSION_THRESHOLD, channelConfig.compressionThreshold, rsslReactorErrorInfo) != ReactorReturnCodes.SUCCESS)
	                {
	                	if (_baseImpl.loggerClient().isErrorEnabled())
	    	        	{
		    	        	StringBuilder temp = _baseImpl.strBuilder();
				...
				{
	                if (rsslReactorChannel.ioctl(com.rtsdk.eta.transport.IoctlCodes.HIGH_WATER_MARK, channelConfig.highWaterMark, rsslReactorErrorInfo) != ReactorReturnCodes.SUCCESS)
	                {
	                	if (_baseImpl.loggerClient().isErrorEnabled())
	    	        	{
		    	        	StringBuilder temp = _baseImpl.strBuilder();
	...
	private String channelParametersToString(ActiveConfig activeConfig,  ChannelConfig channelCfg )
	{
		...
		{
		case com.rtsdk.eta.transport.CompressionTypes.ZLIB:
			{
				compType = "ZLib";
				break;
			}
		case com.rtsdk.eta.transport.CompressionTypes.LZ4:
			{
				compType = "LZ4";
				break;
			}
		case com.rtsdk.eta.transport.CompressionTypes.NONE:
			{
				compType = "None";
				break;
			}
		...
		{
		case com.rtsdk.eta.transport.ConnectionTypes.SOCKET:
			{
		...
		case com.rtsdk.eta.transport.ConnectionTypes.HTTP:
		case com.rtsdk.eta.transport.ConnectionTypes.ENCRYPTED:
			{
				...
				if(channelCfg.rsslConnectionType == com.rtsdk.eta.transport.ConnectionTypes.ENCRYPTED)
				{
		...
		default:
			{	
				...
				strConnectionType += com.rtsdk.eta.transport.ConnectionTypes.toString(channelCfg.rsslConnectionType);
				bValidChType = false;
				break;
			}
		}
	...
	private void initializeReactor()
	{
		...
		com.rtsdk.eta.transport.ConnectOptions connectOptions = null;
		...
		for(int i = 0; i < activeConfigChannelSet.size(); i++)
		{
			...
			if (connectionType == com.rtsdk.eta.transport.ConnectionTypes.SOCKET  ||
				connectionType == com.rtsdk.eta.transport.ConnectionTypes.HTTP ||
				connectionType == com.rtsdk.eta.transport.ConnectionTypes.ENCRYPTED)
			{
				...
				connectOptions.majorVersion(com.rtsdk.eta.codec.Codec.majorVersion());
				connectOptions.minorVersion(com.rtsdk.eta.codec.Codec.minorVersion());
				connectOptions.protocolType(com.rtsdk.eta.codec.Codec.protocolType());
				...
				case com.rtsdk.eta.transport.ConnectionTypes.ENCRYPTED:
				{
					if(channelConfig.encryptedProtocolType == com.rtsdk.eta.transport.ConnectionTypes.HTTP || 
							channelConfig.encryptedProtocolType == com.rtsdk.eta.transport.ConnectionTypes.SOCKET)
					{
	...
	private void tunnelingConfiguration(com.rtsdk.eta.transport.ConnectOptions rsslOptions, HttpChannelConfig channelConfig)
    {    	
	...
	private void socketProxyConfiguration(com.rtsdk.eta.transport.ConnectOptions rsslOptions, SocketChannelConfig channelConfig)
    {    	
        if (channelConfig.httpProxy)
        {
            if (channelConfig.httpProxyHostName == null)
