...
{
	private com.rtsdk.eta.codec.Series	_rsslSeries = com.rtsdk.eta.codec.CodecFactory.createSeries();
			...
			DataDictionary rsslDictionary, Object obj)
	{
		...
		if (_errorCode == ErrorCode.NO_ERROR)
		{
			...
			int rsslContainerType = _rsslSeries.checkHasSummaryData() ? _rsslSeries.containerType() : com.rtsdk.eta.codec.DataTypes.NO_DATA;
	...
	void fillCollection()
	{
		...
		int retCode;
		while ((retCode  = seriesEntry._rsslSeriesEntry.decode(_rsslDecodeIter)) != com.rtsdk.eta.codec.CodecReturnCodes.END_OF_CONTAINER)
		{
			switch(retCode)
			{
			case com.rtsdk.eta.codec.CodecReturnCodes.SUCCESS :
	...
	Buffer encodedData()
	{
		...
		for (com.rtsdk.ema.access.SeriesEntry entry  : _seriesCollection)
		{
			seriesEntry = ((SeriesEntryImpl)entry);
			if (entryType != seriesEntry._entryDataType)
			{
				String errText = errorString().append("Attempt to add entry of ")
						.append(com.rtsdk.eta.codec.DataTypes.toString(seriesEntry._entryDataType))
	...
	private SeriesEntryImpl seriesEntryInstance()
	{
        ...
        if(retData == null)
        {
        	retData = new SeriesEntryImpl(com.rtsdk.eta.codec.CodecFactory.createSeriesEntry(), noDataInstance());
