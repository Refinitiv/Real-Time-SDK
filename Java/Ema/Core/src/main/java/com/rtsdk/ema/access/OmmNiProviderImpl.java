...
import com.rtsdk.ema.access.OmmException.ExceptionType;
...
class OmmNiProviderImpl extends OmmBaseImpl<OmmProviderClient> implements OmmProvider, DirectoryServiceStoreClient {
	...
	public void submit(RefreshMsg refreshMsg, long handle)
	{
		...
		if ( refreshMsgImpl.domainType() == EmaRdm.MMT_DIRECTORY)
		{
			...
			if ( refreshMsgImpl.rsslMsg().containerType() != com.rtsdk.eta.codec.DataTypes.MAP )
			{
				userLock().unlock();
		...
		else
		{
			...
			if ( streamInfo != null )
			{
				...
				if ( ( refreshMsgImpl._rsslMsg.flags() & com.rtsdk.eta.codec.RefreshMsgFlags.HAS_MSG_KEY) != 0 )
				{
	...
	public void submit(UpdateMsg updateMsg, long handle)
	{
		...
		if ( updateMsg.domainType() == EmaRdm.MMT_DIRECTORY)
		{
			...
			if ( updateMsgImpl.rsslMsg().containerType() != com.rtsdk.eta.codec.DataTypes.MAP )
			{
				userLock().unlock();
		...
		else
		{
			...
			if ( streamInfo != null )
			{
				...
				if ( ( updateMsgImpl._rsslMsg.flags() & com.rtsdk.eta.codec.UpdateMsgFlags.HAS_MSG_KEY) != 0 )
				{
	...
	public void submit(StatusMsg statusMsg, long handle)
	{
		...
		if ( statusMsg.domainType() == EmaRdm.MMT_DIRECTORY)
		{
			...
			if ( statusMsgImpl.rsslMsg().containerType() != com.rtsdk.eta.codec.DataTypes.MAP )
			{
				userLock().unlock();
		...
		else
		{
			...
			if ( streamInfo != null )
			{
				...
				if ( ( statusMsgImpl._rsslMsg.flags() & com.rtsdk.eta.codec.RefreshMsgFlags.HAS_MSG_KEY) != 0 )
				{
	...
	int submitDirectoryRefresh(DirectoryRefresh directoryRefresh)
	{
		...
		com.rtsdk.eta.codec.RefreshMsg rsslRefreshMsg = (com.rtsdk.eta.codec.RefreshMsg)CodecFactory.createMsg();
