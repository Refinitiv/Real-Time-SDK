...
import com.rtsdk.eta.valueadd.domainrep.rdm.directory.Service.ServiceLink;
...
abstract class DirectoryServiceStore 
{
    ...
    void populateDefaultService(Service service)
	{
		...
		else if ( _providerRole == OmmProviderConfig.ProviderRole.INTERACTIVE)
		{
	        ...
	        service.info().capabilitiesList().add((long)com.rtsdk.ema.rdm.EmaRdm.MMT_DICTIONARY);	        
        ...
        service.info().capabilitiesList().add((long)com.rtsdk.ema.rdm.EmaRdm.MMT_MARKET_PRICE);
        service.info().capabilitiesList().add((long)com.rtsdk.ema.rdm.EmaRdm.MMT_MARKET_BY_ORDER);
        service.info().capabilitiesList().add((long)com.rtsdk.ema.rdm.EmaRdm.MMT_MARKET_BY_PRICE);
        service.info().capabilitiesList().add((long)com.rtsdk.ema.rdm.EmaRdm.MMT_MARKET_MAKER);
    ...
    boolean decodeSourceDirectory(com.rtsdk.eta.codec.Msg rsslMsg, StringBuilder errorText, IntObject intObj)
	{
		...
		com.rtsdk.eta.codec.Map map = CodecFactory.createMap();
		map.clear();
    ...
    boolean decodeSourceDirectoryKeyUInt(com.rtsdk.eta.codec.Map map, DecodeIterator decodeIt, StringBuilder errorText, IntObject errorCode)
	{
		int retCode = CodecReturnCodes.SUCCESS;
		com.rtsdk.eta.codec.UInt serviceId = CodecFactory.createUInt();
		com.rtsdk.eta.codec.MapEntry mapEntry = CodecFactory.createMapEntry();
		StringBuilder text = new StringBuilder();
		com.rtsdk.eta.codec.FilterList filterList = CodecFactory.createFilterList();
		com.rtsdk.eta.codec.FilterEntry filterEntry = CodecFactory.createFilterEntry();
		com.rtsdk.eta.codec.ElementList elementList = CodecFactory.createElementList();
		com.rtsdk.eta.codec.ElementEntry elementEntry = CodecFactory.createElementEntry();
		...
		while ( ( retCode = mapEntry.decode(decodeIt, serviceId) ) != CodecReturnCodes.END_OF_CONTAINER )
		{
			...
			{
			case com.rtsdk.eta.codec.MapEntryActions.UPDATE:
				text.append("Update");
				break;
			case com.rtsdk.eta.codec.MapEntryActions.ADD:
				text.append("Add");
				break;
			case com.rtsdk.eta.codec.MapEntryActions.DELETE:
				text.append("Delete");
				break;
			default:
				break;
			...
			if ( mapEntry.action() == com.rtsdk.eta.codec.MapEntryActions.DELETE )
			{
			...
			else if ( mapEntry.action() == com.rtsdk.eta.codec.MapEntryActions.ADD )
			{
			...
			if( map.containerType() != com.rtsdk.eta.codec.DataTypes.FILTER_LIST )
			{
			...
			{	
				...
				if ( filterEntry.id() == com.rtsdk.eta.rdm.Directory.ServiceFilterIds.INFO )
				{
					if( mapEntry.action() == com.rtsdk.eta.codec.MapEntryActions.UPDATE )
					{
					...
					if ( filterEntry.checkHasContainerType() && ( filterEntry.containerType() != com.rtsdk.eta.codec.DataTypes.ELEMENT_LIST )
							&& filterList.containerType() != com.rtsdk.eta.codec.DataTypes.ELEMENT_LIST )
					{
					...
					while ( ( retCode = elementEntry.decode(decodeIt)) != CodecReturnCodes.END_OF_CONTAINER )
					{
						...
						if( !bServiceNameEntryFound && elementEntry.name().equals(com.rtsdk.eta.rdm.ElementNames.NAME) )
						{
							if ( elementEntry.dataType() != com.rtsdk.eta.codec.DataTypes.ASCII_STRING )
							{
    ...
    boolean submitSourceDirectory(ClientSession clientSession, com.rtsdk.eta.codec.Msg msg, StringBuilder errorText, boolean storeUserSubmitted, IntObject errorCode)
    {
    ...
    static DirectoryRefresh getDirectoryRefreshMsg(DirectoryCache directoryCache, boolean clearCache)
    {
    	...
    	for (int index = 0; index < serviceList.size(); index++)
		{
    		...
    		if ( service.checkHasInfo() )
    			filters |= com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.INFO;
    		
    		if( service.checkHasState() )
    			filters |= com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.STATE;
    		
    		if( service.checkHasData() )
    			filters |= com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.LOAD;
    		
    		if( service.checkHasLink() )
    			filters |= com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.LINK;
		}
