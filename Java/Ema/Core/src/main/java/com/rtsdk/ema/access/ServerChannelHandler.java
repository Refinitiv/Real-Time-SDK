    ...
    public int reactorChannelEventCallback(ReactorChannelEvent event)
    {
        ...
        switch (event.eventType())
        {
            ...
            case ReactorChannelEventTypes.CHANNEL_UP:
            {
                ...
                if (rsslReactorChannel.ioctl(com.rtsdk.eta.transport.IoctlCodes.SYSTEM_WRITE_BUFFERS, serverConfig.sysSendBufSize, errorInfo) != ReactorReturnCodes.SUCCESS)
                {
                    if (_serverImpl.loggerClient().isErrorEnabled())
                    {
                        StringBuilder temp = _serverImpl.strBuilder();
                ...
                if (rsslReactorChannel.ioctl(com.rtsdk.eta.transport.IoctlCodes.SYSTEM_READ_BUFFERS, serverConfig.sysRecvBufSize, errorInfo) != ReactorReturnCodes.SUCCESS)
                {
                    if (_serverImpl.loggerClient().isErrorEnabled())
                    {
                        StringBuilder temp = _serverImpl.strBuilder();
                ...
                {
	                if (rsslReactorChannel.ioctl(com.rtsdk.eta.transport.IoctlCodes.COMPRESSION_THRESHOLD, serverConfig.compressionThreshold, errorInfo) != ReactorReturnCodes.SUCCESS)
	                {
	                    if (_serverImpl.loggerClient().isErrorEnabled())
	                    {
	                        StringBuilder temp = _serverImpl.strBuilder();
                ...
                {
                    if (rsslReactorChannel.ioctl(com.rtsdk.eta.transport.IoctlCodes.HIGH_WATER_MARK, serverConfig.highWaterMark, errorInfo) != ReactorReturnCodes.SUCCESS)
                    {
                        if (_serverImpl.loggerClient().isErrorEnabled())
                        {
                            StringBuilder temp = _serverImpl.strBuilder();
