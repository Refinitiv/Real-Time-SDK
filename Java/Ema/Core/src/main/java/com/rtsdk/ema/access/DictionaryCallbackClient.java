...
class DictionaryCallbackClient<T> extends CallbackClient<T> implements RDMDictionaryMsgCallback
{
	...
	private com.rtsdk.eta.codec.DataDictionary		_rsslLocalDictionary;
	private com.rtsdk.eta.codec.Buffer 			_rsslEncBuffer;
	private com.rtsdk.eta.transport.Error			_rsslError;
	private com.rtsdk.eta.codec.Int 				_rsslCurrentFid;
	...
	boolean downloadDictionary(Directory directory)
	{
		...
		com.rtsdk.eta.codec.RequestMsg  rsslRequestMsg = rsslRequestMsg();
		
		rsslRequestMsg.domainType(DomainTypes.DICTIONARY);
		rsslRequestMsg.containerType(com.rtsdk.eta.codec.DataTypes.NO_DATA);
		rsslRequestMsg.applyStreaming();
		MsgKey msgKey = rsslRequestMsg.msgKey();
		msgKey.applyHasName();
		msgKey.applyHasFilter();
		msgKey.filter(com.rtsdk.eta.rdm.Dictionary.VerbosityValues.NORMAL);
	...
	com.rtsdk.eta.transport.Error rsslError()
	{
		if (_rsslError == null)
			_rsslError = com.rtsdk.eta.transport.TransportFactory.createError();
		else
			_rsslError.clear();
...
class ChannelDictionary<T>
{
	...
	void notifyStatusToListener(OmmBaseImpl<T> baseImpl, com.rtsdk.eta.codec.State rsslStatus, int streamId)
	{
	...
	int processCallback(RDMDictionaryMsgEvent event)
	{
		...
		case MsgClasses.REFRESH:
		{
			com.rtsdk.eta.codec.RefreshMsg rsslMsg = (com.rtsdk.eta.codec.RefreshMsg)msg;

			com.rtsdk.eta.codec.State state = rsslMsg.state();
			...
			if (rsslRefresh.checkHasInfo())
			{
				...
				{
					case com.rtsdk.eta.rdm.Dictionary.Types.FIELD_DEFINITIONS :
					{
						if (_fldStreamId == 0)
					...
					case com.rtsdk.eta.rdm.Dictionary.Types.ENUM_TABLES :
					{
						if (_enumStreamId == 0)
			...
			com.rtsdk.eta.transport.Error rsslError = _baseImpl.dictionaryCallbackClient().rsslError();
			if (_fldStreamId == rsslMsg.streamId())
			{
	    		...
	    		if (CodecReturnCodes.SUCCESS == _rsslDictionary.decodeFieldDictionary(dIter, 
	    																				com.rtsdk.eta.rdm.Dictionary.VerbosityValues.VERBOSE,
	    																				rsslError))
				{
			...
			else if (_enumStreamId == rsslMsg.streamId())
			{
	    		...
	    		if (CodecReturnCodes.SUCCESS == _rsslDictionary.decodeEnumTypeDictionary(dIter,
	    										com.rtsdk.eta.rdm.Dictionary.VerbosityValues.VERBOSE, rsslError))
				{
					if (rsslRefresh.checkRefreshComplete())
					{
						_isEnumLoaded = true;
		...
		case MsgClasses.STATUS:
		{
			com.rtsdk.eta.codec.StatusMsg rsslMsg = (com.rtsdk.eta.codec.StatusMsg)msg;
...
class DictionaryItem<T> extends SingleItem<T> implements TimeoutClient
{
	...
	int encodeDataDictionaryResp(boolean firstMultiRefresh, DataDictionary rsslDataDictionary, Buffer rsslDictEncBuffer)
	{
		...
		com.rtsdk.eta.transport.Error rsslError = dictCallbackClient.rsslError();
