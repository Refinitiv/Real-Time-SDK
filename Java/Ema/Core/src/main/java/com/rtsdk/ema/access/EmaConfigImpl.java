...
import com.rtsdk.ema.access.ConfigReader.XMLConfigReader;

import com.rtsdk.ema.access.OmmLoggerClient.Severity;
import com.rtsdk.ema.access.ProgrammaticConfigure.InstanceEntryFlag;
...
abstract class EmaConfigImpl extends EmaConfigBaseImpl
{
	...
	protected void addAdminMsgInt(ReqMsg reqMsg)
	{
		...
		{
			case com.rtsdk.eta.rdm.DomainTypes.LOGIN :
				ret = setLoginRequest(rsslRequestMsg);
				
				break;
			case com.rtsdk.eta.rdm.DomainTypes.DICTIONARY :
				addDictionaryReqMsg(rsslRequestMsg, (reqMsg.hasServiceName() ? reqMsg.serviceName() : null));
				break;
			case com.rtsdk.eta.rdm.DomainTypes.SOURCE :
				if (_rsslDirectoryReq == null)
				{
	...
	{	
		com.rtsdk.eta.codec.RefreshMsg rsslRefreshMsg = ((RefreshMsgImpl)refreshMsg).rsslMsg();
	...
	int setLoginRequest(RequestMsg rsslRequestMsg)
	{
	  ...
	  if (msgKey.checkHasAttrib() )
	  {
		    ...
		    com.rtsdk.eta.codec.UInt  tmpUInt = CodecFactory.createUInt();
	...
	int setDirectoryRefresh(com.rtsdk.eta.codec.RefreshMsg rsslRefreshMsg)
	{		
