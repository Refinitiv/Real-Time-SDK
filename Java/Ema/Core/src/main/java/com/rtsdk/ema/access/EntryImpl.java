...
import com.rtsdk.ema.access.OmmAnsiPage;
...
abstract class EntryImpl extends VaNode
{
	...
	protected int _previousEncodingType = com.rtsdk.eta.codec.DataTypes.UNKNOWN;
	...
	Object dateTimeValue(int year, int month, int day, int hour, int minute, int second,	int millisecond, int microsecond, int nanosecond)
	{
		...
		if ( _previousEncodingType != com.rtsdk.eta.codec.DataTypes.DATETIME )
		{
			GlobalPool.lock();
			...
			_previousEncodingType = com.rtsdk.eta.codec.DataTypes.DATETIME;
		}
		else
		{
	...
	Object dateValue(int year, int month, int day)
	{
		...
		if ( _previousEncodingType != com.rtsdk.eta.codec.DataTypes.DATE )
		{
			GlobalPool.lock();
			GlobalPool.returnPool(_previousEncodingType, _entryData);
			...
			_previousEncodingType = com.rtsdk.eta.codec.DataTypes.DATE;
		}
		else
		{
	...
	Object  timeValue(int hour, int minute, int second, int millisecond, int microsecond, int nanosecond)
	{
		...
		if ( _previousEncodingType != com.rtsdk.eta.codec.DataTypes.TIME )
		{
			GlobalPool.lock();
			GlobalPool.returnPool(_previousEncodingType, _entryData);
			...
			_previousEncodingType = com.rtsdk.eta.codec.DataTypes.TIME;
		}
		else
		{
	...
	Object stateValue(int streamState, int dataState, int statusCode, String statusText)
    {
        ...
        if ( _previousEncodingType != com.rtsdk.eta.codec.DataTypes.STATE )
		{
			GlobalPool.lock();
			GlobalPool.returnPool(_previousEncodingType, _entryData);
			...
			_previousEncodingType = com.rtsdk.eta.codec.DataTypes.STATE;
		}
		else
		{
