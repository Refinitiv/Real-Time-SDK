...
class FilterEntryImpl extends EntryImpl implements FilterEntry
{
	...
	protected com.rtsdk.eta.codec.FilterEntry _rsslFilterEntry;
	
	FilterEntryImpl()
	{
		_rsslFilterEntry = com.rtsdk.eta.codec.CodecFactory.createFilterEntry();
	}
	
	FilterEntryImpl(com.rtsdk.eta.codec.FilterEntry rsslFilterEntry, DataImpl load, EmaObjectManager objManager)
	{
		super(load);
	...
	private FilterEntry entryValue(int filterId, int action, ByteBuffer permissionData)
	{
		...
		_rsslFilterEntry.action(action);
		_rsslFilterEntry.applyHasContainerType();
		_rsslFilterEntry.containerType(com.rtsdk.eta.codec.DataTypes.NO_DATA);
