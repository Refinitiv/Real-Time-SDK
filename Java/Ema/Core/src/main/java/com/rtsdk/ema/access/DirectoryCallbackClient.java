...
import com.rtsdk.eta.valueadd.reactor.RDMDirectoryMsgCallback;
...
class DirectoryCallbackClient<T> extends CallbackClient<T> implements RDMDirectoryMsgCallback
{
	...
	public int rdmDirectoryMsgCallback(RDMDirectoryMsgEvent event)
	{
		...
		switch (directoryMsg.rdmMsgType())
		{
			case REFRESH:
			{
				com.rtsdk.eta.codec.State state = ((DirectoryRefresh)directoryMsg).state();
	
				if (state.streamState() != com.rtsdk.eta.codec.StreamStates.OPEN)
				{
					_ommBaseImpl.closeRsslChannel(rsslReactorChannel);
				...
				else if (state.dataState() == com.rtsdk.eta.codec.DataStates.SUSPECT)
				{
			...
			case STATUS:
			{
		    	...
		    	{
					com.rtsdk.eta.codec.State state = ((DirectoryStatus)directoryMsg).state();
	...
	void initialize()
	{
		if (_ommBaseImpl.activeConfig().rsslDirectoryRequest == null)
		{
			...
			_ommBaseImpl.activeConfig().rsslDirectoryRequest.filter(
									 com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.INFO |
									 com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.STATE |
									 com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.GROUP |
									 com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.LOAD |
									 com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.DATA |
									 com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.LINK);
		}
		else
		{
			_ommBaseImpl.activeConfig().rsslDirectoryRequest.streamId(2);
		...
		if (_baseImpl.loggerClient().isTraceEnabled())
		{
			...
			if ((filter & com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.INFO) != 0)
				temp.append(OmmLoggerClient.CR).append("RDM_DIRECTORY_SERVICE_INFO_FILTER");
			if ((filter & com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.STATE) != 0)
				temp.append(OmmLoggerClient.CR).append("RDM_DIRECTORY_SERVICE_STATE_FILTER");
			if ((filter & com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.GROUP) != 0)
				temp.append(OmmLoggerClient.CR).append("RDM_DIRECTORY_SERVICE_GROUP_FILTER");
			if ((filter & com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.LOAD) != 0)
				temp.append(OmmLoggerClient.CR).append("RDM_DIRECTORY_SERVICE_LOAD_FILTER");
			if ((filter & com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.DATA) != 0)
				temp.append(OmmLoggerClient.CR).append("RDM_DIRECTORY_SERVICE_DATA_FILTER"); 
			if ((filter & com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.LINK) != 0)
...
class DirectoryItem<T> extends SingleItem<T>
{
	...
	boolean submit(com.rtsdk.eta.codec.GenericMsg rsslGenericMsg)
	{
