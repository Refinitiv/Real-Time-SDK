...
class DirectoryHandler implements RDMDirectoryMsgCallback
{
		...
		case REQUEST:
			{	
			   ...
			   if ( _ommServerBaseImpl.activeConfig().acceptDirMessageWithoutMinFilters || 
					   ( ( directoryRequest.filter() & com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.INFO ) == com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.INFO )
					   && ( ( directoryRequest.filter() & com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.STATE ) == com.rtsdk.eta.rdm.Directory.ServiceFilterFlags.STATE ) )
			   {
	...
	void notifyOnClose(ReactorChannel reactorChannel, com.rtsdk.eta.codec.Msg msg, ItemInfo itemInfo)
	{
