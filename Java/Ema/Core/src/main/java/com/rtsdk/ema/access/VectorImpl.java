...
{
	private com.rtsdk.eta.codec.Vector	_rsslVector = com.rtsdk.eta.codec.CodecFactory.createVector();
			...
			DataDictionary rsslDictionary, Object obj)
	{
		...
		if (_errorCode == ErrorCode.NO_ERROR)
		{
			...
			int rsslContainerType = _rsslVector.checkHasSummaryData() ? _rsslVector.containerType() : com.rtsdk.eta.codec.DataTypes.NO_DATA;
	...
	void fillCollection()
	{
		...
		int retCode;
		while ((retCode  = vectorEntry._rsslVectorEntry.decode(_rsslDecodeIter)) != com.rtsdk.eta.codec.CodecReturnCodes.END_OF_CONTAINER)
		{
			switch(retCode)
			{
			case com.rtsdk.eta.codec.CodecReturnCodes.SUCCESS :
				int rsslContainerType = (vectorEntry._rsslVectorEntry.action() != VectorEntryActions.DELETE && vectorEntry._rsslVectorEntry.action() != VectorEntryActions.CLEAR)?
														_rsslVector.containerType() : com.rtsdk.eta.codec.DataTypes.NO_DATA;
	...
	Buffer encodedData()
	{
		...
		for (com.rtsdk.ema.access.VectorEntry entry  : _vectorCollection)
		{
			vectorEntry = ((VectorEntryImpl)entry);
			if (entryType != vectorEntry._entryDataType)
			{
				String errText = errorString().append("Attempt to add entry of ")
						.append(com.rtsdk.eta.codec.DataTypes.toString(vectorEntry._entryDataType))
	...
	private VectorEntryImpl vectorEntryInstance()
	{
        ...
        if(retData == null)
        {
        	retData = new VectorEntryImpl(com.rtsdk.eta.codec.CodecFactory.createVectorEntry(), noDataInstance(), _objManager);
