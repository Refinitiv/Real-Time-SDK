...
import com.rtsdk.ema.access.NoDataImpl;
import com.rtsdk.ema.access.FieldListImpl;
import com.rtsdk.ema.access.SingleItem;
...
class EmaObjectManager
{
	...
	<T> void initialize()
	{
		...
		for (int index = 0; index < DATA_POOL_INITIAL_SIZE; ++index)
		{
			...
			_fieldEntryPool.add(new FieldEntryImpl(com.rtsdk.eta.codec.CodecFactory.createFieldEntry(), load));
			
			load = new NoDataImpl();
       	 	_noDataPool .updatePool(load);
			_elementEntryPool.add(new ElementEntryImpl(com.rtsdk.eta.codec.CodecFactory.createElementEntry(), load));
		}
