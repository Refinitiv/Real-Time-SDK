...
{
	private com.rtsdk.eta.codec.FilterList	_rsslFilterList = com.rtsdk.eta.codec.CodecFactory.createFilterList();
	...
	void fillCollection()
	{
		...
		int retCode;
		while ((retCode  = filterEntry._rsslFilterEntry.decode(_rsslDecodeIter)) != com.rtsdk.eta.codec.CodecReturnCodes.END_OF_CONTAINER)
		{
			switch(retCode)
			{
			case com.rtsdk.eta.codec.CodecReturnCodes.SUCCESS :
				int rsslContainerType = (filterEntry._rsslFilterEntry.action() == FilterEntryActions.CLEAR) ?  com.rtsdk.eta.codec.DataTypes.NO_DATA : (filterEntry._rsslFilterEntry.checkHasContainerType() ?
	...
	Buffer encodedData()
	{
		...
		if (_filterListCollection.isEmpty())
		{
			_rsslFilterList.containerType(com.rtsdk.eta.codec.DataTypes.NO_DATA);
		}
		else
		{    
		...
		for (com.rtsdk.ema.access.FilterEntry entry  : _filterListCollection)
		{
	...
	private FilterEntryImpl filterEntryInstance()
	{
        ...
        if(retData == null)
        {
        	retData = new FilterEntryImpl(com.rtsdk.eta.codec.CodecFactory.createFilterEntry(), noDataInstance(), _objManager);
