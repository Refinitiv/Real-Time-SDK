...
The LSEG Real-Time Viewer (RTViewer) is an OMM Consumer
...
Distribution Server, Real-Time - Optimized (RTO) or an
...
NOTE that all RTO endpoints are encryption endpoints.
...
+ "Discover Endpoint": User must specify RTO authentication credentials and
     choose to connect to a RTO endpoint from a list of discovered endpoints.
     The Viewer does Service Discovery based on the type of connection requested
     by user and provides RTO endpoints in a drop down list to choose from.
...
From Java folder, run appropriate command:
  On Unix:
    ...
    ./gradlew runRTViewerDesktop
  On Windows:
    gradlew.bat runRTViewerDesktop


Detailed Description
====================

RTViewer implements the following high-level steps:
+ Choose "Discover Endpoint" or "Specify Endpoint" configuration as appropriate;
  ...
  Configure "Discover Endpoint" 
    ...
    - Optionally specify local dictionary OR if not specified, dictionary is 
      downloaded dictionary from Provider;
      NOTE: Local dictionary must be specified for WebSocket connection to RTO
