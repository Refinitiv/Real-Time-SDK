...
macro(_cmake_debug _typ _dmsg)
    message("DEBUG ${_typ}: ${_dmsg}")
endmacro()

macro(_cmake_mult_debug _typ)
...
# https://stackoverflow.com/questions/32183975/how-to-print-all-the-properties-of-a-target-in-cmake/51987470#51987470
	...
	get_target_property(_target_t ${_tgt} IMPORTED_LOCATION)
	if(_target_t AND NOT (_target_t STREQUAL "NOTFOUND"))
	...
	foreach (_prop ${PROP_LIST})
		...
		if (_propval)
			...
			_cmake_debug("Target  " "${_tgt} ${_prop} = ${_propval}")
		endif()
	endforeach()
	foreach (_build  OPTIMIZED RELEASE DEBUG DEBUG_MDD RELEASE_MD )
		set(_prop "MAP_IMPORTED_CONFIG_${_build}")
		get_property(_propval TARGET ${_tgt} PROPERTY ${_prop} SET)
		if (_propval)
			get_target_property(_propval ${_tgt} ${_prop})
			_cmake_debug("Target  " "${_tgt} ${_prop} = ${_propval}")
		endif()
		set(_prop "IMPORTED_LOCATION_${_build}")
		get_property(_propval TARGET ${_tgt} PROPERTY ${_prop} SET)
		if (_propval)
			get_target_property(_propval ${_tgt} ${_prop})
			_cmake_debug("Target  " "${_tgt} ${_prop} = ${_propval}")
...
function(DEBUG_PRINT vName)
	if (RCDEV_TRACE_ENABLED)
		...
		if (${ARGC} GREATER 1 AND ${_vName_U} MATCHES "^[ ]*::[ ]*MESSAGE")
			...
			if (${ARGC} GREATER 2)
				_cmake_mult_debug("Message " ${ARGN})
			else()
				_cmake_debug("Message " ${ARGN})
			endif()
		elseif (${ARGC} GREATER 1 AND ${_vName_U} MATCHES "^[ ]*::[ ]*SRCREPO")
			...
			if (${ARGC} GREATER 2)
				_cmake_mult_debug("SrcRepo " ${ARGN})
			else()
				_cmake_debug("SrcRepo " "${ARGV1}")
			endif()
		elseif (${ARGC} GREATER 1 AND ${_vName_U} MATCHES "^[ ]*::[ ]*OUTPATH")
			...
			if (${ARGC} GREATER 2)
				_cmake_mult_debug("OutPath " ${ARGN})
			else()
				_cmake_debug("OutPath " "${ARGV1}")
			endif()
		elseif (${ARGC} GREATER 1 AND ${_vName_U} MATCHES "^[ ]*::[ ]*FLAGOPT")
			...
			if (${ARGC} GREATER 2)
				_cmake_mult_debug("FlagOpt " ${ARGN})
			else()
				_cmake_debug("FlagOpt " "${ARGV1}")
			endif()
		...
		elseif (TARGET ${vName})
			...
			elseif (NOT (${vName} MATCHES "::"))	
				if (NOT(${vName}))
					_cmake_debug("Variable" "${vName}=<UNDEFINED>")
				else()
					_cmake_debug("Variable" "${vName}=\${${vName}}")
				endif()
			else()
				_cmake_debug("Message " "${vName}= \'\'")
			endif ()
