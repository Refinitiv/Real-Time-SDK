...
namespace LSEG.Eta.Example.Consumer
{
    public class Consumer : ResponseCallback
    {
        ...
        private const int defaultRuntime = 60;
        //END APIQA
        ...
        //APIQA
        private long initialDirFilter = ServiceFilterFlags.INFO | ServiceFilterFlags.STATE | ServiceFilterFlags.GROUP |
                                       ServiceFilterFlags.LOAD | ServiceFilterFlags.DATA | ServiceFilterFlags.LOAD;
        private long reissueDirFilter = -1;
        private long initialDictFilter = Rdm.Dictionary.VerbosityValues.NORMAL;
        private long reissueDictFilter = -1;
        private bool loadDictFromFile = false;
        
        private int qosRate = 0;
        private int qosTimeliness = 0;
        private int worstQosRate = 0;
        private int worstQosTimeliness = 0;
        private int changeServiceId = 1;
        
        private bool sendGenMsgOnLogin = false;
        private bool sendGenMsgOnDirectory = false;
        private bool sendDirReqWithoutLogin = false;
        private bool sendDictReqWithoutLogin = false;
        private bool sendItemReqWithoutLogin = false;
        //END APIQA
        public Consumer()
        {
        ...
        public void Run()
        {
            ...
            while (true)
            {
                ...
                //}
                if (!sendDirReqWithoutLogin && !sendDictReqWithoutLogin && !sendItemReqWithoutLogin)
                {
                    if (loginHandler.SendRequest(channelSession, out error) != TransportReturnCode.SUCCESS)
                    {
                        Console.WriteLine("Error sending Login request, exit.");
                ...
                else 
                {
                    ...
                    if (sendDirReqWithoutLogin)
                    {
                        srcDirHandler.SendRequest(channelSession, out error, sendGenMsgOnDirectory);
                    }
                    else if (sendDictReqWithoutLogin)
                    {
                        //APIQA
                        dictionaryHandler.SendRequest(channelSession, "RWFFld", DictionaryHandler.FIELD_DICTIONARY_STREAM_ID, initialDictFilter, out error);

                        dictionaryHandler.SendRequest(channelSession, "RWFEnum", DictionaryHandler.ENUM_TYPE_DICTIONARY_STREAM_ID, initialDictFilter, out error);
                        //END APIQA
                    }
                    else if (sendItemReqWithoutLogin)
                    {
                        Console.WriteLine("Sending MP REQUEST");
                        SendMPRequests(channelSession);
                    }
                }
        ...
        public void Init(string[] args)
        {
            ...
            if (!CommandLine.Value("initialDirFilter").Equals("") && !CommandLine.Value("reissueDirFilter").Equals("")) 
            {
                Console.WriteLine("Found -initialDirFilter and -reissueDirFilter");

                long.TryParse(CommandLine.Value("initialDirFilter"), out initialDirFilter);
                Console.WriteLine(initialDirFilter);

                long.TryParse(CommandLine.Value("reissueDirFilter"), out reissueDirFilter);
                Console.WriteLine(reissueDirFilter);

                //set directory flags
                srcDirHandler.SetDirectoryFilter(initialDirFilter, reissueDirFilter);
            }

            if (!CommandLine.Value("initialDictFilter").Equals("") && !CommandLine.Value("reissueDictFilter").Equals(""))
            {
                Console.WriteLine("Found -initialDictFilter and -reissueDictFilter");
             
                List<string> list = CommandLine.Values("reissueDict");

                long.TryParse(CommandLine.Value("initialDictFilter"), out initialDictFilter);
                Console.WriteLine(initialDictFilter);

                long.TryParse(CommandLine.Value("reissueDictFilter"), out reissueDictFilter);
                Console.WriteLine(reissueDictFilter);

                //set directory flags
                dictionaryHandler.SetDictionaryFilter(initialDictFilter, reissueDictFilter);
            }
            if (CommandLine.BoolValue("loadDictFromFile")) 
            {
                loadDictFromFile = true;
            }

            if (!CommandLine.Value("reissueQosRate").Equals("") &&  !CommandLine.Value("reissueQosTimeliness").Equals(""))
            {

                int.TryParse(CommandLine.Value("reissueQosRate"), out qosRate);
                int.TryParse(CommandLine.Value("reissueQosTimeliness"), out qosTimeliness);
                Console.WriteLine("\nReissueQoS for rate = {0} and timeline = {1}", qosRate, qosTimeliness);
                
                marketPriceHandler.SetReissueQos(qosRate, qosTimeliness);
            }

            if (!CommandLine.Value("reissueQosRate").Equals("") && !CommandLine.Value("reissueQosTimeliness").Equals("") && 
                CommandLine.Value("reissueWorstQosTimeliness").Equals("") && CommandLine.Value("reissueWorstQosTimeliness").Equals(""))
            {

                int.TryParse(CommandLine.Value("reissueQosRate"), out qosRate);
                int.TryParse(CommandLine.Value("reissueQosTimeliness"), out qosTimeliness);
                int.TryParse(CommandLine.Value("reissueWorstQosRate"), out worstQosRate);
                int.TryParse(CommandLine.Value("reissueWorstQosTimeliness"), out worstQosTimeliness);
                Console.WriteLine("\nReissueQoS for rate = {0} and timeline = {1}", qosRate, qosTimeliness);
                Console.WriteLine("ReissueWorstQos QoS for worstRate = {0} and worstTimeliness = {1}\n", worstQosRate, worstQosTimeliness);
                marketPriceHandler.SetReissueQosAndWorstQos(qosRate, qosTimeliness, worstQosRate, worstQosTimeliness);
            }
            if (CommandLine.BoolValue("sendGenericMsgOnLogin"))
            {
                sendGenMsgOnLogin = true;
            }
            if (CommandLine.BoolValue("sendGenericMsgOnDirectory"))
            {
                sendGenMsgOnDirectory = true;
            }

            long.TryParse(CommandLine.Value("initialDictFilter"), out initialDictFilter);
            long.TryParse(CommandLine.Value("reissueDictFilter"), out reissueDictFilter);

            if (!CommandLine.Value("reissueServiceId").Equals("")) 
            {
                changeServiceId = Int32.Parse(CommandLine.Value("reissueServiceId"));
                Console.WriteLine("\nReissueServiceId to service ID = {0}", changeServiceId);
                marketPriceHandler.SetReissueChangeServiceName(changeServiceId);
            }
            if (CommandLine.BoolValue("privateStreamForItemReq")) 
            {
                marketPriceHandler.SetPriviateStream();
            }
            if (!CommandLine.Value("requestSameKeyWithDiffStreamId").Equals("")) 
            {
                marketPriceHandler.SetReqSameKeyWithDiffStreamId();
                CommandLine.ParseArgs(new string[] { "-mp", defaultItemName });
            }
            if (!CommandLine.Value("specificServiceId").Equals("")) 
            {
                marketPriceHandler.SetSpecificServiceId(Int32.Parse(CommandLine.Value("specificServiceId")));
            }
            if (CommandLine.BoolValue("sendDirReqWithoutLogin")) 
            {
                sendDirReqWithoutLogin = true;
            }
            if (CommandLine.BoolValue("sendDictReqWithoutLogin"))
            {
                sendDictReqWithoutLogin = true;
            }
            if (CommandLine.BoolValue("sendItemReqWithoutLogin"))
            {
                sendItemReqWithoutLogin = true;
            }
            //END APIQA
        ...
        private void ProcessLoginResponse(ChannelSession chnl, Msg responseMsg, DecodeIterator dIter)
        {
            ...
            if (responseMsg.MsgClass == MsgClasses.GENERIC && responseMsg.ContainerType == Codec.DataTypes.ELEMENT_LIST)
            {
            ...
            else if (loginState == ConsumerLoginState.SUSPECT)
            {
                ...
                //APIQA
                if (sendGenMsgOnLogin)
                {
                    SendGenericMsgOnLogin(chnl);
                    return;
                }
                srcDirHandler.SendRequest(chnl, out error, sendGenMsgOnDirectory);
        ...
        //APIQA
        private void SendGenericMsgOnLogin(ChannelSession chnl) 
        {
            ...
            genMsg.DomainType = (int)DomainType.LOGIN;
            ...
            if ((msgBuf = chnl.GetTransportBuffer(ChannelSession.MAX_MSG_SIZE, false, out error)) == null)
            {
                CloseChannel();
                Console.WriteLine("Failed GetTransportBuffer. Error: {0}", error.ToString());
                Environment.Exit((int)TransportReturnCode.FAILURE);
            }
            ...
            TransportReturnCode retChnl = chnl.Write(msgBuf, out error);
        ...
        private static void AddCommandLineArgs()
        {
            ...
            // APIQA
            CommandLine.AddOption("sendGenericMsgOnLogin", false, "APIQA: Send generic Msg on login");
            CommandLine.AddOption("sendGenericMsgOnDirectory", false, "APIQA: Send generic Msg on directory");
            CommandLine.AddOption("requestDir", "", "APIQA: Request directory");
            CommandLine.AddOption("reissueServiceId", "", "APIQA: Reissue service Id");
            CommandLine.AddOption("reissueDirFilter", "", "APIQA: Reissue directory");
            CommandLine.AddOption("initialDirFilter", "", "APIQA: Initial directory");
            CommandLine.AddOption("reissueDictFilter", "", "APIQA: Reissue dictionary");
            CommandLine.AddOption("initialDictFilter", "", "APIQA: Initial dictionary");
            CommandLine.AddOption("reissueQosRate", "", "APIQA: Reissue QoSRate");
            CommandLine.AddOption("reissueWorstQosRate", "", "APIQA: Reissue QoSRate");
            CommandLine.AddOption("reissueQosTimeliness", "", "APIQA: Reissue QosTimeliness");
            CommandLine.AddOption("reissueWorstQosTimeliness", "", "APIQA: Reissue QosTimeliness");
            CommandLine.AddOption("loadDictFromFile", false, "APIQA: Load dictionary from file");
            CommandLine.AddOption("privateStreamForItemReq", false, "APIQA:Private stream for Item request");
            CommandLine.AddOption("requestSameKeyWithDiffStreamId", "", "APIQA:Request Same Key with different stream ID");
            CommandLine.AddOption("specificServiceId", "", "APIQA:Specific service ID");
            CommandLine.AddOption("sendDirReqWithoutLogin", false, "APIQA:Send dir request without login");
            CommandLine.AddOption("sendDictReqWithoutLogin", false, "APIQA:Send dictionary request without login");
            CommandLine.AddOption("sendItemReqWithoutLogin", false, "APIQA:Send dictionary request without login");
            //END APIQA
        }
