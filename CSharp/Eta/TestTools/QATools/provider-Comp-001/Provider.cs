...
namespace LSEG.Eta.Example.Provider
{
    ...
    public class Provider : IReceivedMsgCallback
    {
        ...
        //API QA
        private bool showTransportDetails = false;
        ...
        public void Init(String[] args)
        {
            ...
            //API QA
            Console.WriteLine("compressionType: " + CommandLine.Value("compressionType"));
            Console.WriteLine("compressionLevel: " + CommandLine.Value("compressionLevel"));
            Console.WriteLine("testCompressionZlib: " + CommandLine.Value("testCompressionZlib"));
            Console.WriteLine("td: " + CommandLine.Value("td"));
            //END API QA
            ...
            //API QA
            if (CommandLine.Value("compressionType") != null)
            {
                if (string.Equals(CommandLine.Value("compressionType"), "ZLib", StringComparison.OrdinalIgnoreCase))
                    bindOptions.CompressionType = CompressionType.ZLIB;
                else if (string.Equals(CommandLine.Value("compressionType"), "LZ4"))
                    bindOptions.CompressionType = CompressionType.LZ4;
            }
        ...
        private static void AddCommandLineArgs()
        {
            ...
            //APIQA
            CommandLine.AddOption("compressionType", defaultValue: null, "Specify compressionType either ZLib or LZ4");
            CommandLine.AddOption("compressionLevel", defaultValue: null, "Specify compressionLevel for ZLib, valid range is 0-9");
                    ...
                    for(int i = 0; i < readList.Count; i++)
                    {
                        ...
                        else
                        {
                            ...
                            if (channel != null)
                            {
								...
								//APIQA
                                if (showTransportDetails && m_ProviderSession?.GetReadArgs?.BytesRead > 0)
                                {
                                    ReadArgs readArgs = m_ProviderSession.GetReadArgs;
                                    Console.Write("\nCompression Stats Bytes In: {0} Uncompressed Bytes In: {1}\n", readArgs.BytesRead, readArgs.UncompressedBytesRead);
                                }
								//END APIQA
                            }
                        }
