...
namespace LSEG.Eta.ValueAdd.Provider
{
    public class VAProvider : IRDMLoginMsgCallback, IDirectoryMsgCallback, IDictionaryMsgCallback, IReactorChannelEventCallback, IDefaultMsgCallback
    {
        ...
        // APIQA:
        private ReactorSubmitOptions reactorSubmitOptions = new ReactorSubmitOptions();
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        ...
        public ReactorCallbackReturnCode DefaultMsgCallback(ReactorMsgEvent reactorEvent)
        {
            ...
            switch (msg.DomainType)
            {
                ...
                case (int)DomainType.SYMBOL_LIST:
                    ...
                    // APIQA:
                    requestCounter++;
                    if (((DomainType)msg.DomainType == DomainType.MARKET_PRICE) && (requestCounter == 1))
                    {
                        Console.WriteLine("----------APIQA: Sending a generic message on the LOGIN domain \n\n");
                        SendGenericMessageLogin(reactorChannel);
                        Console.WriteLine("----------APIQA: Sending a generic message on the SOURCE directory domain \n\n");
                        ...
                        Console.WriteLine("----------APIQA: Sending a generic message on the MP domain \n\n");
        ...
        private void SendGenericMessageSource(ReactorChannel chnl, int serviceId)
        {
            ...
            _genericMsg.DomainType = (int)DomainType.SOURCE;
            _genericMsg.StreamId = 2;
            _genericMsg.ContainerType = LSEG.Eta.Codec.DataTypes.ELEMENT_LIST;
            LSEG.Eta.Codec.Buffer _entryName = new LSEG.Eta.Codec.Buffer();
            _entryName.Data("valueFromProvider");
            _genericMsg.ApplyHasMsgKey();
            _genericMsg.MsgKey.ApplyHasServiceId();
            _genericMsg.MsgKey.ServiceId = serviceId;
            _genericMsg.MsgKey.ApplyHasName();
            _genericMsg.MsgKey.Name.Data("genericMsgInGeneral");
            elementList.Clear();
            elementList.ApplyHasStandardData();
            elementEntry.Clear();
            elementEntry.DataType = LSEG.Eta.Codec.DataTypes.INT;
            elementEntry.Name = _entryName;
            encodeIter.Clear();
            ret = encodeIter.SetBufferAndRWFVersion(_genericBuffer, chnl.MajorVersion, chnl.MinorVersion);

            ret = _genericMsg.EncodeInit(encodeIter, 0);
            if (ret < CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("_genericMsg fail to EncodeInit. Falied: {0}", ret.ToString());
            }
            ret = elementList.EncodeInit(encodeIter, null, 0);
            if (ret < CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("elementList fail to EncodeInit. Falied: {0}", ret.ToString());
            }
            ...
            if (ret < CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("elementEntry fail to encode. Falied: {0}", ret.ToString());
            }
            ret = elementList.EncodeComplete(encodeIter, true);
            if (ret != CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("elementList fail to encodeComplet. Falied: {0}", ret.ToString());
            }

            ret = _genericMsg.EncodeComplete(encodeIter, true);
            if (ret != CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("_genericMsg fail to EncodeComplete {0}", ret.ToString());
            }
        ...
        private void SendGenericMessageMP(ReactorChannel chnl, int serviceId)
        {
            ...
            _genericMsg.Clear();
            _genericMsg.MsgClass = MsgClasses.GENERIC;
            _genericMsg.DomainType = (int)DomainType.MARKET_PRICE;
            _genericMsg.StreamId = 3;
            _genericMsg.ContainerType = LSEG.Eta.Codec.DataTypes.NO_DATA;
            _genericMsg.ApplyHasMsgKey();
            _genericMsg.MsgKey.ApplyHasName();
            _genericMsg.MsgKey.Name.Data("Generic message on MP stream 3 on MP domain");

            encodeIter.Clear();
            encodeIter.SetBufferAndRWFVersion(_genericBuffer, chnl.MajorVersion, chnl.MinorVersion);

            ret = _genericMsg.EncodeInit(encodeIter, 0);
            if (ret < CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("_genericMsg fail to EncodeInit. Failed: {0}", ret.ToString());
            }
            ret = _genericMsg.Encode(encodeIter);
            if (ret < CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("_genericMsg fail to Encode. Failed: {0}", ret.ToString());
            }
            ret = _genericMsg.EncodeComplete(encodeIter, true);
            if (ret != CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("_genericMsg fail to EncodeComplete. Failed: {0}", ret.ToString());
            }
        ...
        private void SendGenericMessageLogin(ReactorChannel chnl)
        {
            ...
            _genericMsg.Clear();
            _genericMsg.MsgClass = MsgClasses.GENERIC;
            _genericMsg.DomainType = (int)DomainType.LOGIN;
            _genericMsg.StreamId = 1;
            _genericMsg.ContainerType = (int)LSEG.Eta.Codec.DataTypes.ELEMENT_LIST;
            Codec.Buffer _entryName = new Codec.Buffer();
            _entryName.Data("valueFromProvider");
            _genericMsg.ApplyHasMsgKey();
            _genericMsg.MsgKey.ApplyHasName();
            _genericMsg.MsgKey.Name.Data("genericMsgInGeneral");
            ...
            if (ret < CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("elementList fail to EncodeInit. Failed: {0}", ret.ToString());
            }
            Int intValue = new Int();
            intValue.Value(3);
            ret = elementEntry.Encode(encodeIter, intValue);
            if (ret < CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("elementEntry fail to encode. Failed: {0}", ret.ToString());
            }
            ret = elementList.EncodeComplete(encodeIter, true);
            if (ret != CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("elementList fail to encodeComplet. Failed: {0}", ret.ToString());
            }
            ret = _genericMsg.EncodeComplete(encodeIter, true);
            if (ret != CodecReturnCode.SUCCESS)
            {
                Console.WriteLine("_genericMsg fail to encodeComplet. Failed: {0}", ret.ToString());
            }
