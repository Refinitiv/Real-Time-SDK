...
namespace LSEG.Eta.ValueAdd.Consumer
{
    ...
    internal class MarketPriceHandler
    {
            ...
            LoginRefresh loginInfo, Service serviceInfo, out ReactorErrorInfo? errorInfo)
        {
            ...
            // APIQA send packed json instead of batch request
            errorInfo = null;

            if (itemNames.Count >= 1)
            {
                ITransportBuffer msgBuf = chnl.GetBuffer(4096, true, out errorInfo); // 4096 - MAX_MSG_SIZE

                Console.WriteLine("\n msgBuf.length before packed: {0}", msgBuf.Length);

                int remainingLength;

                if ((remainingLength = (int)chnl.PackBuffer(msgBuf, out errorInfo)) < 0)
                {
                    Console.WriteLine("Channel.PackBuffer() failed: {0}", errorInfo.ToString());
                    return ReactorReturnCode.FAILURE;
                }

                Console.WriteLine("\n msgBuf.length after packed: {0}", msgBuf.Length);
