...
namespace LSEG.Eta.Example.Common
{
    ...
    public class ProviderDirectoryHandler
    {
        ...
        public CodecReturnCode SendRefresh(IChannel chnl, DirectoryRequest srcDirReqInfo, out Error? error)
        {
            ...
            //APIQA set MAX_BUFFER HIGHER VALUE
            ITransportBuffer msgBuf = chnl.GetBuffer(234567, false, out error);
            // END APIQA
            if (msgBuf is null)
            {
                return CodecReturnCode.FAILURE;
            ...
            // APIQA set 100 services
            string OriginalServiceName = string.Copy(ServiceName);
            int OriginalServiceId = ServiceId;
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                if ((srcDirReqInfo.Filter & Rdm.Directory.ServiceFilterFlags.INFO) != 0)
                {
                    ...
                    m_Service.Info.ServiceName.Data(ServiceName + (i==0?"":i));
                ...
                Service m_Service_tmp = new Service();

                m_Service.Copy(m_Service_tmp);

                m_DirectoryRefresh.ServiceList.Add(m_Service_tmp);
