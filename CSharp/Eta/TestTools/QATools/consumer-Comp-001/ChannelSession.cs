...
namespace LSEG.Eta.Example.Common
{
    ...
    public class ChannelSession
    {
        ...
        //APIQA: Adding vars for transport details.
        public bool showTransportDetails = false;
        private long outBytesTotal = 0;
        private long uncompOutBytesTotal = 0;
        //END APIQA
        ...
        public TransportReturnCode Write(ITransportBuffer buffer, out Error error)
        {
            ...
            if (ret > TransportReturnCode.FAILURE) 
            {
                ...
                {
                    outBytesTotal += writeArgs.BytesWritten;
                    uncompOutBytesTotal += writeArgs.UncompressedBytesWritten;
                    Console.Write("Compression Stats Bytes Out: {0} Uncompressed Bytes Out: {1}\n", outBytesTotal + uncompOutBytesTotal);
                }
                //END API QA
            }
            if (ret > TransportReturnCode.SUCCESS)
                ...
                if (ret == TransportReturnCode.WRITE_CALL_AGAIN)
                {
                    while (ret == TransportReturnCode.WRITE_CALL_AGAIN)
                    {
                        ...
                        if (showTransportDetails)
                        {
                            outBytesTotal += writeArgs.BytesWritten;
                            uncompOutBytesTotal += writeArgs.UncompressedBytesWritten;
                            Console.WriteLine("Compression Stats Bytes Out: " + outBytesTotal + " Uncompressed Bytes Out: " + uncompOutBytesTotal);
                        }
                        //END API QA
                    }
                    if (ret > TransportReturnCode.SUCCESS)
