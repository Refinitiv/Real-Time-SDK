    ...
    public class Consumer : ResponseCallback
    {
        ...
        public void Run()
        {
            ...
            while (true)
            {
                ...
                //API QA
                CompressionType compressType = channelInfo.CompressionType;
                String s_compressType = "None";
                if (compressType == CompressionType.ZLIB)
                    s_compressType = "ZLib";
                if (compressType == CompressionType.LZ4)
                    s_compressType = "LZ4";
                //END API QA
                ...
                Console.WriteLine("  Client To Server Pings: " + channelInfo.ClientToServerPings +
                        ...
                        //API QA
                        "\n  Client Set compressionType: " + (s_compressType) +
                        //END API QA
                        "\n");
            ...
            while (DateTimeOffset.Now.ToUnixTimeMilliseconds() < runtime && channelSession.ShouldRecoverConnection)
            {
                ...
                if (CommandLine.Value("compressionType") != null)
                {
                    if (string.Equals(CommandLine.Value("compressionType"), "ZLib", StringComparison.OrdinalIgnoreCase))
                        copts.CompressionType = CompressionType.ZLIB;
                    else if (string.Equals(CommandLine.Value("compressionType"), "LZ4"))
                        copts.CompressionType = CompressionType.LZ4;
                }
        ...
        public void ProcessResponse(ChannelSession chnl, ITransportBuffer buffer)
        {
            ...
            if (showTransportDetails)
            {
                ...
                Console.Write("Accumulative bytesRead={0}\n", readArgs.BytesRead);
                Console.Write("Accumulative uncompressedBytesRead={0}\n", readArgs.UncompressedBytesRead);
            }
			//END APIQA
