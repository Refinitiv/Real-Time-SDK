...
using Refinitiv.Eta.Transports.proxy;
...
namespace Refinitiv.Eta.Internal.Interfaces
{
    ...
    internal sealed class ChannelBase : IChannel, IInternalChannel
    {
        ...
        private const int READ_MORE_DATA = 1;

        private int m_totalBytesQueued = 0;
        private int m_highWaterMark = 0;
        private bool m_isFlushOrderPending = false;

        // Queues to priority flush order
        private EtaQueue m_highPriorityQueue = new EtaQueue();
        private EtaQueue m_mediumPriorityQueue = new EtaQueue();
        private EtaQueue m_lowPriorityQueue = new EtaQueue();

        private readonly EtaQueue[] m_flushOrder = new EtaQueue[MAX_FLUSH_STRATEGY];
        private int m_flushOrderPosition = 0;
        private readonly TransportBuffer[] m_writeBufferArray = new TransportBuffer[MAX_FLUSH_STRATEGY];
        private readonly List<ArraySegment<byte>> m_ArraySengmentList = new(MAX_FLUSH_STRATEGY);
        private int m_writeArrayMaxPosition = 0;
        private int m_writeArrayPosition = 0;
        ...
        internal ChannelInfo m_ChannelInfo = new ChannelInfo();
        ...
        private readonly Compressor m_ZlibCompressor = new ZlibCompressor();
        private readonly Compressor m_Lz4Compressor = new Lz4Compressor();
       ...
       internal ChannelBase(ConnectOptions connectionOptions, ISocketChannel socketChannel, ChannelState state, int HighWaterMark, 
        ...
        public TransportReturnCode Init(InProgInfo inProg, out Error error)
        {
            ...
            if (ret < TransportReturnCode.SUCCESS)
            {
                ...
                {
                    m_ReadBufferStateMachine.Buffer.Contents = null;
        ...
        private void CloseFromError()
        {
            ...
            else
            {
                ...
                {
                    if(_socketChannel.Socket != null && _socketChannel.Socket.Connected)
                    {
                        _socketChannel.Socket.Shutdown(SocketShutdown.Both);
                    }
                }
                else
                {
...
#region MessagesReadCount
        ...
        public Socket OldSocket => throw new NotImplementedException();
...
#region SocketChannel Event Handlers
        ...
        {
#if DEBUG
            return $"ConnectionType: {ConnectionOptions?.ConnectionType}, Blocking: {ConnectionOptions?.Blocking}, State: {State}";
#else
            return $"ConnectionType: {ConnectionOptions?.ConnectionType}, Blocking: {ConnectionOptions?.Blocking}, State: {State}";
#endif
        }
        ...
        public TransportReturnCode Write(ITransportBuffer msgBuf, WriteArgs writeArgs, out Error error)
        {
            ...
            try
            {
                ...
                {   // message fragmentation
                    BigTransportBuffer bigBuffer = transportBuffer as BigTransportBuffer;

                    if (bigBuffer.IsWritePaused || ProtocolFunctions.WriteAsFragmentedMessage(bigBuffer))
                    {
                        ret = WriteBigBuffer(bigBuffer, writeArgs, out error);
                        if (ret < TransportReturnCode.SUCCESS)
                        {
                            if (ret == TransportReturnCode.FAILURE)
                            {
                    ...
                    {   // The actual payload is less than the maximum fragmentation size
                        TransportBuffer buffer = bigBuffer.FirstBuffer;
        ...
        internal TransportReturnCode WriteBigBuffer(BigTransportBuffer bigBuffer, WriteArgs writeArgs, out Error error)
        {
            error = null;
            TransportReturnCode ret = TransportReturnCode.SUCCESS;
            int bytesLeft = 0;
        ...
        private int WriteGatheringBufferArray(int cumulativeBytesPendingWrite, out Error error)
        {
            ...
            if (SslStream != null) /* Checks for encrypted connection as the SslStream class doesn't support writing a list of buffers */
            {
                for (int i = m_writeArrayPosition; i < m_writeArrayMaxPosition; i++)
                {
                    ...
                    scktBytesWritten += _socketChannel.Send(msgBuffer.Data.Contents, msgBuffer.StartPosition, msgBuffer.Data.Limit - msgBuffer.StartPosition, out error);
            ...
            else
            {
                ...
                {
                    msgBuffer = m_writeBufferArray[i];
                    ArraySegment<byte> buffer = new(msgBuffer.Data.Contents, msgBuffer.StartPosition, msgBuffer.Data.Limit - msgBuffer.StartPosition);
        ...
        internal TransportReturnCode UpdateAppReadBuffer(int entireMessageLength, ReadArgs readArgs)
        {
            ...
            m_AppReadBuffer.Data.Contents = byteBuffer.Contents;
            ...
            if (m_ReadBufferStateMachine.SubState == ReadBufferStateMachine.BufferSubState.PROCESSING_COMPRESSED_MESSAGE)
            {
                ...
                m_AppReadBuffer.Data.Contents = m_DecompressBuffer.Data.Contents;
                    ...
                    case IOCtlCode.COMPRESSION_THRESHOLD:
                        {
                            ...
                            {
                                int compressionThreshold = (m_ChannelInfo.CompressionType == CompressionType.ZLIB ? ZLIB_COMPRESSION_THRESHOLD : LZ4_COMPRESSION_THRESHOLD);
        ...
        internal TransportReturnCode InitChnlSendConnectReq(InProgInfo inProg, out Error error)
        {
            ...
            _socketChannel.Send(m_InitChnlWriteBuffer.Contents, m_InitChnlWriteBuffer.Position, m_InitChnlWriteBuffer.Limit, out error);
        ...
        internal TransportReturnCode InitChnlSendClientAck(InProgInfo inProg, out Error error)
        {
            ...
            SocketChannel.Send(m_InitChnlWriteBuffer.Contents, m_InitChnlWriteBuffer.Position, m_InitChnlWriteBuffer.Limit, out error);
        ...
        internal TransportReturnCode InitChnlFinishSess(InProgInfo inProg, out Error error)
        {
            ...
            else
            {
                ...
                SocketChannel.Send(bufferToWrite.Contents, bufferToWrite.Position, bufferToWrite.Limit, out error);
            }
        ...
        private int GrowGuaranteedOutputBuffers(int numToGrow)
        {
            try
            {
                Transport.GlobalLocker.Enter();
