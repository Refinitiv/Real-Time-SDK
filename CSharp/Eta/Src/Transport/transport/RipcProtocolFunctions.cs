...
namespace Refinitiv.Eta.Transports
{
    class RipcProtocolFunctions : IProtocolFunctions
    {
        ...
        {
            BigTransportBuffer buffer = null;
            error = null;

            try
            {
                buffer = new BigTransportBuffer(size);
            }
            ...
            buffer.FirstBuffer = m_ChannelBase.GetBufferInternal(m_ChannelBase.InternalFragmentSize - RipcDataMessage.HeaderSize, false, RipcDataMessage.HeaderSize);
        ...
        public int RemaingBytesAfterPausing(BigTransportBuffer bigBuffer)
        {
            return bigBuffer.Data.Remaining;
        }

        public int PopulateFragment(BigTransportBuffer bigBuffer, bool firstFragment, TransportBuffer buffer, byte flags, WriteArgs writeArgs)
        {
            int bytesCopied = 0;
            ...
            if(firstFragment)
            {
                buffer.HeaderLength -= buffer.DataStartPosition; // Moves the cursor back before the RIPC header
...
;               return -1;
        ...
        public int WriteCompressed(TransportBuffer buffer, WriteArgs writeArgs, out Error error)
        {
            ...
            int compressedBytesLen = m_ChannelBase.m_Compressor.Compress(buffer, buffer.DataStartPosition, msgLen);
            ...
            buffer.Data.WritePosition = buffer.DataStartPosition;
            ...
            if (retVal >= (int)TransportReturnCode.SUCCESS && compressedBytesLen > MAX_BYTES_FOR_BUFFER)
            {
                ...
                compFragmentBuffer.Data.WritePosition = compFragmentBuffer.DataStartPosition;
        ...
        */
        public int WriteFragmentCompressed(BigTransportBuffer bigBuffer, TransportBuffer fragment, WriteArgs writeArgs, bool firstFragment, out Error error)
        {
            error = null;
