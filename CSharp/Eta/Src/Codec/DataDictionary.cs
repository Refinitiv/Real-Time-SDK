...
namespace Refinitiv.Eta.Codec
{
	...
	sealed public class DataDictionary
	{
		...
		/// <seealso cref="Refinitiv.Eta.Rdm.Dictionary.Types"/>
		...
		/// <seealso cref="Refinitiv.Eta.Rdm.Dictionary.VerbosityValues"/>
		...
		///            <seealso cref="Refinitiv.Eta.Rdm.Dictionary.VerbosityValues"/>. </param>
		/// <param name="error"> ETA error, to be populated in event of an error.
		/// </param>
		/// <returns> <seealso cref="CodecReturnCode"/>
		/// </returns>
        ...
        /// the information from the tables and referencing fields present in this
        /// dictionary. This method supports building the encoded data in multiple parts --
        /// if there is not enough available buffer space to encode the entire dictionary,
        /// subsequent calls can be made to this method, each producing the next segment of fields.
        /// </summary>
        /// <param name="iter">Iterator to be used for encoding.</param>
        ...
        /// <param name="verbosity">The desired verbosity to encode.</param>
        /// <param name="error">error ETA Error, to be populated in event of an error.</param>
        ...
        private static int GetDisplayEntryStringType(Buffer buffer)
        {
            ...
            while (i < buffer.Length + buffer.Position)
            {
                ...
                {
                    if ((i + 1 < buffer.Length) && ((buffer.Data().ReadByteAt(i + 1) & 0xFF) & multibyte_next_mask) == multibyte_next_start)
                    {
                ...
                {
                    if ((i + 2 < buffer.Length) && ((buffer.Data().ReadByteAt(i + 1) & 0xFF) & multibyte_next_mask) == multibyte_next_start
                ...
                {
                    if ((i + 3 < buffer.Length) && ((buffer.Data().ReadByteAt(i + 1) & 0xFF) & multibyte_next_mask) == multibyte_next_start
                            && ((buffer.Data().ReadByteAt(i + 2) & 0xFF) & multibyte_next_mask) == multibyte_next_start
