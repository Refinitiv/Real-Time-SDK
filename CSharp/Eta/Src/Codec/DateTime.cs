    ...
    sealed public class DateTime : IEquatable<DateTime>
	{
		...
		public override string ToString()
		{
			...
			else
			{
				...
				if (_date.Month() != 0)
				{
					oBuffer.Append(string.Format("{0} ", Refinitiv.Eta.Codec.Date.Months[_date.Month() - 1]));
				}
				else
				{
					oBuffer.Append("");
        ...
        private int TranslateMonth(string monthStr)
        {
            ...
            for (i = 0; i < 12; i++)
            {
                if (Refinitiv.Eta.Codec.Date.Months[i].Equals(monthStr, StringComparison.CurrentCultureIgnoreCase))
                {
                    month = i + 1;
                    break;
                }
