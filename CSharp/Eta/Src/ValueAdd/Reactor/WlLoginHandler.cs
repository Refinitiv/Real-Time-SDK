...
namespace LSEG.Eta.ValueAdd.Reactor
{
    ...
    sealed internal class WlLoginHandler : IWlLoginHandler
    {
        ...
        public bool SupportSingleOpen
        {
            get
            {
                if (m_LoginRequest != null && m_LoginRequest.HasAttrib && m_LoginRequest.LoginAttrib.HasSingleOpen)
                {
                    return m_LoginRequest.LoginAttrib.SingleOpen > 0;
                }
                else
                {
        ...
        public bool SupportAllowSuspectData
        {
            get
            {
                if (m_LoginRequest != null && m_LoginRequest.HasAttrib && m_LoginRequest.LoginAttrib.HasAllowSuspectData)
                {
        ...
        public WlLoginHandler(Watchlist watchlist)
        {
            ...
            m_LoginStreamId = WlStreamManager.LOGIN_STREAM_ID;
...
#pragma warning restore CS8625 // Cannot convert null literal to non-nullable reference type.
        ...
        public void ChannelDown()
        {
            ...
            if (Stream != null)
            {
                ...
                {
                    Stream.m_stateTextBuffer.Clear();
                    Stream.State.Clear();
                    Stream.State.Text().Data(Stream.m_stateTextBuffer);
        ...
        public ReactorReturnCode SubmitRequest(WlRequest wlRequest, IRequestMsg requestMsg, bool isReissue, ReactorSubmitOptions submitOptions, out ReactorErrorInfo? errorInfo)
        {
            ...
            {
                // create stream
                Stream = m_Watchlist.StreamManager.LoginStream;
        ...
        public ReactorReturnCode ReadMsg(WlStream wlStream, DecodeIterator dIter, IMsg msg, out ReactorErrorInfo? errorInfo)
        {
            ...
            Debug.Assert(msg.StreamId == Stream.StreamId);
            ...
            if (ret == ReactorReturnCode.SUCCESS)
            {
                switch (wlStream.State.StreamState())
                {
                    ...
                    case StreamStates.REDIRECTED:
                        ...
                        if (wlStream.State.StreamState() == StreamStates.CLOSED || wlStream.State.StreamState() == StreamStates.REDIRECTED)
                        {
                            // close watchlist request
        ...
        ReactorReturnCode ReadRefreshMsg(WlStream wlStream, DecodeIterator dIter, IMsg msg, out ReactorErrorInfo? errorInfo)
        {
            ...
            // call back user
            ret = CallbackUserWithMsgBase("WlLoginHandler.ReadRefreshMsg", msg, m_LoginRefresh, m_Watchlist.StreamIdToWlRequestDict![msg.StreamId], out errorInfo);
        ...
        void SendLoginRequest(bool noRefresh, out ReactorErrorInfo? errorInfo)
        {
            ...
            if (m_Watchlist.ReactorChannel!.EnableSessionManagement())
            {
                ...
                if (LoginRequestForEDP == null && m_Watchlist.ConsumerRole!.RdmLoginRequest != null)
                {
                    newRequest = true;
        ...
        public void OnRequestTimeout(WlStream stream)
        {
            ...
            CallbackUserWithMsgBase("WlLoginHandler.OnRequestTimeout", (Msg)m_StatusMsg, 
                m_LoginStatus, 
