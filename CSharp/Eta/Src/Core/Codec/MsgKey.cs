...
namespace LSEG.Eta.Codec
{
    ...
    /// contents flowing within a particular stream.
    /// </summary>
    /// <remarks>
    ///
    /// This information, in conjunction with <see cref="Msg.DomainType"/> and quality of service
    /// information, can be used to uniquely identify a data stream.
    /// </remarks>
    /// <seealso cref="Msg"/>
	...
	sealed public class MsgKey : IMsgKey
	{
		...
		/// <summary>
		/// Clears <see cref="IMsgKey"/> object. Useful for object reuse during
		/// encoding. While decoding, <see cref="IMsgKey"/> object can be reused without
		...
		[MethodImpl(MethodImplOptions.AggressiveOptimization | MethodImplOptions.AggressiveInlining)]
		internal void CopyReferences(MsgKey msgKey)
		{
			Flags = msgKey.Flags;
		...
		[MethodImpl(MethodImplOptions.AggressiveOptimization | MethodImplOptions.AggressiveInlining)]
		internal CodecReturnCode Copy(IMsgKey destKey, int copyFlags)
		{
			CodecReturnCode ret;
			destKey.Flags = Flags;
		...
		/// types with a <see cref="FilterList"/> payload. Filter identifier values are
		/// defined by the corresponding domain model specification. Must be in the
		/// range of 0 - 4294967296 (2^32).
		/// <ul>
		/// <li>When specified in <see cref="IRequestMsg"/>, filter conveys information
		...
		/// </param>
		/// <returns> <c>CodecReturnCode.SUCCESS</c>, if the key is copied successfully,
		...
		/// </param>
		/// <returns> <c>CodecReturnCode.FAILURE</c> if this key does not contain filter element,
		///     <c>CodecReturnCode.INVALID_DATA</c> if the <paramref name="filterId"/> is greater then maximum filterID value (31), and
		///     <c>CodecReturnCode.SUCCESS</c> otherwise
		/// </returns>
