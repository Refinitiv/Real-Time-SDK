...
namespace LSEG.Eta.Example.VACommon
{
    public class ConnectionArgsParser
    {
        ...
        private int ParseSocketConnectionArgs(string[] args, int argOffset)
        {
            ...
            ConnectionCredentialsParser connCredsParser = new ConnectionCredentialsParser();
            if (connCredsParser.IsStart(args, retCode))
            {
                retCode = connCredsParser.Parse(args, retCode);
                connectionArg.Credentials = connCredsParser.Creds;
            }
            ConnectionTypeParser connTypeParser = new();
            if (connTypeParser.IsStart(args, retCode))
            {
                retCode = connTypeParser.Parse(args, retCode);
                connectionArg.ConnectionTypeItem = connTypeParser.ConnType;
            }
