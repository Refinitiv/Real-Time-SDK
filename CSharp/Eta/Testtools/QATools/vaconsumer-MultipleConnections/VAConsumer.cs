...
namespace LSEG.Eta.ValueAdd.Consumer
{
    ...
    public class VAConsumer : IConsumerCallback, IReactorAuthTokenEventCallback, IReactorOAuthCredentialEventCallback
    {
        ...
        private const string DEFAULT_SRVR_PORT_NO = "14006";
        ...
        private void Init(string[] args)
        {
            ...
            foreach (ConnectionArg connectionArg in m_ConsumerCmdLineParser.ConnectionList)
            {
                ...
                {
                    Console.WriteLine($"Reactor.Connect succeeded.");
                }
        ...
        private void InitChannelInfo(ChannelInfo chnlInfo)
        {
            ...
            ReactorOAuthCredential oAuthCredential = new();
            ...
            if (chnlInfo.ConnectionArg.Credentials != null && !string.IsNullOrEmpty(chnlInfo.ConnectionArg.Credentials.ClientId))
            {
                oAuthCredential.ClientId.Data(chnlInfo.ConnectionArg.Credentials.ClientId);

                if (!string.IsNullOrEmpty(chnlInfo.ConnectionArg.Credentials.ClientSecret))
                {
                    oAuthCredential.ClientSecret.Data(chnlInfo.ConnectionArg.Credentials.ClientSecret);
            ...
            if (chnlInfo.ConnectionArg.Credentials != null && !string.IsNullOrEmpty(chnlInfo.ConnectionArg.Credentials.ClientJwk))
            {
                oAuthCredential.ClientJwk.Data(chnlInfo.ConnectionArg.Credentials.ClientJwk);
            }
            ...
            chnlInfo.ShouldEnableEncrypted = chnlInfo.ConnectionArg.ConnectionTypeItem != null ?
                chnlInfo.ConnectionArg.ConnectionTypeItem.EnableEncrypted :
                false;
            ...
            if (chnlInfo.ShouldEnableEncrypted)
            {
                ...
                cOpt.ConnectionType = ConnectionType.ENCRYPTED;
                cOpt.EncryptionOpts.EncryptedProtocol = chnlInfo.ConnectionArg.ConnectionTypeItem.EncryptedProtocolType;
            }
