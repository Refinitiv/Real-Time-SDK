        ...
        try
        {
            ...
            switch (Consumer._TEST)
            {
                ...
                case 0:
                    ...
                    consumer.RegisterClient(new RequestMsg().ServiceName(Consumer._serviceName).Name("LSEG.L").Payload(view1), appClient, 1);
                    consumer.RegisterClient(new RequestMsg().ServiceName(Consumer._serviceName).Name("LSEG.L").Payload(view2), appClient, 2);
                    break;
                case 11:
                    Console.WriteLine("***APIQA TEST 11 : ReqMsg.qos(Timeliness.REALTIME, Rate.TICK_BY_TICK) & view1(2,22,25) view2(2,30,31).***");
                    consumer.RegisterClient(new RequestMsg().ServiceName(Consumer._serviceName).Name("LSEG.L").Payload(view1).Qos(OmmQos.Timelinesses.REALTIME, OmmQos.Rates.TICK_BY_TICK), appClient, 1);
                    consumer.RegisterClient(new RequestMsg().ServiceName(Consumer._serviceName).Name("LSEG.L").Payload(view2).Qos(OmmQos.Timelinesses.REALTIME, OmmQos.Rates.TICK_BY_TICK), appClient, 2);
                    break;
                case 12:
                    ...
                    consumer.RegisterClient(new RequestMsg().ServiceName(Consumer._serviceName).Name("LSEG.L").Payload(view1).Qos(OmmQos.Timelinesses.INEXACT_DELAYED, OmmQos.Rates.TICK_BY_TICK), appClient, 1);
                    consumer.RegisterClient(new RequestMsg().ServiceName(Consumer._serviceName).Name("LSEG.L").Payload(view2).Qos(OmmQos.Timelinesses.INEXACT_DELAYED, OmmQos.Rates.TICK_BY_TICK), appClient, 2);
                    break;
                case 21:
                    Console.WriteLine("***APIQA TEST 21 : ReqMsg.qos(Timeliness.REALTIME, Rate.JIT_CONFLATED) & view1(2,22,25) view2(2,30,31).***");
                    consumer.RegisterClient(new RequestMsg().ServiceName(Consumer._serviceName).Name("LSEG.L").Payload(view1).Qos(OmmQos.Timelinesses.REALTIME, OmmQos.Rates.JUST_IN_TIME_CONFLATED), appClient, 1);
                    consumer.RegisterClient(new RequestMsg().ServiceName(Consumer._serviceName).Name("LSEG.L").Payload(view2).Qos(OmmQos.Timelinesses.REALTIME, OmmQos.Rates.JUST_IN_TIME_CONFLATED), appClient, 2);
                    break;
