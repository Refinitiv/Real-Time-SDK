...
internal class AppClient : IOmmConsumerClient
{
	...
	public void OnGenericMsg(GenericMsg genericMsg, IOmmConsumerEvent @event)
	{
		...
		//APIQA
		GenericMsg genericMsgClone = new GenericMsg(genericMsg);

		if (genericMsg.HasPermissionData && count % 2 == 0)
		{
			ByteBuffer newPermissionByteBuffer = new ByteBuffer(genericMsg.PermissionData().Length);
			newPermissionByteBuffer.Put(genericMsg.PermissionData().AsByteArray(), 0, genericMsg.PermissionData().Length);
			newPermissionByteBuffer.Flip();
			Array.Reverse(newPermissionByteBuffer.Contents);
			genericMsgClone.PermissionData(new EmaBuffer(newPermissionByteBuffer.Contents));
		}
		...
		GenericMsg genericMsgClone2 = new GenericMsg(genericMsgClone);
		Console.WriteLine("Clone2");
		Console.WriteLine(genericMsgClone2);
		@event.Consumer.Submit(genericMsgClone2, @event.Handle);
		//END APIQA
