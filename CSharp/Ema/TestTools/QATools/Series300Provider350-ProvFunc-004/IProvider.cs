    ...
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            long rwfFld = provider.RegisterClient(new RequestMsg().Name("RWFFld").Filter(appClient.Filter)
                .InterestAfterRefresh(false).ServiceName("INVALID").DomainType(EmaRdm.MMT_DICTIONARY),
                appClient);
            ...
            long rwfFld1 = provider.RegisterClient(new RequestMsg().Name("RWFFld").Filter(appClient.Filter)
                .InterestAfterRefresh(false).ServiceId(999).DomainType(EmaRdm.MMT_DICTIONARY),
                appClient);

            long rwfEnum1 = provider.RegisterClient(new RequestMsg().Name("RWFEnum").Filter(appClient.Filter)
                .InterestAfterRefresh(false).ServiceId(999).DomainType(EmaRdm.MMT_DICTIONARY),
                appClient);
