...
class AppClient : IOmmProviderClient
{
    ...
    void ProcessAppDomainRequest(RequestMsg reqMsg, IOmmProviderEvent providerEvent)
    {
        ...
        providerEvent.Provider.Submit(new RefreshMsg().DomainType(reqMsg.DomainType()).Name(reqMsg.Name())
    ...
    void ProcessAppDomainGenericMsg(GenericMsg genericMsg, IOmmProviderEvent providerEvent)
    {
        ...
        ++count;
        elementList.AddInt("valueFromProvider", count * 10);

        GenericMsg msg = new GenericMsg()
                .DomainType(APP_DOMAIN)
                .Name("genericMsg")
                .ExtendedHeader(new EmaBuffer(new byte[] { 1, 2, 3 }))
                .PermissionData(new EmaBuffer(new byte[] { 50, 51, 52, 53 }));

        if (count % 5 == 0)
            msg.Payload(elementList.Complete());

        providerEvent.Provider.Submit(msg, providerEvent.Handle);
