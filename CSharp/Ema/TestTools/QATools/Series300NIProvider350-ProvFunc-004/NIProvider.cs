        ...
        switch (msg.Payload().DataType)
        {
            case DataType.DataTypes.SERIES:
                ...
                if (fldDictComplete && enumTypeComplete)
                {
                    ...
                    if (dumpDictionary) Console.WriteLine(dataDictionary);
                }
    ...
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            long rwfFld = provider.RegisterClient(new RequestMsg().Name("RWFFld").Filter(appClient.Filter)
                .ServiceName("INVALID").DomainType(EmaRdm.MMT_DICTIONARY).InterestAfterRefresh(false), appClient);

            long rwfEnum = provider.RegisterClient(new RequestMsg().Name("RWFEnum").InterestAfterRefresh(false).Filter(appClient.Filter)
                    .ServiceName("INVALID").DomainType(EmaRdm.MMT_DICTIONARY), appClient);
            long rwfFld1 = provider.RegisterClient(new RequestMsg().Name("RWFFld").Filter(appClient.Filter)
                .ServiceId(999).DomainType(EmaRdm.MMT_DICTIONARY).InterestAfterRefresh(false), appClient);

            long rwfEnum1 = provider.RegisterClient(new RequestMsg().Name("RWFEnum").InterestAfterRefresh(false).Filter(appClient.Filter)
                .ServiceId(999).DomainType(EmaRdm.MMT_DICTIONARY), appClient);
