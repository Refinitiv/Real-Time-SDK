...
class AppClient : IOmmProviderClient
{
    ...
    private int m_FragmentationSize = 96000;
        ...
        if (reqMsg.Name().Equals("RWFFld"))
        {
            ...
            while (!m_Result)
            {
                ...
                providerEvent.Provider.Submit(m_RefreshMsg.Name(reqMsg.Name()).ServiceName(reqMsg.ServiceName())
                    .DomainType(EmaRdm.MMT_DICTIONARY).Filter(reqMsg.Filter()).Payload(m_Series.Complete()).Complete(m_Result)
                    .Solicited(true),
                    providerEvent.Handle);
...
public class IProvider
{
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            provider = new OmmProvider(new OmmIProviderConfig().AdminControlDictionary(OmmIProviderConfig.AdminControlMode.API_CONTROL), appClient);
            ...
            for (int i = 0; i < 60; i++)
            {
                ...
                if (i == 2)
                {
                    provider.Submit(new StatusMsg().Name("RWFFld").ServiceId(1).DomainType(EmaRdm.MMT_DICTIONARY)
                        .State(OmmState.StreamStates.OPEN, OmmState.DataStates.SUSPECT, OmmState.StatusCodes.NONE, "Open Suspect Status on dictionary"), 0);
                }
                if (i == 6)
                {
                    provider.Submit(new StatusMsg().Name("RWFFld").ServiceName("DIRECT_FEED").DomainType(EmaRdm.MMT_DICTIONARY)
                        .State(OmmState.StreamStates.CLOSED, OmmState.DataStates.SUSPECT, OmmState.StatusCodes.NONE, "Closed Status on dictionary"), 0);
                }
