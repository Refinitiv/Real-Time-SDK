        ...
        try
        {
            if (reqMsg.Name().Equals("RWFFld"))
            {
                ...
                while (!m_Result)
                {
                    ...
                    // APIQA
                    providerEvent.Provider.Submit(m_RefreshMsg.Name(reqMsg.Name()).ServiceName("INVALID_SERVICE")
                        .DomainType(EmaRdm.MMT_DICTIONARY).Filter(reqMsg.Filter()).Payload(m_Series.Complete()).Complete(m_Result).Solicited(true),
                        providerEvent.Handle);
                    // END APIQA
