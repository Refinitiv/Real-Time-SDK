...
public class IProvider
{
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            long rwfFld = provider.RegisterClient(new RequestMsg().Name("RWFFld").Filter(EmaRdm.DICTIONARY_VERBOSE)
                .ServiceName("DIRECT_FEED").DomainType(EmaRdm.MMT_DICTIONARY).InterestAfterRefresh(false),
                appClient, ddict);

            long rwfEnum = provider.RegisterClient(new RequestMsg().Name("RWFEnum").Filter(EmaRdm.DICTIONARY_VERBOSE)
                .ServiceName("DIRECT_FEED").DomainType(EmaRdm.MMT_DICTIONARY).InterestAfterRefresh(false),
                appClient, ddict);

            long rwfFld1 = provider.RegisterClient(new RequestMsg().Name("RWFFld").Filter(EmaRdm.DICTIONARY_VERBOSE)
                .ServiceName("DIRECT_FEED").DomainType(EmaRdm.MMT_DICTIONARY).InterestAfterRefresh(false),
                appClient, ddict1);

            long rwfEnum1 = provider.RegisterClient(new RequestMsg().Name("RWFEnum").Filter(EmaRdm.DICTIONARY_VERBOSE)
                .ServiceName("DIRECT_FEED").DomainType(EmaRdm.MMT_DICTIONARY).InterestAfterRefresh(false),
                appClient, ddict1);

            long rwfFld2 = provider.RegisterClient(new RequestMsg().Name("RWFFld").Filter(EmaRdm.DICTIONARY_VERBOSE)
                .ServiceName("DIRECT_FEED").DomainType(EmaRdm.MMT_DICTIONARY).InterestAfterRefresh(false),
                appClient, ddict2);

            long rwfEnum2 = provider.RegisterClient(new RequestMsg().Name("RWFEnum").Filter(EmaRdm.DICTIONARY_VERBOSE)
                .ServiceName("DIRECT_FEED").DomainType(EmaRdm.MMT_DICTIONARY).InterestAfterRefresh(false),
                appClient, ddict2);
            // END APIQA
