            ...
            for (int i = 0; i < 60; i++)
            {
                ...
                if (i == 60)
                {
                    provider.Submit(new StatusMsg().Name("RWFFld").ServiceName("DIRECT_FEED").DomainType(EmaRdm.MMT_DICTIONARY)
                        .State(OmmState.StreamStates.CLOSED, OmmState.DataStates.SUSPECT, OmmState.StatusCodes.NONE, "Closed Status on dictionary"),
                        appClient.ItemHandle);
                }
                if (i == 6)
                {
                    provider.Submit(new UpdateMsg().Name("RWFFld").ServiceName("DIRECT_FEED").DomainType(EmaRdm.MMT_DICTIONARY),
                        appClient.ItemHandle);
                }
            }
        }
