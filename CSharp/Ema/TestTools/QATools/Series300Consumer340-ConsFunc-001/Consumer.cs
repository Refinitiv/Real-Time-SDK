...
internal class AppClient : IOmmConsumerClient
{
	...
	public void OnRefreshMsg(RefreshMsg refreshMsg, IOmmConsumerEvent @event)
	{
			...
			refreshMsg.State().DataState == OmmState.DataStates.OK)
		{
			...
			//APIQA
            @event.Consumer.Submit(postMsg.PostId(postId++).ServiceId(1701)
		...
		//APIQA
		Console.WriteLine("Clone RefreshMsg");
		RefreshMsg cloneRefreshMsg = new RefreshMsg(refreshMsg);
		Console.WriteLine(cloneRefreshMsg);
		Console.WriteLine("Clone Refresh Item State: " + cloneRefreshMsg.State());
		//END APIQA
	}
	...
	public void OnUpdateMsg(UpdateMsg updateMsg, IOmmConsumerEvent @event) 
	{
		...
		//APIQA	
		Console.WriteLine("Clone UpdateMsg");
		UpdateMsg cloneUpdateMsg = new UpdateMsg(updateMsg);
		Console.WriteLine(cloneUpdateMsg);
	...
	public void OnStatusMsg(StatusMsg statusMsg, IOmmConsumerEvent @event) 
	{
		...
		Console.WriteLine();
		//APIQA	
		Console.WriteLine("Clone StatusMsg");
		StatusMsg cloneStatusMsg = new StatusMsg(statusMsg);
		Console.WriteLine(cloneStatusMsg);
		if (cloneStatusMsg.HasState)
			Console.WriteLine("Item State: " + cloneStatusMsg.State());
		//END APIQA	
	}
	...
	public void OnAckMsg(AckMsg ackMsg, IOmmConsumerEvent @event)
	{
		...
		//APIQA	
		Console.WriteLine("Clone AckMsg");
		AckMsg cloneAckMsg = new AckMsg(ackMsg);
		Console.WriteLine(cloneAckMsg);
		Decode(cloneAckMsg);
		//END APIQA	
	}
...
public class Consumer 
{
	public static void Main()
	{
		...
		try
		{
			...
			consumer.RegisterClient(new RequestMsg().ServiceName("NI_PUB").Name("IBM.N"), appClient, consumer);
