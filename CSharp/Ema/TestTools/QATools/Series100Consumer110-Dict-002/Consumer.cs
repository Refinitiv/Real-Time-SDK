...
internal class AppClient : IOmmConsumerClient
{
    ...
    private void Decode(ElementList elementList)
    {
        ...
        {
            Console.Write(" Name = " + elementEntry.Name + (elementEntry.Load is not null ? (" DataType: " + AsString(elementEntry.Load!.DataType)) : string.Empty) + " Value: ");
    ...
    private static void Decode(OmmArray array)
    {
    ...
    private void Decode(Series series)
    {
...
public class Consumer
{
    public static void Main()
    {
        try
        {
            ...
            config.AddAdminMsg(reqMsg.Clear().DomainType(EmaRdm.MMT_DICTIONARY).Filter(EmaRdm.DICTIONARY_NORMAL).InterestAfterRefresh(false).ServiceName("DIRECT_FEED").Name("RWFFld"));
            config.AddAdminMsg(reqMsg.Clear().DomainType(EmaRdm.MMT_DICTIONARY).Filter(EmaRdm.DICTIONARY_NORMAL).InterestAfterRefresh(false).ServiceName("DIRECT_FEED").Name("RWFEnum"));
            config.UserName("user");
            OmmConsumer consumer = new(config);
            long dicthandle = consumer.RegisterClient(reqMsg.Clear().ServiceName("DIRECT_FEED").DomainType(EmaRdm.MMT_DICTIONARY).Filter(EmaRdm.DICTIONARY_NORMAL).Name("RWFFld"), appClient);
            long dicthandle2 = consumer.RegisterClient(reqMsg.Clear().ServiceName("DIRECT_FEED").DomainType(EmaRdm.MMT_DICTIONARY).Filter(EmaRdm.DICTIONARY_NORMAL).Name("RWFEnum"), appClient);
