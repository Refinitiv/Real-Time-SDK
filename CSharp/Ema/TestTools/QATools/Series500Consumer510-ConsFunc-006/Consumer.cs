        ...
        try
        {
            ...
            switch (Consumer._TEST)
            {
                ...
                case 0:
                    ...
                    consumer.RegisterClient(new RequestMsg().ServiceListName("SVG1").Payload(batchView), appClient);
                    break;
                case 11:
                    Console.WriteLine("***APIQA TEST 11 : ReqMsg.qos(Timeliness.REALTIME, Rate.TICK_BY_TICK).***");
                    consumer.RegisterClient(new RequestMsg().ServiceListName("SVG1").Payload(batchView).Qos(OmmQos.Timelinesses.REALTIME, OmmQos.Rates.TICK_BY_TICK), appClient);
                    break;
                case 12:
                    Console.WriteLine("***APIQA TEST 12 : ReqMsg.qos(Timeliness.BEST_DELAYED_TIMELINESS, Rate.TICK_BY_TICK).***");
                    consumer.RegisterClient(new RequestMsg().ServiceListName("SVG1").Payload(batchView).Qos(OmmQos.Timelinesses.INEXACT_DELAYED, OmmQos.Rates.TICK_BY_TICK), appClient);
                    break;
                case 21:
                    Console.WriteLine("***APIQA TEST 21 : ReqMsg.qos(Timeliness.REALTIME, Rate.JIT_CONFLATED).***");
                    consumer.RegisterClient(new RequestMsg().ServiceListName("SVG1").Payload(batchView).Qos(OmmQos.Timelinesses.REALTIME, OmmQos.Rates.JUST_IN_TIME_CONFLATED), appClient);
                    break;
