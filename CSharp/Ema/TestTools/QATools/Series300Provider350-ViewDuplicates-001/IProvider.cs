...
class AppClient : IOmmProviderClient
{
    ...
    void ProcessLoginRequest(RequestMsg reqMsg, IOmmProviderEvent providerEvent)
    {
        ...
        list.AddUInt(EmaRdm.ENAME_SUPPORT_VIEW, 1);
    ...
    {
        // API QA
        Console.WriteLine("processMarketPriceRequest: " + reqMsg);
        Console.WriteLine("reqMsg.hasView(): " + reqMsg.HasView);
        // END API QA
        if (ItemHandle != 0)
        {
            ProcessInvalidItemRequest(reqMsg, providerEvent);
