...
class AppClient : IOmmProviderClient
{
    ...
    {
        providerEvent.Provider.Submit(m_RefreshMsg.Clear().DomainType(EmaRdm.MMT_LOGIN).Name(reqMsg.Name()).NameType(EmaRdm.USER_NAME)
    ...
    void ProcessDictionaryRequest(RequestMsg reqMsg, IOmmProviderEvent providerEvent)
    {
        ...
        if (reqMsg.Name().Equals("RWFFld"))
        {
            ...
            while (!m_Result)
            {
                ...
                // APIQA
                providerEvent.Provider.Submit(m_RefreshMsg.Name(reqMsg.Name()).ServiceId(1).DomainType(EmaRdm.MMT_DICTIONARY)
    ...
    void ProcessMarketPriceRequest(RequestMsg reqMsg, IOmmProviderEvent providerEvent)
    {
        ...
        // APIQA
        providerEvent.Provider.Submit(m_RefreshMsg.Clear().Name(reqMsg.Name()).ServiceId(1).Solicited(true)
...
public class IProvider
{
    public static void Main(string[] args)
    {
        ...
        try
        {
            ...
            provider = new OmmProvider(new OmmIProviderConfig().AdminControlDictionary(OmmIProviderConfig.AdminControlMode.API_CONTROL),
                appClient);
