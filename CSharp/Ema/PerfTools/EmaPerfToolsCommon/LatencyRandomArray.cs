...
namespace LSEG.Ema.PerfTools.Common
{
	...
	public class LatencyRandomArray
    {
		...
		public PerfToolsReturnCode Create(LatencyRandomArrayOptions opts)
		{
			...
			{
				Console.WriteLine("Random Array: Total message m_rate is zero.\n");
				return PerfToolsReturnCode.FAILURE;
			}

			if (opts.LatencyMsgsPerSec == 0)
			{
				Console.WriteLine("Random Array: Latency message m_rate is zero.\n");
				return PerfToolsReturnCode.FAILURE;
			}

			if (opts.LatencyMsgsPerSec > opts.TotalMsgsPerSec)
			{
				Console.WriteLine("Random Array: Latency message m_rate is greater than total message m_rate.\n");
				return PerfToolsReturnCode.FAILURE;
			}
