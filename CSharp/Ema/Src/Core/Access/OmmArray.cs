...
public sealed class OmmArray : Data, IEnumerable<OmmArrayEntry>
{
    ...
    /// </summary>
    public override int DataType { get => Access.DataType.DataTypes.ARRAY; }
    ...
    /// </summary>
    /// <returns>string representing current <see cref="OmmArray"/> instance.</returns>
    ...
    internal override string ToString(int indent)
    {
        ...
        else
        {
            ...
            foreach (OmmArrayEntry arrayEntry in this)
            {
                ...
                if (load.DataType == Access.DataType.DataTypes.BUFFER)
                    m_ToString.AppendLine().Append(load.ToString());
                else if (load.DataType == Access.DataType.DataTypes.ERROR)
    ...
    {
        // todo: Decode(DecodeIterator) method was introduced for primitive ETA types -
        // all they need is the decode iterator.
        //
        // but OmmArray and Containers, which also extend the same Data base class, also
        // require at least the underlying buffer.
        //
        // In case of OmmArray, underlying buffer is needed to create a new DecodeIterator
        // for each new OmmArrayEnumerator. If there was a way to clone DecodeIterator
        // instance, then OmmArrayEnumerator could work directly with it, without the need
        // for buffer, codec version, etc.
        throw new OmmInvalidUsageException($"OmmArray does not support Decode(DecodeIterator)");
    }
