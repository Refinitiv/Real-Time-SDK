...
namespace LSEG.Ema.Access
{
    internal class LoginItem<T> : SingleItem<T>, ITimeoutClient
    {
        ...
        public LoginItem(OmmBaseImpl<T> baseImpl, T client, object? closure)
        ...
        public void HandleTimeoutEvent()
        {
            ...
            if(LoginChannelList is null || LoginChannelList.Count == 0)
            {
                return;
            }
        ...
        bool Submit(LoginRequest rdmRequestMsg)
        {
            ...
            else
            {
                ...
                if (m_OmmBaseImpl.LoggerClient.IsErrorEnabled)
                {
                    message.Append("Internal error: ReactorChannel.Submit() failed in LoginItem.Submit(RequestMsg)")
    ...
    internal class LoginCallbackClient<T> : CallbackClient<T>, IRDMLoginMsgCallback
    {
        ...
        private Locker m_LoginItemLock  = new WriteLocker(new ReaderWriterLockSlim(LockRecursionPolicy.SupportsRecursion));
        ...
        {
            if(m_LoginItemList is null)
            {
                m_LoginItemList = new();
            }

            LoginItem<T> item = new LoginItem<T>(m_OmmBaseImpl, client, closure); // TODO: reuse object from the pool
        ...
        ReactorCallbackReturnCode ProcessRefreshMsg(IMsg msg, ReactorChannel reactorChannel, LoginMsg loginMsg)
        {
            ...
            else
            {
                ...
                m_RefreshMsg.Decode(reactorChannel.MajorVersion, reactorChannel.MinorVersion, m_EncodedBuffer, null, null);
            }
        ...
        ReactorCallbackReturnCode ProcessStatusMsg(IMsg msg, ReactorChannel reactorChannel, LoginMsg loginMsg)
        {
            ...
            else
            {
                ...
                m_StatusMsg.Decode(reactorChannel.MajorVersion, reactorChannel.MinorVersion, m_EncodedBuffer, null, null);
            }
        ...
        {
            if (m_EncodedBuffer.Data() is null)
            {
