...
namespace LSEG.Ema.Access
{
    ...
    internal class DirectoryCallbackClient<T> : CallbackClient<T>, IDirectoryMsgCallback
    {
        ...
        public ReactorCallbackReturnCode RdmDirectoryMsgCallback(RDMDirectoryMsgEvent dirMsgEvent)
        {
            ...
            if (item != null)
               return ProcessCallback(dirMsgEvent, reactorChannel!, (SingleItem<T>) item);
        ...
        ReactorCallbackReturnCode ProcessCallback(RDMDirectoryMsgEvent dirMsgEvent, ReactorChannel reactorChannel, SingleItem<T> item)
        {
        ...
        void ProcessDirectoryPayload(List<Service> serviceList, ReactorChannel channel)
        {
            ...
            if (chnlInfo is null)
            {
                if (m_OmmBaseImpl.LoggerClient.IsErrorEnabled)
                {
                    m_OmmBaseImpl.LoggerClient.Error(CLIENT_NAME, "nternal error: no ReactorChannel.UserSpecObj");
                }
    ...
    internal class DirectoryItem<T> : SingleItem<T>
    {
        ...
        public override void Reset(OmmBaseImpl<T> ommBaseImpl, T client, object? closure, Item<T>? item)
        {
            base.Reset(ommBaseImpl, client, closure, item);
        ...
        bool Submit(IRequestMsg requestMsg, string? serviceName)
        {
            ...
            int msgKeyFlags = requestMsg.MsgKey.Flags;
