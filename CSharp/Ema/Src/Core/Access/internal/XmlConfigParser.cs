        ...
        private void ParseConsumerGroup(XmlNode ConsumerNode)
        {
            ...
            foreach (XmlNode consumerListNode in CurrNodeList)
            {
                ...
                if (CurrNode2 != null)
                {
                    ...
                    try
                    {
                        tmpConfig.ReconnectMaxDelay = Utilities.Convert_uint_int(uint.Parse(XmlAttribute.Value));
                    }
                    catch (SystemException)
                    {
                ...
                if (CurrNode2 != null)
                {
                    ...
                    try
                    {
                        tmpConfig.ReconnectMinDelay = Utilities.Convert_uint_int(uint.Parse(XmlAttribute.Value));
                    }
                    catch (SystemException)
                    {
                ...
                foreach (XmlNode node in consumerListNode.ChildNodes)
                {
                    if (node.NodeType == XmlNodeType.Element && !xmlNodeNames.Contains(node.Name))
                    {
                        OmmConfig.ConfigErrorLog?.Add($"Unknown Consumer entry element: {node.Name} in {tmpConfig.Name}", LoggerLevel.ERROR);
                    }
                }
            }
        }
        ...
        private void ParseClientChannelGroup(XmlNode ChannelNode)
        {
            ...
            foreach (XmlNode channelListNode in CurrNodeList)
            {
                ...
                foreach (XmlNode node in channelListNode.ChildNodes)
                {
                    if (node.NodeType == XmlNodeType.Element && !xmlNodeNames.Contains(node.Name))
                    {
                        OmmConfig.ConfigErrorLog?.Add($"Unknown Channel entry element: {node.Name} in {tmpConfig.Name}", LoggerLevel.ERROR);
                    }
                }
            }
        }
        ...
        private void ParseLoggerGroup(XmlNode LoggerNode)
        {
            ...
            foreach (XmlNode loggerListNode in CurrNodeList)
            {
                ...
                if (CurrNode2 != null)
                {
                    ...
                    {
                        throw new OmmInvalidConfigurationException("The value attribute in the Logger IncludeDateInLoggerOutput element is incorrectly formatted. Correct values are: \"0\" or \"1\".");
                    }
                }
                ...
                if (CurrNode2 != null)
                {
                    ...
                    {
                        throw new OmmInvalidConfigurationException("The value attribute in the Logger NumberOfLogFiles element is incorrectly formatted. Correct format is an unsigned numeric string.");
                    }
                }
                ...
                if (CurrNode2 != null)
                {
                    ...
                    {
                        throw new OmmInvalidConfigurationException("The value attribute in the Logger MaxLogFileSize element is incorrectly formatted. Correct format is an unsigned numeric string.");
                    }
                }
                ...
                foreach (XmlNode node in loggerListNode.ChildNodes)
                {
                    if (node.NodeType == XmlNodeType.Element && !xmlNodeNames.Contains(node.Name))
                    {
                        OmmConfig.ConfigErrorLog?.Add($"Unknown Logger entry element: {node.Name} in {tmpConfig.Name}", LoggerLevel.ERROR);
                    }
                }
            }
        }
        ...
        private void ParseDictionaryGroup(XmlNode DictionaryNode)
        {
            ...
            foreach (XmlNode dictionaryListNode in CurrNodeList)
            {
                ...
                foreach (XmlNode node in dictionaryListNode.ChildNodes)
                {
                    if (node.NodeType == XmlNodeType.Element && !xmlNodeNames.Contains(node.Name))
                    {
                        OmmConfig.ConfigErrorLog?.Add($"Unknown Dictionary entry element: {node.Name} in {tmpConfig.Name}", LoggerLevel.ERROR);
                    }
                }
            }
        }
