...
internal class DictionaryCallbackClient<T> : CallbackClient<T>, IDictionaryMsgCallback
{
    ...
    internal DictionaryCallbackClient(OmmBaseImpl<T> baseImpl)
        : base(baseImpl, "")
    {
    ...
    {
        if (m_OmmBaseImpl.ConfigImpl.AdminFieldDictionaryRequest is not null
        ...
        else if (m_OmmBaseImpl.ConfigImpl.AdminFieldDictionaryRequest is not null
        ...
        else if (m_OmmBaseImpl.ConfigImpl.AdminFieldDictionaryRequest is null
            && m_OmmBaseImpl.ConfigImpl.AdminEnumDictionaryRequest is not null)
        {
            StringBuilder temp = commonImpl.GetStrBuilder();
            temp.Append("Invalid dictionary configuration was specified through the AddAdminMsg() method").Append(ILoggerClient.CR)
        ...
        if ((dictionary is Rdm.DataDictionary)
            && dictionary.rsslDataDictionary() is not null)
        {
    ...
    public ReactorCallbackReturnCode RdmDictionaryMsgCallback(RDMDictionaryMsgEvent msgEvent)
    {
        ...
        if (m_ChannelDictionaryList is not null)
        {
    ...
    internal bool DownloadDictionary(ServiceDirectory directory)
    {
        ...
        {
            if (m_rsslLocalDictionary is not null
                && m_rsslLocalDictionary.NumberOfEntries > 0)
            {
                if (directory!.ChannelInfo!.ParentChannel is not null)
                {
        ...
        if (directory?.ChannelInfo?.DataDictionary is not null
            ...
            && m_OmmBaseImpl.ConfigImpl.AdminEnumDictionaryRequest is not null)
        {
            if (m_OmmBaseImpl.ConfigImpl.AdminFieldDictionaryRequest.ServiceId == directory!.Service!.ServiceId
                || (m_OmmBaseImpl.ConfigImpl.FieldDictionaryRequestServiceName is not null
        ...
        ReactorSubmitOptions rsslSubmitOptions = m_OmmBaseImpl.SubmitOptions;
    ...
    {
        if (m_rsslLocalDictionary is null)
    ...
    {
        if (m_ChannelDictPool is not null && m_ChannelDictPool.Count != 0)
    ...
    {
        if (m_ChannelDictionaryList is not null && m_ChannelDictionaryList.Count != 0)
    ...
    internal bool IsDictionaryReady
    {
        get
        {
            ...
            {
                if (m_ChannelDictionaryList is null || m_ChannelDictionaryList.Count == 0)
                    return false;
    ...
    {
        get => m_rsslLocalDictionary is not null;
    }

    internal Int rsslCurrentFid()
    {
        if (m_rsslCurrentFid is null)
...
{
    internal DictionaryCallbackClientConsumer(OmmBaseImpl<IOmmConsumerClient> baseImpl)
        : base(baseImpl)
    {
    ...
    public override void NotifyOnAllMsg(Access.Msg msg)
    {
...
class ChannelDictionary<T>
{
    ...
    internal Eta.Common.Locker ChannelDictionaryLock
    {
        get
        {
            m_ChannelDictLock ??= new Eta.Common.ReadLocker(new ReaderWriterLockSlim());
    ...
    {
        if (m_ListenerList is null || m_ListenerList.Count == 0)
            return;
...
internal class DictionaryItem<T> : SingleItem<T>, ITimeoutClient
{
    ...
    public DictionaryItem(OmmBaseImpl<T> baseImpl, T client, Object? closure)
    ...
    public override void Reset(OmmBaseImpl<T> baseImpl, T client, Object? closure, Item<T>? item)
    {
    ...
    public override bool Open(Ema.Access.RequestMsg reqMsg)
    {
    ...
    public override bool Close()
    {
        ...
        else
        {
            if (m_NeedRemoved == false)
            {
                ...
                DictionaryCallbackClient<T> dictCBClient = m_OmmBaseImpl.DictionaryCallbackClient!;
                if (dictCBClient.ChannelDictionaryList is not null
    ...
    public Eta.Common.Locker UserLock
    {
