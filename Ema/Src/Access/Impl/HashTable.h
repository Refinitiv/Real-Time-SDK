...
namespace access {
	...
	private:
		struct Node {
		public:
			Node(HashTable & parent, const KeyType & k, const ValueType & v, Node * previous = 0,
				 Node *next = 0)
				: parent(parent), key(k), value(v), previous(previous), next(next) { ++parent.elementCount; }
			~Node() { --parent.elementCount; }
